...
//begin by zhuhengxuan at 20190418
	...
	//add by qxm at 20191107
    private String mScheme = "";
    public boolean mIsTimeShift = false;
    public boolean mIsTimeShift_last = false;
    public boolean mIsTimeShift_next = false;
    public long mTimeShiftToMillis = 0;
	
	public boolean mSeekFlag_start = false;
	public boolean mSeekFlag_already = false;
	public boolean mBufferNoNeedFlag = false;
	private int mDisplayTime = 9;
	public static int mPlayerID = 0;
	//qxm add end
	/**
    ...
    private void setDataSource(String path, String[] keys, String[] values)
		...
		//begin: add by qxm at 20191106
		Log.d(TAG,"setdatasource here and url :"+path);
		if(path.contains("playseek")){
			//mSeekStartSend = true;
			int a = path.indexOf("playseek=");
			String s1 = path.substring(a+9, a+17);
			String s2 = path.substring(a+17, a+23);
			String utc_time = s1 + "T" +s2 +"Z";
		    Log.d(TAG,"playseek flag set:" +utc_time);
			mTimeShiftToMillis = getTimeMillisFromUTCString(utc_time) + 8*60*60*1000;
			Log.d(TAG, "Time shifting to : " + mTimeShiftToMillis);
			//mIsTimeShift_next = true;		
			if(mSeekFlag_start){
				Log.d(TAG,"playseek flag set already.");
				mSeekFlag_already = true;
				mSeekNeedResend = true;
			}else{
				Log.d(TAG,"playseek flag start set.");
				mSeekFlag_start = true;
			}
		}else{
			mSeekFlag_start = false;
			mSeekFlag_already = false;
			mStartCount = 0;
		}
		
		if(!isDuringTimeShift()){
			if(null != info)
		        info = null;
		}
        //qxm add end
    ...
    public boolean isTimeShift() {
        return mSeekFlag_start;
    }

    public boolean isStartingTimeShift() {
        return mSeekFlag_start && (!mSeekFlag_already);
    }

    public boolean isDuringTimeShift() {
        return mSeekFlag_already;
    }
	
    public long getTimeMillisFromUTCString(String utc_time) {
        Log.d(TAG, "utc_time string: " + utc_time);
        String time_string = utc_time.substring(0,8) + utc_time.substring(9,15);
        Calendar calendar = Calendar.getInstance();
        try {
            calendar.setTime(new SimpleDateFormat("yyyyMMddHHmmss").parse(time_string));
            return calendar.getTimeInMillis();
        } catch (Exception e) {
            Log.e(TAG, "Failed to parse time from utc_time string: " + time_string);
            e.printStackTrace();
    ...
    */
    ...
    public  void start() throws IllegalStateException {
        if(SystemProperties.getBoolean("media.player.report.udp", false)&& !mSendPrepareEvent){
            ...
            buf.append('\n');
            buf.append("<contentid>"+String.valueOf(info.player_id)+"</contentid>");
            buf.append('\n');
        ...
        }else if(mContext != null && !mSendPrepareEvent && (!isDuringTimeShift())){//add by qxm
            if(null == info)
                info= getMediaInfo();
		...
		Log.e(TAG,"----zhuhengxuan----PLAY_PREPARE start()");
                mContext.sendBroadcast(cmd);
                mSendPrepareEvent = true;
                mSendQuitEvent = false;
		//mStartCount++;      //modefied by qxm at 20191114

            }
        }

        if(mContext != null && mSeekNeedResend) {
			//modefied by qxm at 20191111
            cmd.putExtra("TYPE", "SEEK_START");
            //cmd.putExtra("PLAY_TIME", mSeekTime/1000);
			long seek_time = System.currentTimeMillis() - mTimeShiftToMillis ;
			//int seek_time_1 = (int)seek_time/1000;
			Log.d(TAG,"SEEK_TO_TIME:"+ (int)seek_time/1000);
			cmd.putExtra("PLAY_TIME", (int)seek_time/1000);
	...
	if(mSeekStartSend) {
		  Log.d(TAG,"SEEK_END here:2185");
		  long end = System.currentTimeMillis();
		  long seek_time = System.currentTimeMillis() - mTimeShiftToMillis ;
                  cmd.putExtra("PLAY_TIME", (int)seek_time/1000); //modefied by qxm
                  ...
                  if(!mbIsFileDescriptor){
                      mContext.sendBroadcast(cmd);
					  mBufferNoNeedFlag = false;  //add by qxm
                  }
        ...
        if(SystemProperties.getBoolean("media.player.report.udp", false) && mSendPrepareEvent && !mSendQuitEvent){
            ...
            if(mContext != null && mSendPrepareEvent && !mSendQuitEvent &&(!isTimeShift())){ //add by qxm
                int starttime = getCurrentPosition();
                ...
                if(null == info)
                    info= getMediaInfo();
                cmd.putExtra("ID", info.player_id);
                Log.d(TAG , "player id is : "+info.player_id);
                if(!mbIsFileDescriptor){
		    Log.d(TAG , "----zhuhengxuan----PLAY_QUIT stop()");
                    mContext.sendBroadcast(cmd);
                    mSendQuitEvent = true;
		    mStartCount = 0;
                }
    ...
    public void release() {
        if(SystemProperties.getBoolean("media.player.report.udp", false) && mSendPrepareEvent && !mSendQuitEvent){
            ...
            if(mContext != null && mSendPrepareEvent && !mSendQuitEvent){
                ...
                Log.d(TAG , "player id is : "+info.player_id);
                if(!mbIsFileDescriptor){
		    Log.d(TAG , "----zhuhengxuan----PLAY_QUIT release()");
                    mContext.sendBroadcast(cmd);
                    mSendQuitEvent = true;
		    mStartCount = 0;
                }
    ...
    public void reset() {
        ...
        if(SystemProperties.getBoolean("media.player.report.udp", false) && mSendPrepareEvent && !mSendQuitEvent){
            ...
            mSendQuitEvent = true;
            Log.e(TAG, buf.length()+" ,:"+buf.toString());
        }else if(mContext != null && mSendPrepareEvent && (!isTimeShift())){//add by qxm
            int starttime = getCurrentPosition();
            Log.d(TAG , "starttime is : "+starttime/1000);
            cmd.putExtra("TYPE", "PLAY_QUIT");
            cmd.putExtra("PLAY_TIME", starttime/1000);
            ...
            Log.d(TAG , "player id is : "+info.player_id);
            if(!mbIsFileDescriptor){
		Log.d(TAG , "----zhuhengxuan----PLAY_QUIT reset()");	
                //mContext.sendBroadcast(cmd);
                mSendQuitEvent = true;
		mStartCount = 0;
            }
    ...
    */
        ...
        public void handleMessage(Message msg) {
            ...
            switch(msg.what) {
                case MEDIA_EXIT:
                    ...
                    if(SystemProperties.getBoolean("media.player.report.udp", false)&& mSendPrepareEvent && !mSendQuitEvent){
                    ...
                    else if(mContext != null && mSendPrepareEvent && !mSendQuitEvent && (!isTimeShift())){//add by qxm
                        int starttime = getCurrentPosition();
                        Log.d(TAG , "starttime is : "+starttime/1000);
                        cmd.putExtra("TYPE", "PLAY_QUIT");
                        cmd.putExtra("PLAY_TIME", starttime/1000);
                        ...
                        Log.d(TAG , "player id is : "+info.player_id);
                        if(!mbIsFileDescriptor){
			    Log.d(TAG , "----zhuhengxuan----PLAY_QUIT case");
                            mContext.sendBroadcast(cmd);
                            mSendQuitEvent = true;
			    mStartCount = 0;
                        }
			...
			Log.d(TAG, "mSecond is:"+mSecond);
			//if (mBufferStartFlag)   modefied by qxm
			//{
				if ( mSecond > 3 )
				{
					mSecond = 3;
			...
			//}
			        Log.d(TAG, "playabe_report is:"+mSecond);
                    ...
                    if(null == info)
                        info= getMediaInfo();
                    cmd.putExtra("ID", info.player_id);
                    cmd.putExtra("PLAY_TIME", mDisplayTime);  //modefied by qxm
					...
					mDisplayTime = mDisplayTime + 10;   //add by qxm
                }
                return;
            ...
            case MEDIA_SEEK_START:
                ...
                if(mContext != null){
                    ...
                    if(!mbIsFileDescriptor){
                       mContext.sendBroadcast(cmd);
		               mSeekStartSend = true;
					   mBufferNoNeedFlag = true;   //add by qxm
                    }
                } else {
            ...
            case MEDIA_RESUME:
                ...
                return;
            case MEDIA_STARTED:
                if(mContext != null && (!isDuringTimeShift())) {
            ...
            case MEDIA_GET_FIRST_PCR:
                ...
                else if(mContext != null && (!isDuringTimeShift())){//add by qxm
		    Log.e(TAG, "----zhuhengxuan----at case PLAY_START");
                    long start = System.currentTimeMillis();
                    int starttime = getCurrentPosition();
                    cmd.putExtra("TYPE", "PLAY_START");
                    cmd.putExtra("PLAY_TIME", starttime/1000);
                    cmd.putExtra("END_TIME", System.currentTimeMillis());
					if(null == info)    //add by qxm at 20191108
                        info = getMediaInfo();
					...
					Log.d(TAG, "startcount is:"+mStartCount);
                    if(!mbIsFileDescriptor && mStartCount == 1){//modefied by qxm at 20191111
                        mContext.sendBroadcast(cmd);
			mStartCount++;
                    }
                }
            ...
            case MEDIA_BITRATE_CHANGE:
                if(SystemProperties.getBoolean("media.player.report.udp", false)){
                    ...
                    buf.append("<utcsecond>"+String.valueOf(time)+"</utcsecond>");
                    buf.append('\n');
                    buf.append("<tocontentid>"+String.valueOf(info.player_id)+"</tocontentid>");
                    sendDataReport(buf.toString(), buf.length());
                    Log.e(TAG, buf.length()+" ,:"+buf.toString());	
                }else if(mContext != null){
                    ...
                    if(null == info)
                        info= getMediaInfo();
                    cmd.putExtra("OLDID", info.player_id);
                    cmd.putExtra("ID", info.player_id);
                    Parcel parcel = (Parcel) msg.obj;
            ...
            case MEDIA_PREPARED:
				...
				}else if(isDuringTimeShift()){//add by qxm at 20191110   zhibo
				    Log.d(TAG,"SEEK_END here:4571");
				    long end = System.currentTimeMillis();
					long seek_time = System.currentTimeMillis() - mTimeShiftToMillis ;
                    cmd.putExtra("PLAY_TIME", (int)seek_time/1000);
                    cmd.putExtra("TYPE", "SEEK_END");
            ...
            case MEDIA_SET_DATASOURCE:
                if(SystemProperties.getBoolean("media.player.report.udp", false)&& !mSendPrepareEvent){
                    ...
                    if(!mbIsFileDescriptor){
			Log.e(TAG, "----zhuhengxuan----PLAY_PREPARE case");
                        mContext.sendBroadcast(cmd);
                        mSendPrepareEvent = true;
                        mSendQuitEvent = false;
            ...
            case MEDIA_INFO:
                ...
                case MEDIA_INFO_BUFFERING_BROADCAST_START:
                    if(SystemProperties.getBoolean("media.player.report.udp", false)){
                        ...
                        sendDataReport(buf.toString(), buf.length());
                        Log.e(TAG, buf.length()+" ,:"+buf.toString());
                    } else if(mContext != null && !mBufferNoNeedFlag){//add by qxm
                        long start = System.currentTimeMillis();
                        Log.i(TAG, "currentTimeMillis =  " + start);
                        int starttime = getCurrentPosition();
                ...
                case MEDIA_ERROR_PLAYER_REPORT:
                    if(SystemProperties.getBoolean("media.player.cmcc_report.enable", false)) {
                        if(mContext != null) {
                            ...
                            //cmd.putExtra("ID", player_id);       modefied by qxm at 20191115
							if(null == info)
                                info= getMediaInfo();      
							cmd.putExtra("ID", info.player_id);    //qxm add end
					...
					boolean error_handled = false;
					if(!(msg.arg1 == 301)) {
                        msg.arg1 = 54009;
                    }else{
                        msg.arg1 = msg.arg2;
                    }
					Log.w(TAG, "dcl media event: ERROR BROADCAST\n");
					Log.d(TAG,"dcl error :: what = "+msg.arg1+" ; extra = "+msg.arg2);
                    if (mOnErrorListener != null) {
                       error_handled = mOnErrorListener.onError(mMediaPlayer, msg.arg1, msg.arg2);
                    }
                    if (mOnCompletionListener != null && ! error_handled) {
                        mOnCompletionListener.onCompletion(mMediaPlayer);
                    }
                    stayAwake(false);
                    return;
	...
	public class SaveLogThread extends Thread { 
	 ...
	 //ysten.penghui,for upload end event for guangdong
 public void shutdownStop(Context mContext){
     ...
     //end by ysten.penghui ,for upload event for guangdong 
}
