...
public final class BroadcastQueue {
    ...
    public void enqueueOrderedBroadcastLocked(BroadcastRecord r) {
        ...
        if (("true".equals(jsmobile) || "unicom".equals(proj_type) || "mobile".equals(proj_type)) && insertnum==0) {
            if (r.intent != null) {
                String act = r.intent.getAction();
                if (act != null && act.equals("net.sunniwell.action.START_UPGRADE_ACTION")) {
                    Object obj = r.intent.getExtra("UpgradeFileUrl");
                    String upgradeurl = null;
                    if (obj instanceof String) {
                        upgradeurl = (String)obj;
                        Slog.d(TAG, "upgradeurl is : "+upgradeurl);
                        if (null == upgradeurl)
                            return;
                    }
                }
                if (act != null &&( act.equals("cn.10086.action.HDC_LOGIN_STATUS_UPDATE")
                        || act.equals("cn.cmvideo.action.LOGIN_SUCESS") ||
                        act.equals("android.net.conn.CONNECTIVITY_CHANGE"))) {
                    Slog.v(TAG, "add this in front "+ r.intent);
    ...
    private boolean mBootCompleteBrocasted = false;
    final void processNextBroadcast(boolean fromMsg) {
        synchronized(mService) {
            ...
            String jsmobile = SystemProperties.get("ro.jsmobile.launcher" , "");
            String proj_type = SystemProperties.get("sys.proj.type", null);
            if ("true".equals(jsmobile) || "unicom".equals(proj_type) || "mobile".equals(proj_type)) {
                if (mOrderedBroadcasts.size() >= 2) {
                    BroadcastRecord br = mOrderedBroadcasts.get(1);
                    if (br.nextReceiver != 0 && insertnum == 0) {
			Slog.d(TAG , "had send to num is : "+br.nextReceiver);
			insertnum = br.nextReceiver;
			if(br.state == BroadcastRecord.APP_RECEIVE)//broadcast receiving
				br.nextReceiver = br.nextReceiver-1;
			else
				br.nextReceiver = br.nextReceiver+1;
			br.state = BroadcastRecord.IDLE;
                    }
            ...
            if(!skip) {
               if(r.intent != null && r.intent.getAction() != null && r.intent.getAction().equals(Intent.ACTION_BOOT_COMPLETED)) {
                   if("tv.icntv.migu".equals(info.activityInfo.packageName)) {
                       Slog.d("wusw", "skipping `tv.icntv.migu");
                       skip = true;
                   } else if ("com.lutongnet.kalaok2".equals(info.activityInfo.packageName)) {
                       Slog.d("wusw", "skipping `com.lutongnet.kalaok2");
                       skip = true;
                   }
               }
            }
            ...
            Slog.d("Process", "check:"+mBootCompleteBrocasted+","+info.activityInfo.applicationInfo.packageName+"["+mQueueName+"],"+r);
            if (action.equals("android.intent.action.BOOT_COMPLETED")) {
                mBootCompleteBrocasted = true;
            } else if (action.equals("android.net.conn.CONNECTIVITY_CHANGE") ||
                       action.equals("android.net.ethernet.ETHERNET_STATE_CHANGE") ||
                       action.equals("android.net.conn.CONNECTIVITY_CHANGE_IMMEDIATE")) {
                if (!mBootCompleteBrocasted) {
                    String blocks[] = {
                            ...
                            "com.bestv.appmanager",
                            "com.bestv.mishitong.ott",
                            "com.bestv.ott.appstore",
                            "com.bestv.ott.myapps",
                            "com.droidlogic.upgrade",
                            "com.amlogic.mediacenter"
                            };
                    for (String s: blocks) {
                        //Slog.d("Process", "search:"+s+", now:"+info.activityInfo.applicationInfo.packageName);
                        if (info.activityInfo.applicationInfo.packageName.equals(s)) {
                            Slog.d("Process", "Block start:"+info.activityInfo.applicationInfo.packageName);
                            logBroadcastReceiverDiscardLocked(r);
                            finishReceiverLocked(r, r.resultCode, r.resultData,
                                    r.resultExtras, r.resultAbort, false);
                            scheduleBroadcastsLocked();
