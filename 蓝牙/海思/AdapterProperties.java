...
class AdapterProperties {
	...
	Log.d("ystenlk","state = " + validateProfileConnectionState(state)  + " ; prevState = " + validateProfileConnectionState(prevState));
        ...
        synchronized (mObject) {
            ...
            if (updateCountersAndCheckForConnectionStateChange(state, prevState)) {
                ...
                Log.d(TAG, "CONNECTION_STATE_CHANGE: " + device + ": "
                        + prevState + " -> " + state);
                if((prevState == 2) && (state == 0)) {
                    mContext = AdapterApp.getContext();
                    Toast.makeText(mContext, "蓝牙遥控器已断开连接，请进入网络蓝牙选项重新配对连接", Toast.LENGTH_LONG).show();
               }
            }
        }
    }
