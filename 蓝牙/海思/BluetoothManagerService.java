...
class BluetoothManagerService extends IBluetoothManager.Stub {
    ...
    private void loadStoredNameAndAddress() {
		...
		//add by hwz at 03.11 for bluetoothName is empty use Realtek Bluetooth as default name
		if(TextUtils.isEmpty(mName)){
			mName = "Realtek Bluetooth";
		}
    ...
    public boolean disable(boolean persist) {
        //begin:add by yst zhuhengxuan for jiangsu forbiden cmcc bluetooth autopair apk close bluetooth
        if(SystemProperties.get("ro.ysten.province","master").contains("c60_jiangsu")){
            for (StackTraceElement e : Thread.currentThread().getStackTrace()) {
                Log.d(TAG, "zhuhx1 test :" + e.getClassName());
                if (e.getClassName().contains("com.iflytek.bt.auto")) {
                    return true;
                }
            }
        }
        //end:add by yst zhuhengxuan for jiangsu forbiden cmcc bluetooth autopair apk close bluetooth
        mContext.enforceCallingOrSelfPermission(BLUETOOTH_ADMIN_PERM,
