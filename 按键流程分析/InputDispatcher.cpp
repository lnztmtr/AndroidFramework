...
InputDispatcher::KeyEntry* InputDispatcher::synthesizeKeyRepeatLocked(nsecs_t currentTime) {
    ...
    property_get("sys.repeat.time", value, "30");
    mKeyRepeatState.nextRepeatTime = currentTime +milliseconds_to_nanoseconds(atoi(value));//mConfig.keyRepeatDelay;
    return entry;
}
...
bool InputDispatcher::isWindowReadyForMoreInputLocked(nsecs_t currentTime,
    ...
    if (connectionIndex >= 0) {
        ...
        if (eventEntry->type == EventEntry::TYPE_KEY) {
            ...
            // prior input events.
            bool var = connection->outboundQueue.isEmpty()
                    && connection->waitQueue.isEmpty();
            if (var) return var;
            if (currentTime >= connection->waitQueue.head->deliveryTime
                        + STREAM_AHEAD_EVENT_TIMEOUT) {
                String8 name = getApplicationWindowLabelLocked(NULL, windowHandle);
                ALOGD("Application not responding but skip..%s",name.string());
                const char * app = "com.bestv.ott";
                if (name.find(app) > 0) {
