...
#define led_ir         0
#define MC_LEDNAME_IR             "/sys/devices/meson_remote.12/remote_led_ctrl"
static int fd[1]         = {0};
static bool enable_keyEvent_check = false;
...
/*
    get kernel version, if kernel release version is 3.14, need use wakeup epoll
*/
static bool is314Linux() {
    struct utsname info;
    int major, minor;
    ...
    ALOGI("linux release version %d.%d", major, minor);
...
inline
int EventHub::MC_LEDInit(void)
{
    int iRet = 0;
	
    char value[PROPERTY_VALUE_MAX];
    if (property_get("sys.sub.proj.type", value, NULL) > 0) {
        if (!strcmp(value, "migu")) {
            ALOGI("enable_keyEvent_check : true\n");
            enable_keyEvent_check = true;
        }
    }
    
    fd[led_ir] = open(MC_LEDNAME_IR, O_RDWR);
    if (fd[led_ir] < 0)
    {
        ALOGI("Error: Open MC_LEDNAME_IR fail!");
        iRet++;
    }
...
inline
void EventHub::MC_CheckLEDDevForCtrl(void)
{

    if (fd[led_ir] < 0)
    {
        fd[led_ir] = open(MC_LEDNAME_IR, O_RDWR);
	  ALOGI("MC_CheckLEDDevForCtrl fd[led_ir]=%d.\n", fd[led_ir]);
...
inline
void EventHub::EH_LedMgmt_ProcIR(unsigned int action)
{
	if (0 == access(MC_LEDNAME_IR, F_OK))    
	{       
		ALOGI("in MC_LedMgmt_ProcIR,IR is exist,ctrol IR  action=%d\n",action);		

		MC_CheckLEDDevForCtrl();
		switch (action)
		{
			case 1:
				write(fd[led_ir],"1",sizeof("1"));
				break;
			case 0:
				write(fd[led_ir],"0",sizeof("0"));
				break;
			default:			
				break;
		}
    ...
    for (;;) {
        ...
        while (mPendingEventIndex < mPendingEventCount) {
            ...
            if (eventItem.events & EPOLLIN) {
			    ...
			    if(enable_keyEvent_check) {
	                        int32_t keyCode = 0;
	                        uint32_t flags = 0;
	                        if (event->type == EV_KEY && device->keyMap.haveKeyLayout()) {
	                            if (!device->keyMap.keyLayoutMap->mapKey(
	                                        event->code, 0, &keyCode, &flags)) {
	                                        event->code = keyCode;
						//ALOGI("device->identifier.name=%s \n",device->identifier.name.string());

						//ALOGI("event->value=%d.",event->value);
						if((0 == strcmp(device->identifier.name.string(),"gpio_keypad")) || 
							(0 == strcmp(device->identifier.name.string(),"cec_input")) ||
							(0 == strcmp(device->identifier.name.string(),"aml_keypad"))){
							//do nothing
						}
						else{
							EH_LedMgmt_ProcIR(event->value);
						}
...
status_t EventHub::openDeviceLocked(const char *devicePath) {
    ...
    if ((strcmp(value, "true") == 0) && !is314Linux()) {
