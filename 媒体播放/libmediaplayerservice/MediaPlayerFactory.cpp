...
class CMCCPlayerFactory : public MediaPlayerFactory::IFactory {
  public:
    virtual float scoreFactory(const sp<IMediaPlayer>& client,
                               ...
                               float curScore) {
        char value[PROPERTY_VALUE_MAX];
        if (property_get("media.cmccplayer.enable", value, NULL)
                && (!strcmp("1", value) || !strcasecmp("true", value))) {
            if (!strncasecmp("http://", url, 7)
                || !strncasecmp("igmp://", url, 7)
                || !strncasecmp("udp://", url, 6)) {
    ...
    virtual sp<MediaPlayerBase> createPlayer() {
        ALOGE("Create CMCCPlayerFactory");
        return new CMCCMediaPlayerManager();
    }
};

class CTCPlayerFactory : public MediaPlayerFactory::IFactory {
  public:
    virtual float scoreFactory(const sp<IMediaPlayer>& client,
                               const char* url,
                               float curScore) {
        char value[PROPERTY_VALUE_MAX];
        if (property_get("media.ctcplayer.enable", value, NULL)
                && (!strcmp("1", value) || !strcasecmp("true", value))) {
            ALOGI("use CTC_PLAYER");
            return 1.0;
        }
        return 0.0;
    }
    virtual sp<MediaPlayerBase> createPlayer() {
        ALOGE("Create CTCPlayerFactory");
        return new CTCMediaPlayer();
    }
};
void MediaPlayerFactory::registerBuiltinFactories() {
    ...
    registerFactory_l(new CMCCPlayerFactory(), CMCC_PLAYER);
    registerFactory_l(new CTCPlayerFactory(), CTC_PLAYER);
    sInitComplete = true;
}
