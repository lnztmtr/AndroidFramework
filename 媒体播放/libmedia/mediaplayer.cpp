...
status_t MediaPlayer::getDuration_l(int *msec)
{
    ...
    ALOGE("Attempt to call getDuration without a valid state");
    return OK;
}
...
void MediaPlayer::notify(int msg, int ext1, int ext2, const Parcel *obj)
{
    ...
    case MEDIA_ERROR:
        ...
        ALOGE("cpp:error (%d, %d)", ext1, ext2);
        mCurrentState = MEDIA_PLAYER_STATE_ERROR;
        if (mPrepareSync)
        {
            ALOGW("signal application thread");
            mPrepareSync = false;
            mPrepareStatus = ext1;
            mSignal.signal();
            if (ext1 != MEDIA_ERROR_PLAYER_NETWORK)
                send = false;
        }
        break;
    case MEDIA_INFO:
...
status_t MediaPlayer::sendDataReport(
        const char *buf, int32_t len) {
    const sp<IMediaPlayerService>& service = getMediaPlayerService();

    if (service != NULL) {
        return service->sendDataReport(buf, len);
    }
