...
#define GPU_SCALE_MODE  "/sys/class/mpgpu/scale_mode"
#define GPU_MAX_PP  "/sys/class/mpgpu/max_pp"
#define GPU_MAX_FREQ  "/sys/class/mpgpu/max_freq"
#define BUILTIN_MIXER_MODE  "/sys/class/amaudio/builtin_mixer"
#define KARAOK_REVERB_ENABLE "/sys/class/amaudio/reverb_enable"

String16 gpu_max_pp("6");

static int setCpuScaling(void){
    /*Now don't set performace for S905*/
    ...
    systemWriteService->setProperty(String16("media.migukaraok.enable"), String16("1"));

	//get gpu max pp for restore.
	systemWriteService->readSysfs(String16(GPU_MAX_PP), gpu_max_pp);

	//set gpu mode for migu
	String16 gpu_scale_mode("2");
	String16 gpu_max_pp("1");
	String16 gpu_max_freq("1");
	systemWriteService->writeSysfs(String16(GPU_SCALE_MODE), gpu_scale_mode);
	systemWriteService->writeSysfs(String16(GPU_MAX_PP), gpu_max_pp);
	systemWriteService->writeSysfs(String16(GPU_MAX_FREQ), gpu_max_freq);

	return 0;
}

static void restoreCpuScaling(void){
    /*Now don't set performace for S905*/
	sp<IServiceManager> sm = defaultServiceManager();
	if (sm == NULL) {
        ALOGE("Couldn't get default ServiceManager\n");
	...
	systemWriteService->setProperty(String16("media.migukaraok.enable"), String16("0"));

	//restore gpu mode for migu
	String16 gpu_scale_mode("1");
	String16 gpu_max_freq("3");
	systemWriteService->writeSysfs(String16(GPU_SCALE_MODE), gpu_scale_mode);
	systemWriteService->writeSysfs(String16(GPU_MAX_PP), gpu_max_pp);
	systemWriteService->writeSysfs(String16(GPU_MAX_FREQ), gpu_max_freq);
	return;
...
static void setBuiltinMixerModeDefault(void)
{
    sp<IServiceManager> sm = defaultServiceManager();
    if (sm == NULL) {
        ALOGE("Couldn't get default ServiceManager\n");
    ...
    String16 migu_builtin_mixer("0");
    systemWriteService->writeSysfs(String16(BUILTIN_MIXER_MODE), migu_builtin_mixer);

    String16 migu_reverb_enable("0");
    systemWriteService->writeSysfs(String16(KARAOK_REVERB_ENABLE), migu_reverb_enable);
}

static void setBuiltinMixerModeEnable(void)
{
    sp<IServiceManager> sm = defaultServiceManager();
    if (sm == NULL) {
        ALOGE("Couldn't get default ServiceManager\n");
    ...
    String16 migu_builtin_mixer("1");
    systemWriteService->writeSysfs(String16(BUILTIN_MIXER_MODE), migu_builtin_mixer);
}

static void setBuiltinMixerModeDisable(void)
{
    sp<IServiceManager> sm = defaultServiceManager();
    if (sm == NULL) {
        ALOGE("Couldn't get default ServiceManager\n");
...
status_t MediaRecorder::setAudioSource(int as)
{
    ...
    if (as == AUDIO_SOURCE_CMCC_KARAOK_SPEAKER) {
        as = AUDIO_SOURCE_REMOTE_SUBMIX;
        mIsCMCCAudioSource = true;
    }
...
//    if(mIsCMCCAudioSource) {
//        of = OUTPUT_FORMAT_AAC_ADTS;
//    }
...
status_t MediaRecorder::stop()
{
    ...
    if (mIsCMCCAudioSource) {
        AudioSystem::setDeviceConnectionState(AUDIO_DEVICE_IN_REMOTE_SUBMIX, AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE, 0);
        AudioSystem::setDeviceConnectionState(AUDIO_DEVICE_OUT_REMOTE_SUBMIX, AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE, 0);
        mIsCMCCAudioSource = false;
        restoreCpuScaling();
        /* Default builit-in mixer is enable,
        for KI unplug, cannot set enable in hal, set it here*/
        setBuiltinMixerModeDefault();
    }
