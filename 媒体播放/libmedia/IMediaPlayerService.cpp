...
public:
    ...
    virtual status_t stopPlayerIfNeed(void) {
        Parcel data, reply;
        data.writeInterfaceToken(IMediaPlayerService::getInterfaceDescriptor());
        remote()->transact(STOP_PLAYER_IF_NEED, data, &reply);
        return reply.readInt32();
    }
    virtual status_t sendDataReport(const char *buf, int32_t len)  {
    Parcel data, reply;
    ...
    remote()->transact(SEND_DATA_REPORT, data, &reply);
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch (code) {
        ...
        case STOP_PLAYER_IF_NEED:
        {
            CHECK_INTERFACE(IMediaPlayerService, data, reply);          
            reply->writeInt32(stopPlayerIfNeed());
            return OK;
        }
        case SEND_DATA_REPORT:
        {
            CHECK_INTERFACE(IMediaPlayerService, data, reply);
            ...
            reply->writeInt32(sendDataReport(buf, len));
            return OK;
        }
        default:
