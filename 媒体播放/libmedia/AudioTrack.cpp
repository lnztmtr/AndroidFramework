...
AudioTrack::~AudioTrack()
{
    if (mStatus == NO_ERROR) {
        ...
        if (mAudioTrackThread != 0) {
            if (getprop_bool("media.migukaraok.enable")) {
    ...
    if (cbf != NULL) {
        ...
        if (getprop_bool("media.migukaraok.enable")) {
            ...
            if (sched_setaffinity(0, sizeof(cpu_set_t), &mask) < 0) {
                ALOGE("Error: cpu id %d sched_setaffinity\n", cpu_id);
            }
        }
    }
...
uint32_t  AudioTrack::latency()
{
    ...
    uint32_t track_delay = 0;
    int frmfilled;
    AudioSystem::getLatency(mOutput, mStreamType, &hal_delay);
    frmfilled = mProxy->getFramesFilled();
    track_delay = frmfilled*1000 / mSampleRate;
    ALOGV("hal delay %d ms,audio track delay %d ms,total delay %d ms \n",hal_delay,track_delay,hal_delay+track_delay);
    return hal_delay+track_delay;
}
        ...
        size_t epoch)
{
    ...
    if (!audio_is_linear_pcm(format)&&!audio_is_raw_data(format)) {
        ...
        size_t minFrameCount = ((int64_t)afFrameCount*sampleRate*minBufCount)/afSampleRate;
