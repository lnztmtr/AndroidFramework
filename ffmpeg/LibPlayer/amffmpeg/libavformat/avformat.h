...
typedef struct _SoftProbeInfo {
    int pre_fec_ratio;//Average error rate before FEC error correction
    int after_fec_ratio;
    int cached_bytes;
} SoftProbeInfo;
...
typedef struct AVFormatContext {
    ...
    int get_duration; //flag of estimating duration
    int64_t last_offset; //last offset of reading packet
 ...
 * @return 0 if OK, AVERROR_xxx on error
 */
int av_rfps_add_frame(AVFormatContext *ic, AVStream *st, int64_t dts);
