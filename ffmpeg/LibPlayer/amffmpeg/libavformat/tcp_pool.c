...
int tcppool_init(void)
{
	...
	inited++;
	av_log(NULL,AV_LOG_INFO,"tcppool: init\n");
	return 0;
}
...
int tcppool_find_free_tcplink(URLContext **h, const char *uri, int flags)
{
	...
	link->flags=flags;
	link->uri=uri;
	item=itemlist_get_match_item_ex(&tcppool_list_free,tomachitem,machlink_check,0);
	int enable = (int)am_getconfig_bool_def("media.amplayer.reuse_tcp", 0);
	if(item){
		int64_t curtime = av_gettime();
		item_free(tomachitem);
		//itemlist_del_item(&tcppool_list_free,item);
		link=(struct tcppool_link *)&item->extdata[0];
		if(enable && (int)(curtime - link->lastreleasetime_us) < 10*1000*1000){/*10S for default timeout*/
			*h=link->h;
		    itemlist_add_tail(&tcppool_list,item);
