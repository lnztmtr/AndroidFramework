...
static const AVOption options[] = {
    ...
    { "fifo_size",      "set the UDP receiving circular buffer size, expressed as a number of packets with size of 188 bytes", OFFSET(circular_buffer_size), FF_OPT_TYPE_INT, {.i64 = 512*4096}, 0, INT_MAX, D },
...
static int udp_get_info(URLContext *h, uint32_t  cmd, uint32_t flag, int64_t *info)
{
	if(h == NULL){
		return -1;
	}
...
static void *circular_buffer_task( void *_URLContext)
{
    ...
    int recv_cnt = 0;
    int recv_bytes = 0;
    #define MEASURE_RECV_MAXNUM (100)
    ...
    while(1) {
        ...
        FD_ZERO(&rdfs);
        FD_SET(s->udp_fd, &rdfs);
        int ret = select(s->udp_fd+1, &rdfs, NULL, NULL, &tv);
        //av_log(NULL, AV_LOG_ERROR, "[%s:%d]select ret=%d errno=%d tv_usec:%d\n",__FUNCTION__,__LINE__,ret,errno, tv.tv_usec);
        if (ret == 0)
        {
            pthread_mutex_lock(&s->mutex);
            continue;
        ...
        len = recv(s->udp_fd, s->tmp, sizeof(s->tmp), MSG_DONTWAIT);
        //pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, &old_cancelstate);
        recv_cnt++;
        if (len > 0)
            recv_bytes += len;

        if (recv_cnt >= MEASURE_RECV_MAXNUM) {
            bandwidth_measure_finish_read(s->bandwidth_measure, recv_bytes);
            recv_cnt = 0;
            recv_bytes = 0;
        }
        ...
        if (len < 0) {
            if (ff_neterrno() != AVERROR(EAGAIN) && ff_neterrno() != AVERROR(EINTR)) {
                ...
                av_log(h, AV_LOG_ERROR, "Circular buffer recv faild, exit\n");
                goto end;
            }
            continue;
        }

        if(av_fifo_space(s->fifo) < len) {
            /* No Space left */
        ...
        av_fifo_generic_write(s->fifo, s->tmp, len, NULL);
...
static int init_def_settings()
{
	...
	gd_report_error_enable = (int)am_getconfig_bool_def("media.player.gd_report.enable",0) ||
                             ...
                             ||(int)am_getconfig_bool_def("media.player.ln_report.enable",0);
	if(am_getconfig_bool_def("media.player.gd_report.enable",0))
        	get_data_timeout_error = 54000;
	   else if (am_getconfig_bool_def("media.player.ln_report.enable",0))
        	get_data_timeout_error = 3514;
	...
	av_log(NULL, AV_LOG_INFO, "get_data timeout error=%d\n",get_data_timeout_error,get_data_timeout);
	return 0;
}
static void  udp_check_report_info(URLContext *h)
{
    if(h == NULL||!gd_report_error_enable)
        return;
    ...
    if((time > (s->latest_get_time_ms +get_data_timeout*1000000))&&(s->report_flag==0)){
        s->report_flag = 1;
        if (am_getconfig_bool_def("media.player.ln_report.enable",0))
        {
            if (1 == s->is_multicast)
            {
                av_log(NULL, AV_LOG_ERROR, "[%s %d] is_multicast:%d", __FUNCTION__, __LINE__, s->is_multicast);
...
static int udp_open(URLContext *h, const char *uri, int flags)
{
	...
	s->circular_buffer_size = am_getconfig_int_def("media.player.udpfifo", UDP_FIFO_SIZE);
    av_log(NULL,NULL, "udp_fifo_size = %d\n",s->circular_buffer_size);
    ...
    if (flags & AVIO_FLAG_WRITE) {
        ...
        h->max_packet_size = am_getconfig_int_def("media.player.udppktsize", UDP_DEFAULT_PKT_SIZE);//64k,avoid avio fillbuff consume too much time PD#139583
    }
    ...
    if (!is_output && s->circular_buffer_size) {
        ...
        pthread_setname_np(s->circular_buffer_thread, "udp_task");
        s->thread_started = 1;
    }
...
static int udp_read(URLContext *h, uint8_t *buf, int size)
{
        ...
        do {
            ...
            if (avail) { // >=size) {
                ...
                if (pthread_cond_timedwait(&s->cond, &s->mutex, &tv) < 0) {
                    pthread_mutex_unlock(&s->mutex);
                    ret = AVERROR(errno == ETIMEDOUT ? EAGAIN : errno);;
                    break;
                }
                nonblock = 1;
            }
        } while( 1);
        udp_check_report_info(h);
        return ret;
    }
    ...
    if (ret > 0) {
        s->latest_get_time_ms = 0;
        s->report_flag = 0;
        bandwidth_measure_finish_read(s->bandwidth_measure, ret);
    } else {
        udp_check_report_info(h);
        bandwidth_measure_finish_read(s->bandwidth_measure, 0);
    }
