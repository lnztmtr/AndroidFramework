...
CFContext * curl_fetch_init(const char * uri, const char * headers, int flags)
{
    ...
    handle->chunk = NULL;
    // add user-agent & keep alive list
    // add connection close
    int keep_alive = am_getconfig_int_def("media.curl.keepalive", 0);
    if (keep_alive == 0)
        handle->chunk = curl_slist_append(handle->chunk, "Connection: close");
    handle->chunk = curl_slist_append(handle->chunk, "User-Agent: AppleCoreMedia/1.0.0.7B367 (iPad; U; CPU OS 4_3_3 like Mac OS X)\r\n");
