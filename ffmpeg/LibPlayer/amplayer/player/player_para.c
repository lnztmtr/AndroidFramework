...
static void get_av_codec_type(play_para_t *p_para)
{
    ...
    if (video_index != -1) {
        ...
        if (p_para->stream_type == STREAM_ES && pCodecCtx->codec_tag != 0) {
            if (pCodecCtx->codec_tag == 0xd2) //the stream type of avs2 is 0xd2
                pCodecCtx->codec_tag = pCodecCtx->codec_id;//avs2 tmp??
        ...
        if (p_para->vstream_info.video_format == VFORMAT_VC1) {
           ...
           if ((VIDEO_DEC_FORMAT_MPEG4_3 == p_para->vstream_info.video_codec_type || VIDEO_DEC_FORMAT_MPEG4_4 == p_para->vstream_info.video_codec_type) && am_getconfig_bool("media.amplayer.nosupportdivx"))
            ...
            if ((p_para->pFormatCtx) && (p_para->pFormatCtx->pb) ) {
                 if (pCodecCtx && (pCodecCtx->poc_type == 1)) {
                    log_print("h264 poc_type %d \n", pCodecCtx->poc_type);
                    p_para->vstream_info.poc_type = pCodecCtx->poc_type;
                 }
            }
        ...
        if (pCodecCtx && p_para->vstream_info.has_video) {
            ...
            if (pStream->start_time < 0) {  //fft:Only set this if you are absolutely 100% sure that the value you set it to really is the pts of the first frame.
                ...
                p_para->vstream_info.has_start_time = 0;
            } else {
                p_para->vstream_info.has_start_time = 1;
            }
    ...
    if (audio_index != -1) {
        ...
        if (p_para->astream_info.has_audio) {
            if (pStream->start_time < 0) {
                pStream->start_time = 0;
                p_para->astream_info.has_start_time = 0;
            } else {
                p_para->astream_info.has_start_time = 1;
            }
            if (0 != pStream->time_base.den) {
...
static void check_cavs_codec(play_para_t *p_para)
{
    char buffer[256] = {0};
    int avs_flag = 0;
    AVCodecContext *pCodecCtx;
    ...
    if (!p_para || !p_para->pFormatCtx || !p_para->pFormatCtx->iformat)
        return NULL;

    log_print("[%s:%d] iformat->name[%s]\n", __FUNCTION__, __LINE__,
        p_para->pFormatCtx->iformat->name);
    if (strcmp(p_para->pFormatCtx->iformat->name, "cavs2video") != 0
        && strcmp(p_para->pFormatCtx->iformat->name, "cavsvideo") != 0)
        return NULL;

    snprintf(buffer, 256, "/proc/%d/fd/%d", gettid(), p_para->start_param->local_fd);
    ...
    if (lstat(buffer, &s) == 0) {
        if ((s.st_mode & S_IFMT) == S_IFLNK) {
            ...
            if (len > 0) {
                ...
                if (strstr(linkto, ".avs2"))
                    avs_flag = 2;
                else if (strstr(linkto, ".avs"))
                    avs_flag = 1;
                else
                    avs_flag = 0;
            }
    ...
    log_print("[%s:%d] avs_flag:%d\n", __FUNCTION__, __LINE__, avs_flag);
    video_index = p_para->vstream_info.video_index;
    log_print("[%s:%d] video_index:%d (%d)\n",
        __FUNCTION__, __LINE__, video_index, p_para->pFormatCtx->nb_streams);
    if (video_index < 0 || video_index >= p_para->pFormatCtx->nb_streams)
        return NULL;

    pCodecCtx = p_para->pFormatCtx->streams[video_index]->codec;
    if (avs_flag == 1)
        pCodecCtx->codec_id = CODEC_ID_CAVS;

    if (avs_flag == 2)
        pCodecCtx->codec_id = CODEC_ID_CAVS2;
    log_print("[%s:%d] codec_id:0x%x, tag=0x%x, %s\n",
        __FUNCTION__, __LINE__, pCodecCtx->codec_id, pCodecCtx->codec_tag, pCodecCtx->codec_name);
}
...
static void get_stream_info(play_para_t *p_para)
{
    ...
    if (video_index != -1) {
        ...
        } else if (p_para->vstream_info.video_format == VFORMAT_AVS2) {
            if (!p_para->vdec_profile.avs2_para.exist) {
                unsupported_video = 1;
                log_print("[%s:%d]avs2, not support now!\n", __FUNCTION__, __LINE__);
            }
        } else {
...
void *player_decoder_reset_thread_t(void *arg)
{
    void *ret;
    play_para_t *player_para;
    player_para = (play_para_t*)arg;

    const stream_decoder_t *decoder;

    decoder = player_para->decoder;

    log_print("---player_decoder_reset_thread_t--\n");

    if (decoder->release(player_para) != PLAYER_SUCCESS) {
        log_error("[player_decoder_reset_thread_t] deocder release failed!\n");
        player_para->player_dec_reset_status = -1;
    }
    /*make sure have enabled.*/
    if (player_para->astream_info.has_audio && player_para->vstream_info.has_video) {
        set_tsync_enable(1);

        player_para->playctrl_info.avsync_enable = 1;
    } else {
        set_tsync_enable(0);
        player_para->playctrl_info.avsync_enable = 0;
    }
    if (decoder->init(player_para) != PLAYER_SUCCESS) {
        log_print("[player_decoder_reset_thread_t] deocder init failed!\n");
        player_para->player_dec_reset_status = -1;
    }

    if (player_para->astream_info.has_audio && player_para->acodec) {
        player_para->codec = player_para->acodec;
        if (player_para->vcodec) {
            player_para->codec->has_video = 1;
        }
        log_print("[%s:%d]para->codec pointer to acodec!\n", __FUNCTION__, __LINE__);
    } else if (player_para->vcodec) {
        player_para->codec = player_para->vcodec;
        log_print("[%s:%d]para->codec pointer to vcodec!\n", __FUNCTION__, __LINE__);
    }

    player_para->player_dec_reset_status = 2;

    return NULL;
}


int player_decoder_reset(play_para_t *p_para)
{
    int ret = 0;
    pthread_t       tid;
    ...
    p_para->player_dec_reset_status = 1;

    pthread_attr_init(&pthread_attr);
    pthread_attr_setstacksize(&pthread_attr, 0);   //default stack size maybe better
    ret = amthreadpool_pthread_create(&tid, &pthread_attr, (void*)&player_decoder_reset_thread_t, (void*)p_para);
    if (ret != 0) {
        log_print("creat player thread failed !\n");
        return ret;
    }
    log_print("[player_decoder_reset:%d]creat thread success,tid=%lu\n", __LINE__, tid);
    //pthread_setname_np(tid, "AmplayerMain");
    pthread_attr_destroy(&pthread_attr);
    p_para->player_dec_reset_tid = tid;
...
int player_dec_reset(play_para_t *p_para)
{
    ...
    log_print("---player_dec_reset, seek_async=%d--\n", p_para->seek_async);


    p_para->player_dec_reset_status = 0;
    ...
    if (p_para->seek_async) {
        ret = player_decoder_reset(p_para);
    } else {
...
RETRY:
        ...
        if (p_para->seek_async) {
            int count = 0;
            while (count++ < 100) {
                if (p_para->player_dec_reset_status == -1) {
                    amthreadpool_pthread_join(p_para->player_dec_reset_tid, NULL);
                    return -1;
                } else if (p_para->player_dec_reset_status == 1) {
                    usleep(2000);
                } else {
                    break;
                }
            }
            if (p_para->player_dec_reset_tid) {
                amthreadpool_pthread_join(p_para->player_dec_reset_tid, NULL);
            }

            log_print("player_dec_reset: p_para->player_dec_reset_status=%d, count=%d\n", p_para->player_dec_reset_status, count);
            p_para->player_dec_reset_status = 0;
        }
...
int player_decoder_init(play_para_t *p_para)
{
    ...
    log_print("player_decoder_init, enter\n");
    ...
    log_print("player_decoder_init, enter--2, decoder->name=%s--\n", decoder->name);
