...
int player_notify_callback(int pid, int msg, unsigned long ext1, unsigned long ext2)
{
    ...
    if (player_para) {
		log_print("[player_notify_callback]send_event,ext1:%d, ext2:%d\n",ext1,ext2);
...
int ffmpeg_open_file(play_para_t *am_p)
{
    ...
    int retry_count= am_getconfig_int_def("media.amplayer.file_retry", 3);
...
Retry_open:
        ...
        audio_register_notify(player_notify_callback);
        ffmpeg_register_id_notify(player_notify_callback, am_p->player_id);
        ...
        if (ret != 0) {
            ...
            if ((retry_count-- > 0) && (ret !=AVERROR_EXIT)) {
                goto Retry_open;
            }
...
int ffmpeg_parse_file_type(play_para_t *am_p, player_file_type_t *type)
{
    ...
    if (pFCtx->iformat != NULL) {
        ...
        for (i = 0; i < pFCtx->nb_streams; i++) {
            ...
            if (st->codec->codec_type == CODEC_TYPE_VIDEO) {
                ...
                if (st->codec->codec_id == CODEC_ID_RV40 && (st->codec->width * st->codec->height > 1920 * 1088)) {
                    if (rm_flag == 0) {
                        rm_flag = 1;
			if(st->codec->width * st->codec->height <= 1936 * 1088)
			    sprintf(vpx_string, "%s", "rmhw");
			else
            ...
            if (st->codec->codec_id == CODEC_ID_MPEG4) {
                int video_index = am_p->vstream_info.video_index;
                AVStream *pStream;
                AVCodecContext  *pCodecCtx;
                int ret;
		notify_callback *ncb = am_p->update_state.notify_fn;
		am_p->update_state.notify_fn = NULL;
		ret = player_dec_init(am_p);
		am_p->update_state.notify_fn = ncb;
                video_index = am_p->vstream_info.video_index;
                if (video_index != -1) {
                    pStream = pFCtx->streams[video_index];
                    pCodecCtx = pStream->codec;
                    if (am_p->vstream_info.has_video == 0) { // not support totally
                            log_print("hwdecoder do not support, retry sw decoder\n");
                            sprintf(format_string, "%s","amsoftdec");
                            type->fmt_string = format_string;
                    }
                }
            }
		...
		memset(format_string, 0, sizeof(format_string));
            ...
            if ((strstr(type->fmt_string, "wav") != NULL) && (sttmp->codec->codec_id == CODEC_ID_DTS)) {
                memset(format_string, 0, sizeof(format_string));
                sprintf(format_string, "%s", "dts");
                log_print("NOTE: change type->fmt_string=%s to dts\n", type->fmt_string);
                type->fmt_string = format_string;
...
int ffmpeg_get_hls_info(play_para_t* para, int type, void* value)
{
    int ret = -1;

    if (para && para->pFormatCtx && para->pFormatCtx->pb) {
        ret = avio_getinfo(para->pFormatCtx->pb, AVCMD_GET_HLSINFO, type, value);
    } else {
        int iptv_v5_support = (int)am_getconfig_bool_def("media.amplayer.hlsv5_support", 0);
        if (iptv_v5_support) {
            if (para && para->pFormatCtx && para->pFormatCtx->iformat && para->pFormatCtx->iformat->get_parameter) {
                para->pFormatCtx->iformat->get_parameter(para->pFormatCtx, AVCMD_GET_HLSINFO, type, value, NULL);
            }
        }
    }
...
int ffmpeg_hls_has_Iframe_list(play_para_t* para)
{
    int value = 0;
    if (para && para->pFormatCtx && para->pFormatCtx->pb) {
        avio_getinfo(para->pFormatCtx->pb, AVCMD_GET_NETSTREAMINFO, 10, &value);
    } else {
        int iptv_v5_support = (int)am_getconfig_bool_def("media.amplayer.hlsv5_support", 0);
        if (iptv_v5_support) {
            if (para && para->pFormatCtx && para->pFormatCtx->iformat && para->pFormatCtx->iformat->get_parameter) {
                para->pFormatCtx->iformat->get_parameter(para->pFormatCtx, 5, -1, &value, NULL);
            }
        }
    }
    log_print("[%s]:%d\n", __FUNCTION__,value);
