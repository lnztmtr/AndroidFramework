...
void adts_add_header(play_para_t *para)
{
    ...
    if (pkt->avpkt && (pkt->avpkt->flags & AV_PKT_FLAG_WITH_HEADER)) {
...
{
    AVStream *pStream = NULL;
    char sei_nal_mastering_start_code[] = {0x0, 0x0, 0x0, 0x1, 0x4e, 0x01, 0x89, 0x18};
	char sei_nal_cll_start_code[] = {0x0, 0x0, 0x0, 0x1, 0x4e, 0x01, 0x90, 0x4};	
    AVCodecContext *avcodec;
    am_packet_t *pkt = para->p_pkt;
    int ret = -1;
	int j;
	...
	if (avcodec->sei_mastering_display_info_present){
    	...
    	int header_len = 0;
		void *bufp = sei_nal_mastering_start_code;
    	char* buffer = pkt->hdr->data;
		MEMCPY(&(buffer[header_len]), bufp, 8);
		header_len += 8;
		 for (j = 0; j < 3; j ++) {
		 ...
		 for (j = 0; j < 2; j ++) {
			  ...
			  if (buffer[header_len] == 0 && buffer[header_len + 1] == 0) {
			  	buffer[header_len + 2] = 3;
			 	buffer[header_len + 3] = *((char *)&p->luminance[j] + 1);
			 	buffer[header_len + 4] = *(char *)&p->luminance[j];	
			  	header_len += 5;
			  } else {
	...
	if (avcodec->sei_cll_info_present){
	    AVhevc_ContentLightLevl_info *p = (struct AVhevc_ContentLightLevl_info *)&avcodec->cll_info;
    	int header_len = 0;
		void *bufp = sei_nal_cll_start_code;
    	char* buffer = pkt->hdr->data;
		MEMCPY(&(buffer[header_len]), bufp, 8);
		header_len += 8;
		 for (j = 0; j < 2; j ++) {
			  buffer[header_len] = *((char *)&p->content_light_level[j] + 1);
			  buffer[header_len + 1] = *(char *)&p->content_light_level[j];
			  header_len += 2;
		 }
		 pkt->hdr->size = header_len;
		 pkt->type = CODEC_VIDEO;
...
#define EXT_START_CODE      (0x000001b5)
    ...
    unsigned char buf[MPEG_PROBE_SIZE] = {0};
    ...
    avio_seek(s->pb, cur_offset, SEEK_SET);
    /* +[SE][BUG][BUG-172575][chengshun.wang]: only need free malloc buffer*/
    if (seq_size > MPEG_PROBE_SIZE && pbuf) {
        FREE(pbuf);
        pbuf = NULL;
    }
...
int pre_header_feeding(play_para_t *para)
{
    ...
    if (para->stream_type == STREAM_ES && para->vstream_info.has_video) {
        ...
        } else if ((CODEC_TAG_WVC1 == avcodec->codec_tag) ||
                   (CODEC_TAG_vc_1 == avcodec->codec_tag) ||
                   (CODEC_TAG_WMVA == avcodec->codec_tag)) {
