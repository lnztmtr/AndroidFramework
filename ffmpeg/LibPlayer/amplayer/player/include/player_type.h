...
typedef struct {
    ...
    aformat_t       audio_format[MAX_AUDIO_STREAMS]; //max audio streams is 16 per programe;
    char programe_name[64];
    vformat_t       video_format;
} ts_programe_detail_t;


typedef struct hls_para {
    int bitrate;//m3u8 bandwidth

    int m3u8_get_delay_avg_time;
    int m3u8_get_delay_max_time;

    int ts_get_delay_max_time;
    int ts_get_delay_avg_time;
    int ts_get_suc_times;
    int ts_get_times;

    char m3u8_server[32];//ip:port
    char ts_server[32]; //ip:port
}hls_para_t;

typedef struct player_para_report {
    //setting
    int enable; //support player para report
    int report_period_ms;//millisecond
    long last_report_time;

    //param
    int first_pic_time;//msec

    //caton info
    int carton_times;//caton times
    int carton_time;//caton time
    ...
    int ts_cc_discont;
    int ts_sync_lost_num;
    int avpts_diff;
    char transport_protocol[8];
    struct codec_quality_info quality_info;
}player_para_report_t;
...
typedef struct {
    ...
    mstream_info_t new_stream_info;
    int vfmt_change;
    int afmt_change;
} ts_programe_info_t;

typedef struct {
    player_para_report_t report_para;
...
typedef struct player_info {
    ...
    unsigned int audio_decode_error_cnt;
    float audio_bufferlevel; // relative value
    float video_bufferlevel; // relative value
    float sub_bufferlevel; // relative value
    float audio_bufferlevel_last;
    float video_bufferlevel_last;
    ...
    int seek_point;
    int seek_delay;
    int64_t enter_buffering_time;
    int64_t buffer_ok_time;
    unsigned long cache_time;
    int64_t unload_start_time;
    int64_t unload_end_time;
    aformat_t aformat;
    int total_audio_num;
    int cur_audio_index;
    int channel;
    int sample_rate;
    //0,init;1,during overflow;2,finish overflow
    int status_overflow;
    int  start_overflow_time;
    int64_t delta_current_time;
} player_info_t;
...
typedef enum {
    ...
    PLAYER_EVENTS_BLURREDSCREEN_START, //blurred screen start
    PLAYER_EVENTS_BLURREDSCREEN_END,   //blurred screen end
    PLAYER_EVENTS_UNLOAD_START,
    PLAYER_EVENTS_UNLOAD_END,
    PLAYER_EVENTS_PLAYER_PARAM_REPORT,
    PLAYER_EVENTS_NEED_SEEK,
} player_events;
...
typedef struct {
    ...
    union {
        struct {
            ...
            unsigned int is_bootplay : 1;//0:is not bootplay 1:is bootplay
        };
        int mode;                           //no use
    };
