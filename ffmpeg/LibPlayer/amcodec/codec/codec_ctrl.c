...
int codec_init(codec_para_t *pcodec)
{
    ...
    //if(pcodec->has_audio)
    //  audio_stop();
    CODEC_PRINT("CTC_KPI::Stage 2_1 codec init time,codec_init_begin_time\n");
    pcodec->handle = -1;
	...
	if (pcodec->video_type == VFORMAT_H264 && pcodec->poc_type == 1) {
		pcodec->dec_mode = STREAM_TYPE_STREAM;
	}
    /* [SE][BUG][BUG-168382][chengshun] avs2 run on stream mode in default */
    ...
    CODEC_PRINT("codec_init: pcodec->video_type=%d, dec_mode=%d\n", pcodec->video_type, pcodec->dec_mode);
    ...
    if (pcodec->has_audio) {
        CODEC_PRINT("CTC_KPI::Stage 2_1_2 audio codec init time,codec_init_begin_time\n");
        ...
        a_ainfo.localplay_flag  = pcodec->localplay_flag;
        a_ainfo.start_no_out = pcodec->start_no_out;
        a_ainfo.player_id = pcodec->player_id;
        ...
        CODEC_PRINT("CTC_KPI::Stage 2_1_2 audio codec init time,codec_init_end_time\n");
    }

    if(pcodec->start_no_out){
        codec_pause(pcodec);
        CODEC_PRINT("%s,  start player with pause %d\n", __FUNCTION__,   pcodec->start_no_out);
    }
    ...
    CODEC_PRINT("CTC_KPI::Stage 2_1 codec init time,codec_init_end_time\n");
    return ret;
}
...
{
    int res = 0;
    CODEC_PRINT("CTC_KPI::Stage 1_3_1 audio codec close cost,close_time\n");
    if (pcodec->has_audio) {
        audio_stop(&pcodec->adec_priv);
        CODEC_PRINT("[%s]audio stop OK!\n", __FUNCTION__);
    }
    CODEC_PRINT("CTC_KPI::Stage 1_3_1 audio codec close cost,close_endtime\n");
#ifdef SUBTITLE_EVENT
...
/**
* @brief  codec_get_sbuf_state  Get the state of  Subtittle buffer by codec device
*
* @param[in]   p    Pointer of codec parameter structure
* @param[out]  buf  Pointer of buffer status structure to get Subtittle buffer state
*
* @return      Success or fail type
*/
/* --------------------------------------------------------------------------*/
int codec_get_sbuf_state(codec_para_t *p, struct buf_status *buf)
{
    int r;
    if (codec_h_is_support_new_cmd()) {
        struct buf_status status;
        r = codec_h_ioctl(p->handle, AMSTREAM_IOC_GET_EX, AMSTREAM_GET_EX_SB_STATUS, (unsigned long)&status);
        memcpy(buf, &status, sizeof(*buf));
    }
    return system_error_to_codec_error(r);
}
...
int codec_reset(codec_para_t *p)
{
    ...
    CODEC_PRINT("[%s:%d] closed then init\n", __FUNCTION__, __LINE__);
    ret = codec_init(p);
...
/* --------------------------------------------------------------------------*/
/**
* @brief  codec_set_video_speed   speed video playing by codec device
*
* @param[in]  speed  Pointer of codec parameter structure
*
* @return     0 for success, or fail type if < 0
*/
/* --------------------------------------------------------------------------*/
int codec_set_video_speed(codec_para_t *p, float speed)
{
    int pcr_rate;
    ...
    amsysfs_set_sysfs_int("/sys/class/video/pcr_slow_factor", pcr_rate);
    CODEC_PRINT("video_set_speed:%f, pcr_rate:%d\n",speed,pcr_rate);
    return 0;
}

/* --------------------------------------------------------------------------*/
/**
* @brief  codec_set_audio_speed  set audio by codec device
*
* @param[in]  p  Pointer of codec parameter structure
*
* @param[in]  speed  speed of playing

* @return     Success or fail error type
*/
/* --------------------------------------------------------------------------*/
int codec_set_audio_speed(codec_para_t *p, float speed)
{
    int ret = CODEC_ERROR_NONE;
    if (speed < 0.5 || speed > 2) {
        CODEC_PRINT("[%s:%d] speed is error :%f!", __FUNCTION__, __LINE__,speed);
        return 0;
    }
    if (p) {
        CODEC_PRINT("[codec_set_speed]p->has_audio=%d,speed:%f p->has_video:%d\n", p->has_audio, speed,p->has_video);
        if (p->has_audio) {
            audio_set_speed(p, speed);
            CODEC_PRINT("[codec_set_speed]speed:%f\n",  speed);
