...
int amvideo_utils_set_virtual_position(int32_t x, int32_t y, int32_t w, int32_t h, int rotation)
{
    ...
    // Check whether need to skip
    {
        char tmp[PROPERTY_VALUE_MAX];
        memset(tmp, 0, sizeof(tmp));
        property_get("persist.sys.sc_nx.flag", tmp, "0");
        if(strstr(tmp, "1")){ //ningxia
	    LOGI("TCN_ADD: persist.sys.sc_nx.flag is 1!!\n");
            int ret = 0;
            char val[PROPERTY_VALUE_MAX];
            memset(val, 0, sizeof(val));
            property_get("media.amplayer.nxmobile_live", val, "false");
            int need_check = (strncmp(val, "true", 4) == 0);
            LOGI("amvideo_utils_set_virtual_position: need_check=%d, val:%s, w=%d, disp_w=%d\n", need_check, val, w, dev_w);
            if(need_check) {
                if(h >= 715 && h <= 725)
                    dst_h = h = 720;
                if(h >= 1075 && h <= 1085)
            ...
            if (need_check && (w < dev_w/2 || w > dev_w)) {
                LOGI("amvideo_utils_set_virtual_position: skip video axis set.\n");
                return ret;
            }
...
OUT:
    ...
    amvideo_iptv_setscreenmode(dst_w,dst_h);  
