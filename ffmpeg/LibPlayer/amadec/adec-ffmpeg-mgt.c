...
unsigned long  armdec_get_pts(dsp_operations_t *dsp_ops)
{
    ...
    unsigned long pts = 0;
    int data_width = 0, channels = 0, samplerate = 0;
    unsigned long long frame_nums = 0;
    ...
    if (!audec->first_apts_lookup_over) {
        ...
        if (audec->g_bst->format == ACODEC_FMT_RAAC || audec->g_bst->format == ACODEC_FMT_WMAPRO) {
            len = audec->g_bst->buf_level + audec->pcm_cache_size;
            frame_nums = (len * 8 / (data_width * channels));
            delay_pts = (frame_nums * 90000 / samplerate);
			adec_print("[%s %d]first pts delay:0x%x\n", __FUNCTION__, __LINE__, delay_pts);
            if (pts > delay_pts) {
                pts -= delay_pts;
            } else {
                pts = 0;
        ...
        adec_print("[%s %d]first pts:0x%x format:%d\n", __FUNCTION__, __LINE__, pts, audec->g_bst->format);
        return pts;
    }
    if (pts == 0) {
...
long  armdec_get_pcrscr(dsp_operations_t *dsp_ops)
{
    long val = 0;
    if (dsp_ops->dsp_file_fd < 0) {
...
long  armdec_set_pts(dsp_operations_t *dsp_ops, unsigned long apts)
{
    if (dsp_ops->dsp_file_fd < 0) {
...
int get_decoder_status(void *p, struct adec_status *adec)
{
    ...
    if (audec && audec->g_bst) {
        ...
        adec->decode_error_count = audec->audio_decode_error_count;
...
static int OutBufferInit_raw(aml_audio_dec_t *audec)
{
    ...
    if ((audec->format == ACODEC_FMT_DTS ||audec->format == ACODEC_FMT_EAC3) && amsysfs_get_sysfs_int("/sys/class/audiodsp/digital_raw") == 2) {
...
static int audio_codec_init(aml_audio_dec_t *audec)
{
    ...
    audec->audio_decode_error_count = 0;
    audec->g_bst = NULL;
    audec->g_bst_raw = NULL;
    audec->fd_uio = -1;
...
static int start_adec(aml_audio_dec_t *audec)
{
    ...
    if (audec->state == INITTED) {
        ...
        if(audec->start_no_out){
            pause_adec(audec);
            audec->start_no_out = 0;
            adec_print("amadec start no out\n");
        }
    } else {
...
static void stop_adec(aml_audio_dec_t *audec)
{
    ...
    if (audec->state > INITING) {
       ...
       // amsysfs_set_sysfs_int("/sys/class/audiodsp/digital_raw",audec->last_digital_raw_state);
    }
}
...
exit_decode_loop:
        ...
        if (nCurrentReadCount > 0) {
            while (declen < rlen && !audec->exit_decode_thread) {
                ...
                if(audec->report_flag) {
                    audio_get_error_nb_frames(audec);
                    if((audec->last_error_nb_frames != audec->error_nb_frames)&&
                        (audec->error_flag == 0)){
                        adec_print("last:%d,error:%d,report\n",
                                    audec->last_error_nb_frames,audec->error_nb_frames); 
                        last_error_time = gettime();
                        audio_notify(audec, PLAYER_EVENTS_ERROR, AUDIO_DATA_ERROR, 0);
                        audec->last_error_nb_frames = audec->error_nb_frames;
                        audec->error_flag = 1;
                    }else if(audec->error_flag == 1) {
                        current_time = gettime();
                        if(audec->last_error_nb_frames == audec->error_nb_frames) {
                            if((current_time - last_error_time) >= 2000000) 
                                audec->error_flag = 0;
                        }else {
                            last_error_time = gettime();
                            audec->last_error_nb_frames = audec->error_nb_frames;
                ...
                if (dlen <= 0) {
                    ...
                    audec->audio_decode_error_count++;
                    needdata = 0;
                    break;
                }
