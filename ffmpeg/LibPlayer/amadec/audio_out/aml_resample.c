...
static int resample_process(struct resample_para *resample, unsigned int in_frame, short* input, short* output)
{
    ...
    while (inputIndex == 0) {
        ...
        frac += FractionStep;
        inputIndex += (frac >> 28);
        //adec_print("input  FractionStep:%x,in_frame:%d  inputIndex:%d,skip:%d,outputIndex:%d\n",resample->FractionStep,in_frame,inputIndex, frac >> 28,outputIndex+1);
    ...
    while (inputIndex < in_frame) {
        ...
        frac += FractionStep;
        inputIndex += (frac >> 28);
        //adec_print("input inputIndex:%d,skip:%d,outputIndex:%d\n",inputIndex, frac >> 28,outputIndex+1);
        frac = (frac & kPhaseMask);
        outputIndex++;
 ...
 void af_resample_api_new(char* buffer, unsigned int * size, int Chnum, aml_audio_dec_t* audec, int enable, double rate)
{
    ...
    if (rate >= 0.5 && rate <= 2.0) {
        ...
        adec_print("warning: af_resample_api_new rate error :%f\n", rate);
        return;
    }

    if (last_resample_enable != enable  || last_delta != delta) {
        adec_print("new pcr audio resample changed: %s,rate:%f,delta %d,Chnum %d\n", enable ? "Enabled" : "Disabled", rate,delta, Chnum);
        last_resample_enable = enable;
        last_delta = delta;
        resampler_init(&resampler, Chnum, audec->samplerate, delta);
        amsysfs_set_sysfs_int("/sys/class/video/pcr_slow_factor", pcr_rate);
    }
    if (enable == 0 || delta == 0 || *size < 2 * frame_size) {
        *size = pcrmaster_dsp_pcm_read(audec, buffer, *size);
        read_from_buf = *size;
        if (*size % frame_size) {
            adec_print("warning: read pcm not frame align,read size %d,frame size %d\n", *size, frame_size);
        }
        out_frame = *size / frame_size;
        if (out_frame > 0) {
            //store the last frame here for preparing the resample
            pbuf = (short*)buffer;
            for (ch = 0; ch < Chnum; ch++) {
                resampler.lastsample[ch] = pbuf[Chnum * (out_frame - 1) + ch];
            }
        }
        goto exit;
    }
    if (delta < 0) {
        ...
        input_frame = input_frame *(RESAMPLE_FRAMES + delta) / RESAMPLE_FRAMES ;
        if (input_frame > 1) {
            input_frame -= 1;
        }
        *size = input_frame * frame_size;
        //adec_print("req:%d,calc read size:%d  frame:%d\n", req_size, *size,input_frame);
    }
    *size -= (*size % (frame_size));
    read_bytes = pcrmaster_dsp_pcm_read(audec, pbuf, *size);
    read_from_buf = read_bytes;
    if (read_bytes % frame_size) {
        adec_print("warning read size  before src not frame align %d\n", read_bytes);
        goto exit;
    }
    if (read_bytes > 0) {
        ...
        out_frame = resample_process(&resampler, read_bytes / frame_size,
                                     pbuf, buffer);
        *size =  out_frame * frame_size;
    } else {
        *size = 0;
    }
    //adec_print(":af_resample_api_new rate :%f, size :%d\n", rate,*size);
exit:
