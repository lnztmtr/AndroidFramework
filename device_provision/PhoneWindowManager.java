...
//        sApplicationLaunchKeyCategories.append(
	...
	boolean isStandby=false;//False standby
	...
	private static final int MSG_RESET_RECOVERY = 11;//at 20200530 for henan open recovery
	...
	private int recoveryCount=0;//at20200531 for henan recovery
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
				...
				case MSG_OPEN_WIFI_DIALOG:
					showWifiDialog();
                    break;
				case MSG_RESET_RECOVERY:                    
					//mContext.deleteDatabase("/params/stbconfig/stbconfig.db")
...
//add by ysten.huanghongyan 2018.11.27 for CM201_henan
        if(
			...
			//add by liangk at 20200320 for liaoning default 4 hour sleepTime
	  ...
	  //add by liangk at 20200320 for liaoning default 4 hour sleepTime
			if((sleepDelay == -1)&&!SystemProperties.get("ro.ysten.province","master").contains("liaoning")){
        ...
        //begin:add by ysten guxin for jiangxi sleep time delay
        if(SystemProperties.get("ro.ysten.province","master").contains("jiangxi")){
            mHandler.removeCallbacks(mSystemSleep);
            int sleepDelay = Settings.System.getInt(mContext.getContentResolver(),Settings.System.SLEEP_TIME,-1);
            if(sleepDelay > 0)
                mHandler.postDelayed(mSystemSleep,sleepDelay*60*1000);
            Log.d(TAG,"new jiangxi sleepdelay init = " + sleepDelay);
        }
        //end:add by ysten guxin for jiangxi sleep time delay
		 //begin:add by ysten zhanghui for henan sleep time delay
        if(SystemProperties.get("ro.ysten.province","master").contains("CM201_henan")){
            mHandler.removeCallbacks(mSystemSleep);
            int sleepDelay = Settings.System.getInt(mContext.getContentResolver(),Settings.System.SYSTEM_SLEEP_TIME_DELAY,-1);
            if(sleepDelay > 0)
                mHandler.postDelayed(mSystemSleep,sleepDelay*SLEEP_DELAY_UNIT);
            Log.d(TAG,"new henan sleepdelay init = " + sleepDelay);
        }
        //end:add by ysten zhanghui for henan sleep time delay
...
private Runnable mSystemSleep = new Runnable(){
	public void run(){ 
        if(SystemProperties.get("ro.ysten.custom.type","none").equals("viper")
            ||SystemProperties.get("ro.ysten.province").contains("CM201_hubei")){
            Log.d(TAG, "custom.type:viper4"+mInPlaying);
			if(mInPlaying)return;
			int sleepDelay = Settings.System.getInt(mContext.getContentResolver(), SYSTEM_SLEEP_TIME_DELAY, -1);
			...
			boolean isUpgrading=(Settings.System.getInt(mContext.getContentResolver(), ROOM_UPGRADE_FLAG, -1)==1);
			Log.d(TAG, "about to start sleep  = " + sleepDelay+"isUpgrading"+isUpgrading);
			final AudioManager am = (AudioManager)mContext.getSystemService(Context.AUDIO_SERVICE);//determin music is action
			boolean isPlayingAudio = false;
			if(null != am)isPlayingAudio = SystemProperties.get("service.media.playstatus","none").equals("running");
            Log.d(TAG, "about to start sleep isPlayingAudio = " + isPlayingAudio+"isUpgrading"+isUpgrading);
		...
		if(SystemProperties.get("ro.ysten.province","master").contains("CM201_henan")){
			int sleepDelay = Settings.System.getInt(mContext.getContentResolver(),SYSTEM_SLEEP_TIME_DELAY,-1);
			if(sleepDelay<0){
				Log.d(TAG,"4 hour standby is enable");
			}else{
				showSleepDelayDialog();
			}
        ...
        //end:add by zhanghk at 20190522:for 4 hour standby   
			//begin:by ysten liangk at 20200227:liaoning 4 hours go to sleep
			ActivityManager activityManager=(ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
			String currentTopAct=activityManager.getRunningTasks(1).get(0).topActivity.getClassName();
			Log.d("ystenlk","delay = " + Settings.System.getInt(mContext.getContentResolver(),SYSTEM_SLEEP_TIME_DELAY,-1));
			if(SystemProperties.get("ro.ysten.province","master").contains("liaoning") && (!currentTopAct.equals("com.softwinner.dragonbox"))){
				showLiaoningSleepDelayDialog();
				return;
			}
			//end:add by ysten liangk at 20200227:liaoning 4 hours go to sleep
		}
	};
	
	//add by ysten liangk at 20200227:liaoning 4 hours go to sleep
...
//			mSleepDelayTextView.setText(R.string.continue_show1);
	...
	//end by ysten liangk at 20200227:liaoning 4 hours go to sleep
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
	    ...
	    /*if(isCancelDialog){
		    ...
		    isCancelDialog=false;
                ...
                if(sleepDelay > 0)
			   	...
			   	//start by zhanghui 20200601 for henan open recovery mode				
			   	Log.d("zhanghui", "recoveryCount>>=="+recoveryCount);
				if(keyCode==82 && recoveryCount==0){
					recoveryCount++;

				}else if(keyCode==25 && recoveryCount==1){
					recoveryCount++;

				}else if(keyCode==82 && recoveryCount==2){		
					recoveryCount++;				

				}else if(keyCode==25 && recoveryCount==3){					
					recoveryCount++;

				}else if(keyCode==82 && recoveryCount==4){		
					recoveryCount++;
				}else if(keyCode==25 && recoveryCount==5){
					mContext.deleteDatabase("/params/stbconfig/stbconfig.db");
					mHandler.sendEmptyMessage(MSG_RESET_RECOVERY);					
				}else {
					recoveryCount=0;
					
				}
				//end by zhanghui 20200601 for henan open recovery mode
            }
			if(down){
        ...
        if (DEBUG_INPUT) {
            Log.d(TAG, "interceptKeyTi keyCode=" + keyCode + " down=" + down + " repeatCount="
                    ...
                    + " canceled=" + canceled+" "+SystemProperties.get("ro.ysten.province","master"));
        }
		...
		if(SystemProperties.get("ro.ysten.province","master").contains("CM201_jiangsu")
			...
			if(down){
                ...
                if("cm201_jiangxi".equals(SystemProperties.get("ro.ysten.province"))){
                    sleepDelay= Settings.System.getInt(mContext.getContentResolver(),Settings.System.SLEEP_TIME,-1); 
                    Log.d(TAG,"new jiangxi  sleeptime  = " + sleepDelay);
                }else{
       ...
       if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            if(SystemProperties.get("ro.ysten.province").equals("CM201_guangdong")){
                ...
                String currentPackage=activityManager.getRunningTasks(1).get(0).topActivity.getClassName();
                Log.d(TAG," currentPackage = " + currentPackage);
                if(currentPackage.contains("com.ysten.auth.guangdong.MainActivity")){
                    return -1;
                }
            }
            //add end by zhangjunjian at 2019118 for guangdong fix home press bug
			//begin by ysten wenglei at 20200303 for guangdong_zhuoying fix home press bug
			if(SystemProperties.get("ro.ysten.province").equals("CM201_guangdong_zhuoying")){
                ActivityManager activityManager = (ActivityManager)mContext.getSystemService(Context.ACTIVITY_SERVICE);
                String currentPackage = activityManager.getRunningTasks(1).get(0).topActivity.getClassName();
                Log.d(TAG, "==leo currentPackage = " + currentPackage);
                if(currentPackage.contains("com.ysten.auth.guangdong.MainActivity") || ("false".equals(SystemProperties.get("sys.logined", "false")))){
                    return -1;
                }
            }
			//end by ysten wenglei at 20200303 for guangdong_zhuoying fix home press bug
	...
	//add by zhanghk at 20181018 begin:press some key to start allapp
		...
		if("jiangxi".equalsIgnoreCase(SystemProperties.get("sys.proj.tender.type"))){
			...
			if(	down
				...
				Slog.d(TAG,"===,receive colorfunkey jiangxi");
				ColorFunKey(keyCode);
			}
		}
        ...
        if("cm201_jiangxi".equals(SystemProperties.get("ro.ysten.province"))){
            if(down&&
				...
				//bengin: add by penghui at 20191010 for jianxi ott apk5.0 four color key
				else{
					Slog.d(TAG,"===,receive colorfunkey cm201_jiangxi");
				   ColorFunKey(keyCode);
				}
				//end: add by penghui  at 20191010  for jianxi ott apk5.0 four color key
            }
			//bengin: add by penghui at 20191010 for jianxi home broadcast
			if (keyCode == KeyEvent.KEYCODE_HOME && down){
				Slog.d(TAG,"===,receive home,send a broadcast:android.intent.action.HOME_KEY_PRESSED");
				startOTTHome();
				return -1;
    ...
    //end:add  by ysten zengzhiliang at 20190123 yunnan 4 color key proc
        ...
        //end by ysten.huanghongyan 2018.11.27 for CM201_henan  
            if (!down) {
                ...
                if (!(SystemProperties.get("ro.ysten.province").contains("fujian")||SystemProperties.get("ro.ysten.province","master").contains("CM201_henan")
                    ...
                    ||SystemProperties.get("ro.ysten.province").contains("guangdong"))) {
                    launchHomeFromHotKey();
					}
                }
			...
			if(SystemProperties.get("ro.ysten.province","master").equals("CM201_guangdong")){
                String topPackageName = getTopPackage(mContext);
                if(!TextUtils.isEmpty(topPackageName) && !topPackageName.contains("cn.gd.snm.snmcm")) {
                    if(!down){
                        Intent iptvIntent = new Intent();
                        iptvIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			...
			//begin by ysten wenglei at 20200303 for guangdong_zhuoying
			if(SystemProperties.get("ro.ysten.province","master").equals("CM201_guangdong_zhuoying")){
                String topPackageName = getTopPackage(mContext);
                Log.d(TAG, "==leo guangdong zhuoying topPackageName "+topPackageName);
                if(!TextUtils.isEmpty(topPackageName) && !topPackageName.contains("cn.gd.snm.snmcm")) {
                    if(!down){
                        Log.d(TAG, "==leo guangdong zhuoying open cn.gd.snm.snmcm.LauncherActivity");
                        Intent iptvIntent = new Intent();
                        iptvIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                        ComponentName comp =new ComponentName("cn.gd.snm.snmcm","cn.gd.snm.snmcm.LauncherActivity");
                        iptvIntent.setComponent(comp);
                    ...
                    Log.d(TAG, "==leo guangdong zhuoying home to user");
                    Intent panelIntent=new Intent("KEY_HOME");
                    mContext.sendBroadcast(panelIntent);
                   return 0;
                }  
            }
            //end by ysten wenglei at 20200303 for guangdong_zhuoying
        ...
        if("cm201_jiangxi".equals(SystemProperties.get("ro.ysten.province"))){
            sleepDelay= Settings.System.getInt(mContext.getContentResolver(),Settings.System.SLEEP_TIME,-1);
            Log.d(TAG,"new jiangxi  sleeptime in viper  = " + sleepDelay);
        }else{
            sleepDelay= Settings.System.getInt(mContext.getContentResolver(),SYSTEM_SLEEP_TIME_DELAY,-1);
        }
        ...
        //begin: add by tianchining at 20191118: switch mode
            ...
            if(robotKeyComponent2(keyCode,down)){
               String topPackageName = getTopActivity(mContext);
               Log.d(TAG, "CM201_xingxia: topPackageName = "+topPackageName);
               if (!"nx.mobile.iptv.player.LivePlayerActivity".equals(topPackageName)) {
    ...
    private boolean robotKeyComponent1(int keyCode) {
        ...
        private static final boolean[] BOOLEAN_KEY_LIST2 = {
            true,
            false
        };
        ...
        private boolean robotKeyComponent2(int keyCode, boolean bKeyDown){
            ...
            if(keyCode == ROBOT_KEY_LIST2[mCurrentRobotKeyIndex2] 
                //&& getTopActivityInfo(mContext).equals("nx.mobile.iptv") 
                 ...
                 if (BOOLEAN_KEY_LIST2[mCurrentRobotKeyIndex2 % 2] == bKeyDown) {
                ...
                mCurrentRobotKeyIndex2 = 0;
                if(keyCode == ROBOT_KEY_LIST2[0] && bKeyDown == true) {
                    mCurrentRobotKeyIndex2++;
                    return false;
                }
            }
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags, boolean isScreenOn) {
        ...
        if ((isScreenOn && !mHeadless) || (isInjected && !isWakeKey)) {
            ...
            if (down && isWakeKey && isWakeKeyWhenScreenOff(keyCode)) {
				...
				(SystemProperties.get("sys.proj.type", "ott").equals("mobile") && suspendbycec)){
					//begin: add by zhanghui at 20200413: henan wake the device or standby
					if(SystemProperties.get("ro.ysten.province", "master").contains("CM201_henan")){
						if(isStandby){
					...
					//end: add by zhanghui at 20200413: henan wake the device or standby
                    ...
                    if(SystemProperties.get("ro.ysten.province", "master").contains("master")||
						SystemProperties.get("ro.ysten.province", "master").contains("jiangxi")){ //add by xuyunfeng at 20191224:fallback ningxia fake standby
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_MOBILE_M: {
                //begin: add by ysten wenglei at 20200526 for : 广东现网认证界面屏蔽M键
                if((SystemProperties.get("ro.ysten.province", "master").equals("CM201_guangdong"))
                    && is_topwindow("com.ysten.auth")){
                    break;
                }
                //end: add by ysten wenglei at 20200526 for : 广东现网认证界面屏蔽M键
				//begin: add by ysten wenglei at 20200302 for BUGID 6303: 卓影认证界面屏蔽M键
                if((SystemProperties.get("ro.ysten.province", "master").equals("CM201_guangdong_zhuoying"))
                    && is_topwindow("com.android.smart.terminal.iptv")){
                    break;
                }
                //end: add by ysten wenglei at 20200302 for BUGID 6303: 卓影认证界面屏蔽M键
                // begin: add by ysten 翁磊 at 20200402: 广东现网修复M键导致应用商城apk崩溃
                if (SystemProperties.get("ro.ysten.province", "master").equals("CM201_guangdong")
                    ...
                    if (!down) {
                        ...
                        if (!"cn.gd.snm.appstore".equals(topActivityPactage)) {
                            Log.d("zjj", "M key");
                            Intent intent = new Intent();
                            if (null != mContext) {
                                intent.setComponent(
                                        new ComponentName("cn.gd.snm.appstore", "cn.gd.snm.appstore.MainActivity"));
                                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                                try {
                                    mContext.startActivity(intent);
                                } catch (Exception e) {
                ...
                // end: add by ysten 翁磊 at 20200402: 广东现网修复M键导致应用商城apk崩溃
                break;
            }
            ...
            case KeyEvent.KEYCODE_STB_POWER: {
				...
				if(SystemProperties.get("ro.ysten.custom.type","none").equals("viper")
				    ...
				    //add by liangk at 20200320 for liaoning poweroff fun
					&& !SystemProperties.get("ro.ysten.province","master").contains("liaoning"))
				{
				...
				if(SystemProperties.get("ro.ysten.province","master").equals("CM201_henan")){
					//start：add zhanghui,at 2020.05.26,cm201_henan for 假待机前回首页
					String packagename = SystemProperties.get("epg.launcher.packagename");
					isStandby=true;
		            if(packagename == null || packagename.equals("")) {
                       Log.d(TAG,"KEYCODE_HOME start com.huawei.tvbox");
					...
					//end: add by zhanghui at 20200526: cm201_henan for 假待机前回首页
                    ...
                    if (interceptPowerKeyUp(mHdmiPlugged || canceled || mPendingPowerKeyUpCanceled)) {
                        ...
                        if(SystemProperties.get("ro.ysten.province","master").contains("hubei")
							||SystemProperties.get("ro.ysten.province","master").contains("jiangxi")){
								 Log.w(TAG, "sys.fake.standby"+ SystemProperties.get("sys.fake.standby", "1"));
            ...
            case KeyEvent.KEYCODE_SETTINGS: {
					...
					if(down)
					{
							...
							}else if(productName.equals("CM201_guangdong_zhuoying")){
							//add by ysten.wenglei 20200722 for guangdong zhuoying settings 
							...
							//end by ysten.wenglei 20200722 for guangdong zhuoying settings 
                            }
                            ...
                            } else if(productName.contains("M302A_gw")
                                ||productName.contains("M301A_gw")
                                ||productName.contains("CM201_gw")){
								intent.setComponent(new ComponentName("com.android.settings", "com.android.settings.Settings"));
							}
	...
	case KeyEvent.KEYCODE_POWER: {
		...
		break;
            ...
            case KeyEvent.KEYCODE_AVR_POWER: {
				...
				Intent mintent = new Intent();
			    if(SystemProperties.get("ro.ysten.province","master").equals("cm201_jiangxi")){
					//add by penghui 20191014  IPTV的home键信息上报
				     // inputkeyevent(181);
					  Log.i(TAG, " KeyEvent.KEYCODE_AVR_POWER： " + KeyEvent.KEYCODE_AVR_POWER+"，result: " + result );
...
//end:add  by ysten zengzhiliang at 20190123 yunnan 4 color key proc
	...
	private void doJXFunAction(int keyCode) {
		...
		if(launcherValue.equals("2")){
			if(IPTVKey==181){
				...
				Log.d(TAG,"jiangxi----inputkeyevent2 com.ysten.sjiptv.app.SJIPTVActivity: "+IPTVKey);
				ComponentName compName= new ComponentName("com.ysten.sjiptv","com.ysten.sjiptv.app.SJIPTVActivity");
				mIntent.setComponent(compName);
				mIntent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
				mContext.startActivity(mIntent);
			...
			if(IPTVKey!=0){
				inputkeyevent(IPTVKey);
                                Log.d(TAG,"jiangxi----inputkeyevent2 IPTV-keycode: "+IPTVKey);
			}
		}
	}
        ...
        mHandler.post(new Runnable() {
            ...
            public void run() {
               ...
               builder.setTitle(com.android.internal.R.string.suspend_off);
                ...
                AlertDialog suspendDialog = builder.create();
                suspendDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
                suspendDialog.show();
                

                 
                if(suspendDialog.getButton(AlertDialog.BUTTON_NEGATIVE)==null){
                     Slog.d(TAG, "SDialog button is null");
                }else{
                    suspendDialog.getButton(AlertDialog.BUTTON_NEGATIVE).setFocusable(true);
                    suspendDialog.getButton(AlertDialog.BUTTON_NEGATIVE).setFocusableInTouchMode(true);
                    suspendDialog.getButton(AlertDialog.BUTTON_NEGATIVE).requestFocus();
                }
            }
        });
        new Thread(new Runnable(){
            ...
            public void run() {
                try{
                  ...
                  Slog.d(TAG, "Show Suspend Dialog 001");
                }
              Slog.d(TAG, "Show Suspend Dialog 002");
              mShowPowerDialog = false;
              if(SystemProperties.get("ro.ysten.province","master").contains("jiangxi")){
                Slog.d(TAG, "Show Suspend Dialog 003");
                SystemProperties.set("persist.sys.smartsuspendin", "0");
                sendCloseSystemWindows(SYSTEM_DIALOG_REASON_GLOBAL_ACTIONS);
                mWindowManagerFuncs.shutdown(true);
              }else{
                Slog.d(TAG, "Show Suspend Dialog 004");
                SystemProperties.set("persist.sys.smartsuspendin", "1");
                mPowerManager.goToSleep(SystemClock.uptimeMillis());
              }
              //add by penghui 待机后关闭倒计时对话框
	...
	AlertDialog mDialog,mWifiDialog,jsDialog;//add zhanghui for dialog not dismiss
	...
	void showSleepDelayDialog() {
        ...
        //end: add by tianchining at 20190327: showSleepDelayDialog for ningxia
			...
			mCount = 30;
			Slog.d(TAG, "jiangxi chinamobile showSleepDelayDialog");
			mSleepDelayTextView = new TextView(mContext);
			Slog.d(TAG, "mSleepDelayTextView 1 "  + mSleepDelayTextView);
			...
			/*mSleepDelayTextView.setText(mContext.getResources().getString(com.android.internal.R.string.standby_warn,mCount));
			mSleepDelayTextView.setTextSize(25);*/
			Slog.d(TAG, " jiangxi mSleepDelayTextView 3 "  + mSleepDelayTextView);
			mHandler.removeCallbacks(mSleepCancleTimer);
			mHandler.postDelayed(mSleepCancleTimer, 1000);
			isCancelDialog=true; 	
		}
    ...
    private Runnable mSleepCancleTimer = new Runnable()
    {
        public void run()
        {
            ...
            if(mCount>=0) {
                mHandler.removeCallbacks(mSleepCancleTimer);
                mHandler.postDelayed(mSleepCancleTimer, 1000);
                Slog.d(TAG, "mSleepCancleTimer " + mSleepDelayTextView + "mCount = " + mCount);
				...
				//begin:add by liangk at 20200320 for liaoning 
		        }else if(SystemProperties.get("ro.ysten.province").contains("liaoning")){
                	mSleepDelayTextView.setText(mContext.getResources().getString(com.android.internal.R.string.standby_warn_ln,mCount));
				//end:add by liangk at 20200320 for liaoning 
		        }else{
                    mSleepDelayTextView.setText(mContext.getResources().getString(com.android.internal.R.string.standby_warn,mCount)); 
		        }   
		        /*modify end*/
	...
	if(SystemProperties.get("ro.ysten.province","master").contains("jiangxi")){
		SystemProperties.set("persist.sys.smartsuspendin", "0");
		Intent mIntent = new Intent("android.intent.action.ACTION_REQUEST_SHUTDOWN");
        mIntent.putExtra("android.intent.extra.KEY_CONFIRM", false);
	...
	jsHandler=new Handler();
        jsHandler.post(new Runnable() {
            ...
            public void run() {			
                ...
                jsDialog = builder.create();
                jsDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
                jsDialog.show();
                if(jsDialog.getButton(AlertDialog.BUTTON_NEGATIVE)==null){
                    Slog.d(TAG, "SDialog button is null");
                }else{
                    jsDialog.getButton(AlertDialog.BUTTON_NEGATIVE).setFocusable(true);
                    jsDialog.getButton(AlertDialog.BUTTON_NEGATIVE).setFocusableInTouchMode(true);
                    jsDialog.getButton(AlertDialog.BUTTON_NEGATIVE).requestFocus();
                }
            }
	});
        ...
        final Runnable shutdown = new Runnable() {
            ...
            public void run() {
              try{
                Thread.sleep(5000);
                Slog.d(TAG, "Show Suspend Dialog  0001");
                mShowPowerDialog = false;
				/*modify by zhaghui for henan at20200528 for dialog haven't dismiss*/
				if(jsDialog!=null){
					jsDialog.dismiss();
				}
                //auto poweroff
                ...
                if("cm201_guizhou".equals(SystemProperties.get("ro.ysten.province"))){
                  Slog.d(TAG, "Show Suspend Dialog  0002");
                  mPowerManager.goToSleep(SystemClock.uptimeMillis());
				   
                }else{
                  Slog.d(TAG, "Show Suspend Dialog  0003");
                  mWindowManagerFuncs.shutdown(true);
                }
                dissSleepDelayDialog();
                if(suspendDialog !=null){
