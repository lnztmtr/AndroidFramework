...
int Process::pathMatchesMountPoint(const char* path, const char* mountPoint) {
    ...
    // check for mounts that are bind mounted:
    // /storage/external_storage/sdcard1 bind mounted to
    //      /storage/emulated/0/external_storage/sdcard1
    if (strncmp(mountPoint, "/storage/", strlen("/storage/")) == 0) {
        // search for /external_storage/sdcard1 with leading slash
        tem_pos = strstr(path, mountPoint + strlen("/storage"));
        if (tem_pos) {
            if (mountPoint[length - 1] == '/')
                return 1;
            // make sure match has trailing slash
            return (tem_pos[length - strlen("/storage")] == '/');
        }
    }
    return 0;
}
    ...
    while ((de = readdir(dir))) {
        ...
        if (checkFileDescriptorSymLinks(pid, path, openfile, sizeof(openfile))) {
            ...
            if (!strcmp(name, "system_server")) {
                SLOGW("Skipping system_server");
                continue;
            }
            if (!strcmp(name, "net.sunniwell.service.swupgrade.usb")) {
                SLOGW("Skipping net.sunniwell.service.swupgrade.usb");
                continue;
            }
