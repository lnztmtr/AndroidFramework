...
#define TOTAL_TRIALS 3

    for (j=0; j<TOTAL_TRIALS; j++) {
        if (j == (TOTAL_TRIALS-1)) {
            /* somehow app_process will not exit.  Force it here */
            system("kill `pidof app_process`");
        }
        ...
        if (i < WAIT_UNMOUNT_COUNT) {
            SLOGD("unmounting %s succeeded at %d trial\n", mountpoint, j);
            rc = 0;
            break;
        } else {
            SLOGE("unmounting %s failed at %d trial\n", mountpoint, j);
            rc = -1;
        }
    }
