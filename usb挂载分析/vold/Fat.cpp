...
int Fat::check(const char *fsPath) {
    ...
    do {
        ...
        // in case of fsck be killed or exceptional terminated
        if (!WIFEXITED(status)) {
...
#else
    /**
     * fsck_msdos will corrupt exFat header
     * disable FAT fsck if exFAT is enabled
     */
     SLOGW("Skipping FAT check");
     return ENODATA;
#endif /* ! HAS_EXFAT */
}
...
int Fat::format(const char *fsPath, unsigned int numSectors, bool wipe, const char *label) {
    ...
    if (numSectors) {
        ...
        rc = android_fork_execvp(argc, (char **)args, &status,
                false, true);
    } else {
        if (label != NULL) {
            args[7] = "-L";
        ...
        rc = android_fork_execvp(argc, (char **)args, &status, false,
                true);
    }
