...
#include "Asec.h"
#include "cryptfs.h"
#define MASS_STORAGE_FILE_PATH_0  "/sys/class/android_usb/android0/f_mass_storage/lun0/file"
#define MASS_STORAGE_FILE_PATH_1  "/sys/class/android_usb/android0/f_mass_storage/lun1/file"
#define MASS_STORAGE_FORCE_SYNC_1  "/sys/class/android_usb/android0/f_mass_storage/lun1/force_sync"
...
void VolumeManager::coldbootUmsSwitch() {
#if 0
    FILE *fp;
    char state[255];
...
int VolumeManager::deleteVolume(VolumeCollection::iterator it) {
    (*it)->~Volume();
    mVolumes->erase(it);
    return 0;
}
...
#define VOLD_MOUNT_DIR "/storage/external_storage/"
...
void VolumeManager::handleBlockEvent(NetlinkEvent *evt) {
...
#ifdef NETLINK_DEBUG
	     if(NetlinkEvent::NlActionRemove == evt->getAction())
			SLOGD("remove event handled\n");
	     else if(NetlinkEvent::NlActionAdd == evt->getAction())
			SLOGD("add event handled\n");
	     else if(NetlinkEvent::NlActionChange == evt->getAction())
		 	SLOGD("change  event handled\n");
            SLOGD("Device '%s' event handled by volume %s\n", devpath, (*it)->getLabel());
#endif
            hit = true;
	     if(media_poll_flag)
	     {
		      //if it is a ums device && remove uevent recevied, delete it in volume collection
	            if ((NetlinkEvent::NlActionRemove == evt->getAction()||(NetlinkEvent::NlActionChange == evt->getAction()&&(*it)->getState() != Volume::State_Mounted))
	                    && VOLUME_TYPE_UMS == (*it)->getVolumeType()) {
#ifdef FUNCTION_UMS_PARTITION
			 if(!strcmp(devtype, "partition")){
				deleteVolume(it);					//partition 
			 }
			 else
			 {
				if(DiskNumParts == 0)
				{
					deleteVolume(it);				//no partition table case
				}
			 }
#else
    ...
    if (!hit) {
        /* Is it a ums device? If yes, add it to volume collection and handle it*/
        for (iter = mUms->begin(); iter != mUms->end(); ++iter) {
            if ((*iter)->isInUmsDevPath(devpath)) {
                if (NetlinkEvent::NlActionAdd == evt->getAction()) {
                    char mount_point[256] = {0};
                    sprintf(mount_point, "%s%s", VOLD_MOUNT_DIR, devname);
		      
#ifdef NETLINK_DEBUG
                    SLOGD("Device '%s' is add in by ums path %s\n", devpath, (*iter)->getUmsDevPath());
#endif
					...
					if(media_poll_flag){
						if(!sr_disk)
						{
							...
							if (fd >= 0) {
								close (fd);
#ifdef NETLINK_DEBUG							
								SLOGE("1- open success for %s", nodepath);
#endif
							}else{
#ifdef NETLINK_DEBUG
								SLOGE("1-open failed for %s: %s", nodepath, strerror (errno));
#endif
								return;
							}
						}
						...
						{
							SLOGD("if sr disk,do nothing in add event");
							return;
						}
					}
					DirectVolume *dv = new DirectVolume(this, devname, mount_point, -1);
		                     dv->addPath(devpath);
				...
				{
					SLOGD("Device '%s'(add) has %d partitions,will mount by partition uevent\n", devpath,DiskNumParts);		
				}
                	} else {
                    DirectVolume *dv = new DirectVolume(this, devname, mount_point, -1);
                    dv->addPath(devpath);
...
#ifdef NETLINK_DEBUG
	       		SLOGD("Device '%s' change is in by ums path %s\n", devpath, (*iter)->getUmsDevPath());
#endif					 
	               	if (!strcmp(devtype, "disk")) {
					snprintf(nodepath,sizeof(nodepath), "/dev/block/%s",devname);	
					fd = open (nodepath, O_RDONLY);
					if (fd >= 0) {
						close (fd);
#ifdef NETLINK_DEBUG						
						SLOGE("2- open success for %s", nodepath);
#endif
						if(sr_disk)
						{
							DirectVolume *dv = new DirectVolume(this, devname, mount_point, -1);
...
#ifdef NETLINK_DEBUG					
						SLOGE("2-open failed for %s: %s", nodepath, strerror (errno));
#endif
						if(errno==ENOMEDIUM)
							return;
					}
						
					SLOGD("Device '%s' has %d partitions\n", devpath,DiskNumParts);					             	
					return;
				}
                	}
                }
...
int VolumeManager::setAsecVolume(const char * label)
{
    VolumeCollection::iterator i;
    ...
    for (i = mVolumes->begin(); i != mVolumes->end(); ++i) {
        ...
        {
             (*i)->setVolHasAsec(true);
        SLOGI("Set Asec Volume:%s",label);
        }
        else
             (*i)->setVolHasAsec(false);
    }
    return 0;
}
...
int VolumeManager::formatVolumeWithFsType(const char *label, const char *fsType, bool wipe) {
    Volume *v = lookupVolume(label);
    ...
    return v->formatVolWithFsType(fsType, wipe);
}

int VolumeManager::getVolumeFSLabel(const char *path, SocketClient *cli) {
    Volume *v = lookupVolume(path);
    char label[200];
    char *labelp = label;
    int err;
    ...
    label[0] = 0;
    err = v->getFSLabel(&labelp);
    if (!err) {
        cli->sendMsg(ResponseCode::VolumeLabelResult, label, false);
    }
    return err;
}
...
int VolumeManager::createAsec(const char *id, unsigned int numSectors, const char *fstype,
     ...
     * Add some headroom
     */
    unsigned fatSize = (((numSectors * 4) / 512) + 1) * 4;
...
int VolumeManager::unmountAsec(const char *id, bool force) {
    ...
    int ret = unmountLoopImage(id, idHash, asecFileName, mountPoint, force, false);
    if (ret)
        ret = unmountLoopImage(id, idHash, asecFileName, mountPoint, force, true);
    return ret;
}
...
int VolumeManager::unmountObb(const char *fileName, bool force) {
    ...
    return unmountLoopImage(fileName, idHash, fileName, mountPoint, force, false);
}
...
int VolumeManager::unmountLoopImage(const char *id, const char *idHash,
    ...
    char msg[255];
    snprintf(msg, sizeof(msg), "Volume sdcard /mnt/sdcard disk removed (253:0)");
    ...
    for (i = 1; i <= UNMOUNT_RETRIES; i++) {
        ...
        if (triggerDiskRemove && errno == EBUSY)
            getBroadcaster()->sendBroadcast(ResponseCode::VolumeDiskRemoved, msg, false);
...
int VolumeManager::shareVolume(const char *label, const char *method) {
    ...
    int shareindex = v->getVolumeShareIdx();
    // some disks, eg flash, should have only one partition shared
    if (v->getVolumeShareIdx() == -1) {
        ...
        int shareminor = v->getVolumeMinor(v->getVolumeShareIdx() - 1);
        if (shareminor == -1) {
           errno = EINVAL;
           return -1;
        }
    ...
    const char* lunpath;
    if (VOLUME_TYPE_SDCARD == v->getVolumeType())
        lunpath = MASS_STORAGE_FILE_PATH_0;
    else if (VOLUME_TYPE_FLASH == v->getVolumeType())
        lunpath = MASS_STORAGE_FILE_PATH_1;
    else {
        SLOGE("sharing type %d unsupported", v->getVolumeType());
		 v->handleVolumeShared();
        return 0;
    }

    if (VOLUME_TYPE_FLASH == v->getVolumeType()){
    	char value[PROPERTY_VALUE_MAX];
		property_get("gadget.nand.force_sync", value, "false");
		if(!strcmp(value, "true"))
		{
			if ((fd = open(MASS_STORAGE_FORCE_SYNC_1, O_WRONLY)) < 0) {
        		SLOGE("Unable to open ums force_sync (%s)", strerror(errno));
        		return -1;
    		}

    		if (write(fd, "1", strlen(nodepath)) < 0) {
        		SLOGE("Unable to write to ums force_sync (%s)", strerror(errno));
        		close(fd);
        		return -1;
    		}
			close(fd);
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
    ...
    if (VOLUME_TYPE_SDCARD == v->getVolumeType())
        ...
        SLOGE("unsharing type %d unsupported", v->getVolumeType());
...
int VolumeManager::cleanupAsec(Volume *v, bool force) {
    ...
    for (AsecIdCollection::iterator it = removeObb.begin(); it != removeObb.end(); ++it) {
        ContainerData *cd = *it;
        SLOGI("Unmounting OBB %s (dependant on %s)", cd->id, v->getMountpoint());
