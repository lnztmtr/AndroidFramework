...
class MountService extends IMountService.Stub
    ...
    private void updatePublicVolumeState(StorageVolume volume, String state) {
        ...
        Slog.d(TAG, "VolumeLabel: " + getVolumeLabel(path));
        // Tell PackageManager about changes to primary volume state, but only
        // when not emulated.
        if ((volume.isPrimary() && !volume.isEmulated())
			||SystemProperties.getBoolean("ro.storage.move2sdcard",false)){
            if (Environment.MEDIA_UNMOUNTED.equals(state)) {
                if(!SystemProperties.getBoolean("ro.storage.move2sdcard",false)
                    || getVolumeLabel(path).startsWith("sdcard"))
    ...
    private void notifyVolumeStateChange(String label, String path, int oldState, int newState) {
        ...
        if (newState == VolumeState.Init) {
             ...
             */
            if (!state.equals(Environment.MEDIA_BAD_REMOVAL) 
    ...
    private int doFormatVolumeWithFsType(String path, String fsType) {
        try {
            mConnector.execute("volume", "format", path, "fstype", fsType);
            return StorageResultCode.OperationSucceeded;
        } catch (NativeDaemonConnectorException e) {
            int code = e.getCode();
            if (code == VoldResponseCode.OpFailedNoMedia) {
            ...
            while (true) {
                ...
                if (TAG_STORAGE.equals(element)) {
                    ...
                    int mtpReserve = a.getInt(
                            com.android.internal.R.styleable.Storage_mtpReserve, 100);
   ...
   /**
    ...
    public String getVolumeFSLabel(String mountPoint) {
        final NativeDaemonEvent event;
        try {
            event = mConnector.execute("volume", "label", mountPoint);
        } catch (NativeDaemonConnectorException ex) {
            Slog.e(TAG, "Failed to read response to volume label, mountPoint:" + mountPoint);
            return null;
        }
        String label = null;
        label = (event==null)?null:event.getMessage();
        Slog.d(TAG,"getVolumeFSLabel(), label:"+label);
        return label ;
    }
    ...
    public int formatVolumeWithFsType(String path, String fsType) {
        validatePermission(android.Manifest.permission.MOUNT_FORMAT_FILESYSTEMS);
        waitForReady();

        return doFormatVolumeWithFsType(path, fsType);
    }
    ...
    public String getSecureContainerPath(String id) {
        ...
        waitForReady();
        //warnOnNotMounted();
        boolean notmounted = warnOnNotMounted();
        if (notmounted) {
            return "/mnt/asec/" + id;
        }
