...
//begin add by ysten chenfeng at 20190121 for jiangsu recovery store wifi account
#include "cutils/properties.h"

static const char HOSTAPD_CONF_FILE[]    = "/data/misc/wifi/hostapd.conf";
static const char HOSTAPD_CONF_FILE_JS[]    = "/private/misc/wifi/hostapd.conf";
...
int SoftapController::startSoftap() {
    ...
    if (!pid) {
        ...
        //begin add by ysten chenfeng at 20190121 for jiangsu recovery store wifi account
        char province_property[PROPERTY_VALUE_MAX] = {0};
        if (property_get("ro.ysten.province", province_property, "master") && strstr(province_property, "jiangsu")!=NULL)
        {
        if (execl(HOSTAPD_BIN_FILE, HOSTAPD_BIN_FILE,
                  "-e", WIFI_ENTROPY_FILE,
                  HOSTAPD_CONF_FILE_JS, (char *) NULL)) {
        ...
        //end add by ysten chenfeng at 20190121 for jiangsu recovery store wifi account
...
int SoftapController::setSoftap(int argc, char *argv[]) {
    ...
    if (property_get("ro.ysten.province", province_property, "master") && strstr(province_property, "jiangsu")!=NULL)
    {
    if (WIFI_RALINK_MT7601U == wifi_device || WIFI_AMPAK_AP6356P == wifi_device) {
        asprintf(&wbuf, "interface=%s\ndriver=nl80211\nctrl_interface="
                "/private/misc/wifi/hostapd\nssid=%s\nchannel=%d\nieee80211n=1\n"
                "hw_mode=%c\nht_capab=[SHORT-GI-20]\nignore_broadcast_ssid=%d\n",
                argv[2], argv[3], channel, hw_mode, hidden);
	} else if (WIFI_AMPAK_AP6356S == wifi_device) {
        asprintf(&wbuf, "interface=%s\n"
                "driver=nl80211\n"
                "ctrl_interface=/private/misc/wifi/hostapd\n"
                "ssid=%s\n"
                "channel=%d\n"
                "ieee80211n=1\n"
                "hw_mode=%c\n"
    ...
    if (property_get("ro.ysten.province", province_property, "master") && strstr(province_property, "jiangsu")!=NULL)
    {
    fd = open(HOSTAPD_CONF_FILE_JS, O_CREAT | O_TRUNC | O_WRONLY | O_NOFOLLOW, 0660);
    if (fd < 0) {
        ALOGE("Cannot update \"%s\": %s", HOSTAPD_CONF_FILE_JS, strerror(errno));
        free(wbuf);
        free(fbuf);
        return ResponseCode::OperationFailed;
    }
    if (write(fd, fbuf, strlen(fbuf)) < 0) {
        ALOGE("Cannot write to \"%s\": %s", HOSTAPD_CONF_FILE_JS, strerror(errno));
    ...
    if (fchmod(fd, 0660) < 0) {
        ALOGE("Error changing permissions of %s to 0660: %s",
                HOSTAPD_CONF_FILE_JS, strerror(errno));
        close(fd);
        unlink(HOSTAPD_CONF_FILE_JS);
        return ResponseCode::OperationFailed;
    }

    if (fchown(fd, AID_SYSTEM, AID_WIFI) < 0) {
        ALOGE("Error changing group ownership of %s to %d: %s",
                HOSTAPD_CONF_FILE_JS, AID_WIFI, strerror(errno));
        close(fd);
        unlink(HOSTAPD_CONF_FILE_JS);
        return ResponseCode::OperationFailed;
    }
...
int SoftapController::fwReloadSoftap(int argc, char *argv[])
{
	...
	if (WIFI_AMPAK_AP6356S != wifi_device)
        return 0;
