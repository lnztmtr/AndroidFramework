...
G3Controller *CommandListener::sG3Ctrl = NULL;
/**
...
#define PPPOE_CONNECT_RESULT_CONNECT           "11"
int CommandListener::IpFwdCmd::runCommand(SocketClient *cli,
    ...
    if (!strcmp(argv[1], "status")) {
        ...
        rc = property_get("pppoe.result", value, "0");

        ALOGD("property_get(pppoe.result), value:%s, ret:%d", value, rc);

        if (!strcmp(value, PPPOE_CONNECT_RESULT_CONNECT))
        {
            ALOGD("setNetShare pppoe");
            sTetherCtrl->setNetShare("192.168.43.0/24", "ppp0");
        } else {
            ALOGD("setNetShare eth0");
            sTetherCtrl->setNetShare("192.168.43.0/24", "eth0");
        }
...
#ifdef HI_3G_SUPPORT
CommandListener::G3Cmd::G3Cmd() : NetdCommand("g3ctrl") {
}

int CommandListener::G3Cmd::runCommand(SocketClient *cli, int argc,
                                       char **argv)
{
    int rc = 0;
    ...
    if (!strcmp(argv[1], "autoConnect")) {
        ALOGD("recv:autoConnect");
        ...
        rc = sG3Ctrl->startG3AutoConnect(cli);

        return 0;
    } else if (!strcmp(argv[1], "manualConnect")) {
        ALOGD("recv:manualConnect");
        ...
        rc = sG3Ctrl->startG3ManualConnect(cli, argv[2], argv[3], argv[4],
                                           argv[5], argv[6], argv[7]);
        return 0;
    } else if(!strcmp(argv[1], "disconnect")) {
        ALOGD("recv:disconnect");

        rc = sG3Ctrl->stopG3Connect(cli);

        return 0;
    }else if(!strcmp(argv[1], "enableG3StateCheckThread")) {
        if(argc < 3){
            cli->sendMsg(ResponseCode::CommandSyntaxError, "Missing argument", false);
            ALOGD("g3ctl enableG3StateCheckThread missing argument");
            return 0;
        }

        ALOGD("recv:enableG3StateCheckThread %s", argv[2]);
        if (!strcmp(argv[2], "true"))
            rc = G3NetSteteManager::Instance()->start();
        else if (!strcmp(argv[2], "false"))
            rc = G3NetSteteManager::Instance()->stop();

        if (!rc) {
            cli->sendMsg(ResponseCode::G3StatusResult, "G3 enable success", false);
        } else {
            cli->sendMsg(ResponseCode::G3StatusResult, "G3 enable failed", false);
        }
