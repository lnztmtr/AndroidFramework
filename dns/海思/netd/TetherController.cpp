...
#define CHECK_RES(errmsg) \
    if(res) ALOGD("iptables " errmsg " failed")

int TetherController::setNetShare(const char *gw, const char *ifname) {
    int res ;

    ALOGD("setNetShare ifname:%s, gw:%s", ifname, gw);
    res = execIptables(V4,
        "-F",
        NULL);
    CHECK_RES("-F");
    ...
    res |= execIptables(V4,
        ...
        "-j", "MASQUERADE",
        NULL);
    CHECK_RES("-t nat -A POSTROUTING -s 192.168.43.0/24 -j MASQUERADE");

    if(!strncmp(ifname, "ppp", 3)) {
        ALOGD("iptables TCPMSS, ifname:%s, gw:%s", ifname, gw);
        ...
        res |= execIptables(V4,
            ...
            "--clamp-mss-to-pmtu",
            NULL);
         CHECK_RES("-t mangle -A FORWARD -s 192.168.43.0/24 -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --clamp-mss-to-pmtu");
    }
