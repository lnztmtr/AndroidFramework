...
#define AV_PKT_FLAG_WITH_HEADER  0x020000 //has HEADER
...
/* */
typedef struct AVhevc_display_info {
	unsigned short display_primaries[3][2];
	unsigned short white_point[2];
	unsigned int luminance[2];
} AVhevc_display_info;

typedef struct AVhevc_ContentLightLevl_info {
	unsigned short content_light_level[2];
} AVhevc_ContentLightLevl_info;
...
#define FF_THREAD_SLICE   2 //< Decode more than one part of a single frame at once
	...
	/* for hevc mastering display info */
	int sei_mastering_display_info_present;
	int sei_cll_info_present;
	int poc_type;
	struct AVhevc_display_info mastering_display_info;
	struct AVhevc_ContentLightLevl_info cll_info;
} AVCodecContext;
