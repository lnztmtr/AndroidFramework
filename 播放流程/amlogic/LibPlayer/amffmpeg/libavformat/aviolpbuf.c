...
int url_lpopen(URLContext *s,int size)
{
	...
	if(!lp->buffer)
	{
		unsigned int failedsize=size/2;/*if no memory used 1/2 size */
		ret=am_getconfig_float("libplayer.ffmpeg.lpbuffaildsize",&value);
		if(ret>=0 && value>=1024){
			failedsize=(unsigned int)value;
		}
...
#if 0
			if ((s->seekflags & URL_SEEK_FAIL_NO_BACK) == 0)
...
int64_t url_lpexseekoffset(URLContext *s, int64_t offset)
{
	...
	if((seek_time=s->prot->url_exseek(s, offset, AVSEEK_LOOPBUFFER_OFFSET))<0){
		lp_print( AV_LOG_ERROR,"[%s:%d] LOOPBUFFER seek failed seek_time=%lld\n",__FUNCTION__,__LINE__,seek_time);
		return -1;			
	}	
	...
	if(s->priv_info>buffer_validdata_size){
		...
		return -1;	
    } else if (s->priv_info < 0) {
        lp_print( AV_LOG_ERROR,"[%s:%d] LOOPBUFFER seek error, maybe cached hls data is not continuous !\n",__FUNCTION__,__LINE__);
        return -1;
    } else {
	 	...
	 	if(s->prot->url_exseek){
            ...
            if(strcmp(s->prot->name, "vhls")==0){							// hls can return the stream offset
				...
				ret=url_lpexseekoffset(s,offset);
	 			lp_lock(&lp->mutex);
	 			if(ret>=0){
					s->priv_info = AVSEEK_LOOPBUFFER_OFFSET;
	 				lp_print( AV_LOG_INFO,"[%s:%d] LOOPBUFFER seek deal successful\n",__FUNCTION__,__LINE__);
	 				goto seek_end;
	 			}
				s->priv_info = AVSEEK_TO_TIME;
