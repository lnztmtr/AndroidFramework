...
//*************************************************/	
	...
	*/
    int64_t jumppts;
	int64_t basepts;
	int64_t pts[RECALC_MAX_PROG];
	int64_t dts[RECALC_MAX_PROG];
	unsigned int  pts_nb[RECALC_MAX_PROG];
...
static void clear_avprograms(MpegTSContext *ts)
{
    AVProgram *prg = NULL;
    int i;

    for (i = 0; i < ts->stream->nb_programs; i++){
        if (ts->stream->programs[i]) {
            prg = ts->stream->programs[i];
            prg->nb_stream_indexes = 0;
        }
    }
...
{
    int i=0;
    /*when PAT table update,other table all new coming update*/
    for (i = 1; ts->nb_prg&&(i < NB_PID_MAX); i++) {
        if (ts->pids[i] &&ts->pids[i]->type == MPEGTS_SECTION) {
           av_log(NULL, AV_LOG_INFO,"use new table pid=0x%x\n",i);
                                     ...
                                     void *opaque)
{
    MpegTSFilter *filter;
    MpegTSPESFilter *pes;
    //av_log(NULL, AV_LOG_INFO,"mpegts_open_pes_filter Filter: pid=0x%x\n", pid);
    ...
    filter->pid = pid;
    filter->last_cc = -1;
    filter->encrypt_pktnum = 0;
    filter->encrypt_pktnum_max = am_getconfig_int_def("media.amplayer.encryptmax", 50);
...
{
    int pid;
    //av_log(NULL, AV_LOG_INFO,"mpegts_close_filter Filter: pid=0x%x\n", filter->pid);
    pid = filter->pid;
...
static const StreamType ISO_types[] = {
    ...
    { 0xd2, AVMEDIA_TYPE_VIDEO,      CODEC_ID_CAVS2 },
    { 0 },
};
...
static int mpegts_check_startcode(const uint8_t *buf, int buf_size)
{
	int i;
	uint8_t *p;
	int ret = 0;
                            ...
                            int64_t pos)
{
    ...
    if (is_start) {
		...
		//some times, ts pack data may be cut off in ts file, and the first ts packet is not a pes header, this will cause the following ts packets been discarded.
		//However, those discarded data may be contain some useful video data. In order to support this case, we have to check the discarded data, if we check
		//there is some start code for hevc in the bits flow, we set pes stata from skip to pay load, then these data will been push into es stream and delive to
		//video decoder.
		if(ts->stream->hevc_no_pes_header_support == 1) {
			if (ts->stream->pes_pid == pes->pid) {
				if(mpegts_check_startcode(buf, buf_size)) {
					av_log(pes->stream, AV_LOG_ERROR, "pid = 0x%x, total_size = 0x%x, state = %d, pes->data_index = %d, pes->st = 0x%x, pes->buffer = 0x%x\n",
						pes->pid, pes->total_size, pes->state, pes->data_index, pes->st, pes->buffer);

					if(pes->state == MPEGTS_SKIP) {
						...
						ts->stream->hevc_no_pes_header_support = 0;
                ...
                if (pes->header[0] == 0x00 && pes->header[1] == 0x00 &&
                    ...
                    if (!pes->st) {
                        ...
                        if (!pes->st)
                            return AVERROR(ENOMEM);
                        if (report_flag){
                            av_log(NULL,NULL, "stream not present in PMT\n");
                            ffmpeg_id_notify(PLAYER_EVENTS_ERROR, STREAM_DEMUX_ERROR, 0);
                        }
                        mpegts_set_stream_info(pes->st, pes, 0, 0);
                    }
                    ...
                    // avs error handle
                    if(pes->total_size <= 12) {
                        if(pes->st && pes->st->codec && pes->st->codec->codec_id == CODEC_ID_CAVS) {
                            av_log(NULL, NULL, "AVS pes total size less than 12.\n");
                            pes->total_size = MAX_PES_PAYLOAD;
                        }
                    }
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    av_log(ts->stream, AV_LOG_DEBUG1, "pmt sid=0x%x sec_num=%d/%d version=%d tid=%d\n",
    ...
    while(program_info_length >= 2) {
        ...
        if (tag == 0x1d) { // IOD descriptor
            ...
            len -= 4;
        } else if (tag == 0x09 && len >= 2 && ts->has_ca_desc == 0) {
            av_log(NULL, AV_LOG_INFO, "[pmt_cb] ca_system_id=%d\n", get16(&p, p_end));
            len -= 2;
            ts->has_ca_desc = 1;
        }
        p += len;
    ...
    for(;;) {
        ...
        if (!pes->stream_type){
            av_log(NULL,NULL,"function:%s,set streaminfo,pid:0x%x,index:%d\n",__FUNCTION__,pid,st->index);
...
static void pat_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    for(;;) {
        ...
        if (pmt_pid < 0){
            if (report_flag){
                av_log(NULL,NULL,"Invalid pmt_pid, pid wrong\n");
                ffmpeg_id_notify(PLAYER_EVENTS_ERROR, STREAM_DEMUX_ERROR, 0);
            }
            break;
        }

        av_log(ts->stream,AV_LOG_DEBUG1, "PAT sid=0x%x pid=0x%x\n", sid, pmt_pid);
...
static int recalcpts_startwhetherornot(AVFormatContext *s,AVPacket *pkt)
{
	...
	int i;
        ...
        /*
            make sure element in this array is not out of range
        */
        if (!(pkt->stream_index < RECALC_MAX_PROG && pkt->stream_index >= 0)){
            av_log(NULL, AV_LOG_INFO," recalcpts_startwhetherornot error stream_index:%d is out of range :%d \n",pkt->stream_index, RECALC_MAX_PROG);
            return -1;
        }
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    if ((packet[3] & 0xC0) && tss->encrypt == 0) {
        tss->encrypt_pktnum++;
        /*av_log(NULL, AV_LOG_WARNING, "pid=0x%x,encrypt_pktnum=%d\n",
            tss->pid, tss->encrypt_pktnum);*/
        if (tss->encrypt_pktnum > tss->encrypt_pktnum_max) {
            tss->encrypt=1;
            /*av_log(NULL, AV_LOG_WARNING, "encrypt pid=0x%x,max=%d\n",
                tss->pid, tss->encrypt_pktnum_max);*/
        }
    }
    ...
    if (!cc_ok) {
        ...
        av_log(ts->stream, AV_LOG_DEBUG,
               "Continuity check failed for pid %d expected %d got %d, discontinue_count:%d\n",
               pid, expected_cc, cc, s->ts_cc_discont);
        ...
        if (pc) {
            ...
            if (st && st->codec->codec_id == CODEC_ID_CAVS && st->need_check_avs_version == 1) {
                ...
                if (avs_version == 1) {
                    ...
                    #if (defined PROVINCE_TYPE_CM201_GUANGDONG || defined PROVINCE_TYPE_CM201_GUANGDONG_ZHUOYING)
                        /* +[SE] [BUG][BUG-173509][chuanqi.wang]:don't return,otherwise discard one pes packet*/
                        //return 0;
                        av_log(NULL, AV_LOG_ERROR, "leo guangdong qietai heiping\n");
                    #else
                        return 0;
                    #endif
...
static int mpegts_resync(AVFormatContext *s)
{
    ...
    for(i = 0;i < MAX_RESYNC_SIZE; i++) {
        ...
        // c == 0 means lpbuf empty, do some sleep
        if ((c == -1) && 1 != pb->local_playback){
            if(i%500==0){
                av_log(s, AV_LOG_ERROR, "mpegts_resync sleep 1ms,could not find sync byte\n");
            }
...
static int read_packet(AVFormatContext *s, uint8_t *buf, int raw_packet_size)
{
    ...
    for(;;) {
        ...
        total = 0;
RETRY:
        /*yinli.xia 20180109 local play exit immediately
            web play will retry*/
        ...
        len = avio_read(pb, buf+total, pkt_size);
        if (len <= 0 && ((len == AVERROR_EOF)
            ...
            if (len != AVERROR(EAGAIN)) {
                av_log(s, AV_LOG_ERROR, "avio read error, retry_cout = %d!, len = %d,seek:%d,%s\n", retry_count, len,total,errbuf);
	    }else{
                amthreadpool_thread_usleep(1000);
                avio_seek(pb, -total, SEEK_CUR);
                total = 0;
                goto RETRY;
            }
                              ...
                              AVFormatParameters *ap)
{
    ...
    int maxretry_time = am_getconfig_int_def("media.amplayer.headerretry",1000);
    int retry = 0;
    ...
    probesize = am_getconfig_int_def("libplayer.netts.probesize", 0);
    report_flag = am_getconfig_int_def("media.player.cmcc_report.enable", 0);
    av_log(NULL,AV_LOG_ERROR,"livingstream ts->pownon_recalcpts[%d]\n",ts->pownon_recalcpts);
    av_log(NULL,NULL,"mpegts report:%d\n", report_flag);
...
reget_packet_size:
    ...
    do {
        ...
        if (len == AVERROR(EAGAIN)) {
            retry++;
            if (retry > maxretry_time){
                av_log(s, AV_LOG_WARNING, "mpegts_read_header: timeout\n");
                return AVERROR(EAGAIN);
            }
        } else {
            retry = 0;
    ...
    if (s->iformat == &ff_mpegts_demuxer) {
        ...
        if(probesize&&1 != pb->local_playback){
            av_log(NULL,AV_LOG_INFO,"mpegts net stream use probesize:%d\n",probesize);
            handle_packets(ts, probesize / ts->raw_packet_size);
        } else {
            int default_size = 0;
            int probe_size = 0;
            if (s->probesize < 1024*1024) {
                default_size = 1;
                av_log(NULL,AV_LOG_INFO,"mpegts probesize too small use default probesize:1048576\n");
            }
            if (pb->fastdetectedinfo || pb->is_slowmedia || default_size) {
                probe_size = 1024*1024;
            } else {
                probe_size = s->probesize;
            }
            //find PMT, would stop parse
            if (1 == pb->local_playback) {
                probe_size = 6 * 1024 * 1024;
            }

            av_log(NULL,AV_LOG_INFO,"[%s],probe_size=%d, s->probesize=%d, fastdetected=%d, is_slowmedia=%d, local_play=%d\n",
                __FUNCTION__, probe_size, s->probesize, pb->fastdetectedinfo, pb->is_slowmedia, pb->local_playback);
                              ...
                              AVPacket *pkt)
{
    ...
    if (avio_tell(s->pb) != ts->last_pos) {
        ...
        av_log(NULL, AV_LOG_ERROR, "free %d pes packet buffer\n", i);
    }
    ...
    if (ret < 0) {
        ...
        for (i = 0; i < NB_PID_MAX; i++) {		
            for (j=0;j<s->nb_streams;j++){
                if(s->streams[j]->encrypt == 0
                    && ((ts->pids[i] &&s->streams[j]->id==ts->pids[i]->pid&&ts->pids[i]->encrypt==1)
                        || (ts->has_ca_desc == 1 && s->streams[j]->codec
                            && s->streams[j]->codec->codec_type == AVMEDIA_TYPE_VIDEO))){
                    s->streams[j]->encrypt = 1;
                    av_log(NULL, AV_LOG_ERROR, "mpegts find encrypt stream pid 0x%x\n",s->streams[j]->id);
                }
            }
            if (ts->pids[i] && ts->pids[i]->type == MPEGTS_PES) {
                PESContext *pes = ts->pids[i]->u.pes_filter.opaque;
                              ...
                              AVPacket *pkt)
{
    ...
    if (ts->is_hevc == 1 && ts->hevc_first_packet == 1 && pkt->stream_index == ts->hevc_stream_index) {
        ...
        ret = get_hevc_csd_packet(s, st, pkt);
        ts->hevc_first_packet = 0;

		if (ret == -1)
			ts->stream->hevc_no_csd_data = 1;
    }
...
static int read_seek(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    ...
    int64_t pos;
	int ret;
        /*use dts seek, don't need del pts and pcr diffs */
        if (0)
...
static int mpegts_read_seek(AVFormatContext *s, int stream_index, int64_t target_ts, int flags){
    ...
    // we would adopt read_seek to keyframe.
