                          ...
                          unsigned int offset, unsigned int max_probe_size)
{
    ...
    int maxretry = 0;
    int maxretry_time = am_getconfig_int_def("media.amplayer.probetry",1000);
    int eof_flag = 0;
...
int ffmpeg_register_id_notify(const player_notify_t notify_fn, int playerid)
{
    if (notify_fn){
        player_notify = notify_fn;
        player_id = playerid;
        av_log(NULL,NULL,"function:%s, player_id=%d\n",__FUNCTION__, player_id);
    }

    return 0;
}

int ffmpeg_id_notify(int msg, unsigned long ext1, unsigned long ext2)
{
   if (player_notify){
     player_notify(player_id, msg, ext1, ext2);
   }
   return 0;
}
...
//PD#140725 int verify(const char*url, int urlLen, const char *ip, int ipLen);
static int url_verfiy(const char *url)
{
    int ret = AVERROR_EXIT;
    const char *ex_lib = "libtermplug.so";
    const char *ex_libevent = "libevent.so";
    int fd = -1,fd_ev =-1;
    int port = 0;
    char hostname[1024],proto[1024],path[1024];
    char str[128] = {0};
    char ip[128] = {0};
    int urlLen=0,ipLen=0;
    int (*lib_verify)(const char*url, int urlLen, const char *ip, int ipLen);
    int (*getaddrinfo_async_cancel)(char* ptr,char* ip);
    fd = dlopen(ex_lib, RTLD_NOW);
    if (fd != 0) {
        lib_verify = dlsym(fd, "verify");
    } else {
         av_log(NULL, AV_LOG_ERROR,"cant find lib %s\n",ex_lib);
        return -1;
    }
    fd_ev = dlopen(ex_libevent, RTLD_NOW);
    if (fd_ev != 0) {
        getaddrinfo_async_cancel = dlsym(fd_ev, "getaddrinfo_async_cancel");
        if(!getaddrinfo_async_cancel)
            av_log(NULL, AV_LOG_ERROR,"cant find getaddrinfo_async_cancel\n");
    } else {
        av_log(NULL, AV_LOG_ERROR,"cant find lib %s\n",ex_libevent);
        return -1;
    }
    ...
    av_log(NULL, AV_LOG_INFO,"hostname:%s will get address from dns!\n",hostname);
    if(getaddrinfo_async_cancel)
        ret = getaddrinfo_async_cancel(hostname, ip);
    if(ret){
        av_log(NULL, AV_LOG_INFO,"Failed to resolve hostname:%s  from dns,ret:%d\n",hostname,ret);
        ret = AVERROR_EXIT;
        goto erro;
    }
    av_log(NULL, AV_LOG_INFO,"hostname:%s -> %s from dns,ret:%d\n",hostname,ip,ret);
    urlLen = strlen(url);
    ipLen = strlen(ip);
    av_log(NULL, AV_LOG_INFO,"url_verfiy url:%s urlLen:%d,ip:%s,ipLen:%d \n",url,urlLen,ip,ipLen);
    ret = lib_verify(url, urlLen, ip, ipLen);
 erro:
    ...
    if(fd_ev)
        dlclose(fd_ev);
     av_log(NULL, AV_LOG_INFO,"[url_verfiy %d] ret:%d \n",__LINE__,ret);
    if (ret != 0) {
        ret = AVERROR_EXIT;
    }
    return ret;
...

{
...
#endif
    AVProbeData pd = {.filename = filename, .buf=NULL, .buf_size=0, .s=NULL, .options=options};
    ...
    if (strstr(filename, "shttp") != NULL&&am_getconfig_bool_def("media.amplayer.verify", 0)) // htp protocol
    {
        int ret = 0;
        char* url = strstr(filename, "http");
        ret = url_verfiy(url);
        if(ret != 0){
            av_log(NULL, AV_LOG_INFO, "[%s:%d]url_verfiy error\n", __FUNCTION__, __LINE__);
            return ret;
        }
        av_log(NULL, AV_LOG_INFO, "[%s:%d]url_verfiy ret:%d\n", __FUNCTION__, __LINE__, ret);
    }
    ...
    else
    {
            ...
            if(iptv_v5_support == 0) {
                ...
                if(ret) {
                    ...
                    if(ret==-ERROR_URL_NOT_M3U8){
                         ...
                         if (options && *options)
                            ret = avio_open_h2(&s->pb, listfile, flags, headers, options);
                        else
        ...
        else
        {
            ...
            if (av_strstart(filename, "udp:", NULL)){
                flags |= URL_NO_LP_BUFFER;
                av_log(NULL,NULL,"udp set flags no lpbuffer\n");
            }
            av_log(NULL,NULL,"--1--%s---\n",__FUNCTION__);
            if (options && *options)
                ret = avio_open_h2(&s->pb, filename, flags, headers, options);
            else
                ret = avio_open_h(&s->pb, filename, flags, headers);
            if (ret < 0)
                return ret;
            if (!strncmp(filename, "udp:", strlen("udp:")) || !strncmp(filename, "rtp:", strlen("rtp:")) || !strncmp(filename, "rtsp:", strlen("rtsp:"))) //no need to try in new protocol matching
           {
                goto PASS_THROUGH;
            }
        }
...
retry_open:
	...
	if (s->pb && s->pb->is_slowmedia) {
		av_log(NULL, NULL, "Set less buff data flag\n");
		url_set_seek_flags(s->pb, LESS_BUFF_DATA);
	}
...
got_packet:
                    ...
                    if (!strcmp(s->iformat->name, "mpegts") &&
                        ...
                        */
                    {
                        /*if dts is overflow should use compute dts,avoid seek use error max dts PD#147600*/
                        if (!(old_pts != AV_NOPTS_VALUE && old_dts != AV_NOPTS_VALUE &&
                            st->pts_wrap_bits < 63 &&
                            old_dts - (1LL << (st->pts_wrap_bits - 1)) > old_pts)) {
                            pkt->pts = old_pts;
                            pkt->dts = old_dts;
                        }
                    }
...
int64_t av_gen_search(AVFormatContext *s, int stream_index, int64_t target_ts, int64_t pos_min, int64_t pos_max, int64_t pos_limit, int64_t ts_min, int64_t ts_max, int flags, int64_t *ts_ret, int64_t (*read_timestamp)(struct AVFormatContext *, int , int64_t *, int64_t )){
    ...
    av_log(NULL, NULL, "[%s]: ---------------------------------\n", __func__);
    av_log(NULL, NULL, "stream_index:%d target_ts: %"PRId64" \n",stream_index, target_ts);
    av_log(NULL, NULL, "pos_min:%"PRId64" ts_min: %"PRId64" \n",pos_min, ts_min);
    av_log(NULL, NULL, "pox_max:%"PRId64" ts_max: %"PRId64" \n",pos_max, ts_max);
    av_log(NULL, NULL, "pos_limit: %"PRId64" data_offset:%d \n", pos_limit, s->data_offset);
    av_log(NULL, NULL, "[%s]: ---------------------------------\n", __func__);
...
RETRY_MIN:
    if(ts_min == AV_NOPTS_VALUE){
    	...
    	av_log(NULL, NULL, "estimate pos_min:%"PRId64" ts_min: %"PRId64" \n",pos_min, ts_min);
    }
...
RETRY_MAX:
    if(ts_max == AV_NOPTS_VALUE || ts_max < target_ts  ){
        ...
        do{
            ...
            step += step;
            if (ts_max == AV_NOPTS_VALUE && (s->pb != NULL && s->pb->eof_reached == 1) && !strcmp(s->iformat->name, "asf")) {
        ...
        pos_limit= pos_max;
    	av_log(NULL, NULL, "Estimate pox_max:%"PRId64" ts_max: %"PRId64" \n",pos_max, ts_max);
    }
        ...
        if(seek_finish_range != -1) {
	       ...
	       int match_type = -1;  // 0 max 1 min
               if(llabs(target_ts - ts_max) < seek_finish_range){
                       av_log(NULL, NULL, "No need to seek, equal to ts_max \n");
                       match_ts = ts_max;
                       match_pos = pos_max;
                       match_flag = 1;
		       match_type = 0;
                       ...
                       av_log(NULL, NULL, "No need to seek, equal to ts_min.\n");
                       match_ts = ts_min;
                       match_pos = pos_min;
                       match_flag = 1;
		       match_type = 1;
               ...
               if(match_flag)
               {
                       ...
                       if(llabs(target_ts - ts) < seek_finish_range){
                               *ts_ret = ts;
                               av_log(NULL, NULL, "ts match, %lld %lld %lld \n", ts_min, ts_max, ts);  
                               return match_pos;
                       }
                       av_log(NULL, NULL, "ts not match, %lld %lld %lld \n", ts_min, ts_max, ts);
		       if(match_type == 0)
		       {
	               	       ts_max = AV_NOPTS_VALUE;
		       	       goto RETRY_MAX; 
...
static int find_last_valid_offset(AVFormatContext *s)
{
    int ret = -1;
    AVPacket pkt;
    AVStream *st;
    int64_t offset = 0;
    int64_t start = 0;
    int64_t end = s->file_size;
    int ts_pkt_size = TS_PACKET_SIZE;

    if (!strncmp(s->filename, "bluray:", strlen("bluray:")))
        ts_pkt_size = TS_DVHS_PACKET_SIZE;

    for (;;) {
        offset = start + (end - start >> 1);

    if (offset < ts_pkt_size)
            offset = 0;

        offset -= (offset % ts_pkt_size);

    /*av_log(NULL, AV_LOG_INFO,
        "[%s:%d] entry:  start: 0x%llx, end: 0x%llx, offset: 0x%llx.\n",
        __FUNCTION__, __LINE__, start, end, offset);*/

        avio_seek(s->pb, offset, SEEK_SET);

        do {
            ret = av_read_packet(s, &pkt);
        } while (ret == AVERROR(EAGAIN));

    /*av_log(NULL, AV_LOG_INFO,
        "[%s:%d] packet, size: 0x%x, pts: 0x%llx, stream_index: 0x%x\n",
        __FUNCTION__, __LINE__, pkt.size, pkt.pts, pkt.stream_index);*/

        if (ret != 0) {
            end = offset - ts_pkt_size;
        if ((start + ts_pkt_size) == end) {
                av_log(NULL, AV_LOG_INFO,
                    "[%s:%d] No found anything.\n",
                    __FUNCTION__, __LINE__);
                 break;
            }
        } else {
            start = offset;
            if ((start + ts_pkt_size) == end) {
                s->file_size = offset * ts_pkt_size;
                s->valid_offset = offset;
                s->valid_offset_done = 1;

				av_log(NULL, AV_LOG_INFO,
                    "[%s:%d] The last packet was found, offset : 0x%llx.\n",
                    __FUNCTION__, __LINE__, offset);
                 break;
            }
        }
...
static void av_estimate_timings_from_pts(AVFormatContext *ic, int64_t old_offset)
{
    ...
    av_log(NULL, AV_LOG_WARNING, "Estimate Size:%lld (Byte)\n", estimate_size);
    /* flush packet queue */
    flush_packet_queue(ic);
    for (i = 0; i < ic->nb_streams; i++)
    {
    ...
    do
    {
        ...
        offset1 = avio_seek(ic->pb, offset, SEEK_SET);
        if(abs(offset1-offset) > 1024*1024) { //if seek fail continue
            av_log(NULL,AV_LOG_INFO, "[%s:%d]seek fail retry \n", __FUNCTION__, __LINE__);
            continue;
        }
        read_size = 0;
        for (;;)
        {
                     ...
                     st->first_dts  != AV_NOPTS_VALUE))
            {
                ...
                if (st->start_time != AV_NOPTS_VALUE)  {
                    //avoid duration little than start time
                    if (duration > st->start_time) {
                        duration -= st->start_time;
                    } else {
                        int64_t temp_time = duration;
                        duration = st->start_time - temp_time;
                        //st->start_time = temp_time;
                    }
                }
...
static int av_estimate_timings(AVFormatContext *ic, int64_t old_offset)
{
    ...
    else
    {
        if (!strcmp(ic->iformat->name, "mpegts"))
            ret = find_last_valid_offset(ic);
        if (ret < 0) {
            av_log(NULL, AV_LOG_ERROR, "[%s]error, return\n", __FUNCTION__);
            return ret;
        }
    }
    if ((valid_offset > 2) && (ic->valid_offset != 0x7fffffffffffffff) && !ic->valid_offset_done)
    {
    ...
    if(tmp) {
        av_log(NULL, NULL, "Skip calc duration \n");
        return ret;
    }
    ...
    if (!fastmode)
    {
        switch (enc->codec_type)
        {
            case AVMEDIA_TYPE_AUDIO:
                ...
                break;
            case AVMEDIA_TYPE_VIDEO:
                if (enc->codec_id == CODEC_ID_CAVS2)
                    val = enc->width;
                else
                    val = enc->width && enc->pix_fmt != PIX_FMT_NONE;
                break;
        ...
        switch (enc->codec_type)
        {
            case AVMEDIA_TYPE_AUDIO:
                if (fastmode &&
                        ...
                        (enc->codec_id == CODEC_ID_AAC_LATM))
                {
...
int av_rfps_add_frame(AVFormatContext *ic, AVStream *st, int64_t ts)
{
    int i, j;
...
void av_rfps_calculate(AVFormatContext *ic)
{
    int i, j;
    for (i = 0; i < ic->nb_streams; i++)
    {
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO)
        {
            ...
            /* +[SE] [BUG][BUG-170582][yinli.xia] added: modify to get avs2 video fps*/
...
#define DEFAULT_PROBE_SIZE 6500000
    ...
    if (fast_switch)
    {
     ...
     /* +[SE] [BUG][BUG-171393][wenlong.yuan]:set the probesize 5M of wmv for http playing*/
    if (!local_playback
        && ic->filename
        && strstr(ic->filename, "http")
        && strstr(ic->filename, ".wmv"))
    {
        http_playback = 1;
        file_suffix_format_wmv = 1;
        ic->probesize = 1024*1024*5;
    }
    ...
    if (!strcmp(ic->iformat->name, "mpegts")
        ...
        && ic->pb->local_playback != 1) {
        ic->probesize = 1024*256;
        fast_switch = 0;
    }

	if (strstr(ic->filename, "http://") != NULL && url_is_segment_media(ic->pb) == 0 && strcmp(ic->iformat->name, "mpegts") == 0) 
	{
	    fast_switch = 0;
		av_log(NULL, AV_LOG_INFO, "http vod fast_switch = %d\n",fast_switch);
	}
    ...
    av_log(NULL, AV_LOG_INFO, "[%s:%d]fast_switch=%d, seekkeyframe=%x,nb_stream:%d\n", __FUNCTION__, __LINE__, fast_switch, (am_getconfig_bool("media.amplayer.seekkeyframe")),ic->nb_streams);
    for (i = 0; i < ic->nb_streams; i++)
    {
        AVCodec *codec;
        st = ic->streams[i];
    ...
    for (;;)
    {
                ...
                for(j=0; j< ic->nb_streams; j++){
                    ...
                    if(((ic->nb_streams == 1)&&(st->codec->codec_type == AVMEDIA_TYPE_VIDEO))||
                        (need_continue_parse>=2)){
                            av_log(NULL,NULL,"probe_stop 1, find stream info end\n");
                            probe_stop = 1;
                    }
                }
            }
        ...
        {
               if (av_strstart(ic->filename, "rtp://",NULL) || av_strstart(ic->filename, "rtpcc://",NULL) || av_strstart(ic->filename, "udp://",NULL)) {
                ret = count;
		   ...
		   fast_switch,ic->probesize,read_size,ic->nb_streams);
                ...
                for(k = 0; k < ic->nb_streams; k++)
                {
                    ...
                    {
                        if (-1 == force_audio_valid_idx)
                        {
                            found_one_audio_param = 1;
                            force_audio_valid_idx = k;
                            av_log(NULL, AV_LOG_INFO, "[%s:%d], force_audio_valid_idx:%d! \n", __FUNCTION__, __LINE__, force_audio_valid_idx);
                        }
                    }
                ...
                av_log(ic, AV_LOG_DEBUG,"found_one_audio_param:%d,found_one_video_param:%d\n",
                        found_one_audio_param, found_one_video_param);
                if(found_one_audio_param == 0 || found_one_video_param == 0) {
                    ...
                    if(ic->file_size > 0 && ic->file_size < ic->probesize)
                        break;
                    av_log(NULL, NULL, "No param found. promote probesize. probesize:%u \n", ic->probesize);
        ...
        if (st->codec->codec_type == AVMEDIA_TYPE_VIDEO) // just video
          av_rfps_add_frame(ic, st, pkt->dts);
           ...
           !ic->pb->is_slowmedia)
        {
            if (st->codec_info_nb_frames <= 1) {
                ...
                // set the first dts value for discontinue checking,PD#105693
                if (st->codec_info_nb_frames==0){
                    st->info->fps_first_dts = pkt->dts;
                } else {
                    //Check for a discontinuity in dts for second frame,PD#144788
                    if(av_rescale_q(abs(pkt->dts - st->info->fps_first_dts), st->time_base,
                                 AV_TIME_BASE_Q) >1000000) {
                    st->info->fps_first_dts = pkt->dts;
                    st->start_time = pkt->pts;
                        av_log(ic, AV_LOG_WARNING,
                            "second frame DTS discontinuity in stream %d: packet %d with DTS "
        ...
        /* +[SE] [BUG][BUG-171393][wenlong.yuan]: when get the start_time,break*/
        if (!local_playback
            && ic->filename
            && http_playback
            && file_suffix_format_wmv
            && st->start_time != AV_NOPTS_VALUE)
        {
            break;
        }
        ...
        if ((!has_codec_parameters_ex(st->codec, fast_switch) || (!fast_switch && !has_decode_delay_been_guessed(st) && !has_codec_parameters_ex(st->codec, 0))))
        {
    ...
    for (i = 0; i < ic->nb_streams; i++)
    {
                ...
                && (st->codec->codec_type == AVMEDIA_TYPE_AUDIO))
        {
            if (st->codec->codec_id != CODEC_ID_DTS)
            {
                ...
                st->stream_valid = 0;
               if (!strcmp(ic->iformat->name, "rtp") || i == force_audio_valid_idx) {
                   av_log(NULL, AV_LOG_INFO, "[%s:%d] rtp %d ,force_audio_valid_idx:%d,is valid \n", __FUNCTION__, __LINE__, i,force_audio_valid_idx);
                   st->stream_valid = 1;
               }
            }
        }
...
find_stream_info_err:
    ...
    if (ic->pb)
        av_log(ic, AV_LOG_DEBUG, "After avformat_find_stream_info() pos: %"PRId64" frames:%d\n",
               avio_tell(ic->pb), count);
    return ret;
}
...
void av_close_input_file(AVFormatContext *s)
{
	...
	if(strncmp(s->iformat->name,"mhls",4)==0){
        if(s->pb){
            av_log(NULL,NULL,"mhls free s->pb=0x%x\n",s->pb);
            pb = s->pb;
        }
    }
