...
static int gd_report_timeout = 30;
extern int m_bconnectfail;
...
static int init_def_settings()
{
	...
	open_wait_time = (int)am_getconfig_float_def("media.player.openwaittime", 0);
        ...
        gd_report_timeout = am_getconfig_int_def("media.player.read_report.timeout",30);
...
static int http_open_cnx(URLContext *h)
{
    ...
    int curr_wait;
    int opencheck= (int)am_getconfig_bool_def("libplayer.amplayer.netcheck", 0);
 ...
 redo:
    ...
    if ((ret=http_connect(h, path, hoststr, auth, &location_changed) ) < 0){
        ...
        /* +[SE] [BUG][IPTV-7972][yinli.xia] added:
            report 54009 code to service when 404/502 error for nanchuan*/
        if (opencheck && (s->http_code == 404 || s->http_code == 502)) {
            av_log(h, AV_LOG_ERROR, "==leo http_open_cnx:http_connect failed opencheck=%d\n",opencheck);
            ffmpeg_notify(h, MEDIA_INFO_DOWNLOAD_ERROR, 54009, 0);
			goto fail;
        }
...
static int http_getc(HTTPContext *s)
{
	...
	int64_t timeout_set = 1000*10000;

    if (am_getconfig_float("http.getc.timeout_us", &timeout_set) == 0) {
        timeout_us = timeout_set;
    }
                        ...
                        const char *auth, int *new_location)
{
    ...
    if (!has_header(s->headers, "\r\nAccept: "))
        len += av_strlcpy(headers + len, "Accept: */*\r\n",
                          sizeof(headers) - len);	
    av_log(NULL,NULL, "debug s->off = %lld\n",s->off);
    if (!has_header(s->headers, "\r\nRange: ") && (s->off>=0)
        &&!has_header(headers, "\r\nRange: ")){/*&&!h->is_segment_media&&!s->hd->is_streamed*/
        len += av_strlcatf(headers + len, sizeof(headers) - len,
                           "Range: bytes=%"PRId64"-\r\n", s->off);
        av_log(NULL,NULL,"Add Range Headers\n");
    }else
        av_log(NULL,NULL,"not add Range Header\n");
...
static int http_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    if(s->http_code == 404 && s->filesize<=0){
        av_log(h, AV_LOG_INFO, "0822 debug return http_read error!");
        return AVERROR(EIO);;
    }
...
retry:
    ...
    if (s->chunksize >= 0) {
        if (!s->chunksize) {
            ...
            for(;;) {
                ...
                av_log(h, NULL, "Chunked encoding data size: %"PRId64"'\n", s->chunksize);
    ...
    if (len > 0) {
        ...
        //in chunked mode, s->filesize = 1, when len little than chunksize, had some data not read
        if (s->filesize > 0 && s->off >= s->filesize && s->chunksize <= 0){
            av_log(h, AV_LOG_ERROR, "http_read eof len=%d, s->chunksize=%lld\n", len, s->chunksize);
	      bandwidth_measure_finish_read(s->bandwidth_measure,0);	
            return 0;
        }
        if(s->hd){
    ...
    if(len==AVERROR(EAGAIN)){
		...
		av_log(h, AV_LOG_INFO, "new_time_mseconds=%u,latest_get_time_ms=%u diff=%u max_wait_time=%u,s->off=%lld,s->filesize=%lld \n", 
			new_time_mseconds,s->latest_get_time_ms,(new_time_mseconds-s->latest_get_time_ms),max_wait_time,s->off, s->filesize);
                if(gd_report_error_enable&&(new_time_mseconds-s->latest_get_time_ms>gd_report_timeout*1000)&&!s->report_flag){
                    s->report_flag = 1;
...
static int64_t http_seek(URLContext *h, int64_t off, int whence)
{
    ...
    if (whence == AVSEEK_SIZE)
        /*[SE][BUG][IPTV-5030][houren.wang] http should return filezie, instead of content_length*/
        return s->filesize;
