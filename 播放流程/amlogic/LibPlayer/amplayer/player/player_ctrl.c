...
#define PLAYER_AV_AM_INFO_PATH "/sys/class/avinfo/am_av_info"
#define AM_PLAYER_INFO "get_num:0,checkin_vpts_count:0,checkout_vpts_count: 0,vbuf_len:0,\
vbuf_size:0,abuf_len:0,abuf_size:0,first_pic_coming:0,vformat_type:NULL,\
frame_format:NULL,width:0,height:0,fps:0,current_fps:0,first_vpts:0,vpts:0,\
vpts_err_num:0,toggle_frame_count:0,dec_error_count:0,dec_err_frame_count:0,\
dec_frame_count:10471,dec_drop_frame_count:0,ts_error_num:0,tsync_mode:0,apts:0,\
apts_err_num:0,"
...
int player_stop(int pid)
{
    ...
    fd2 = open(PLAYER_AV_AM_INFO_PATH,O_RDWR,0666);
	if (fd2 >= 0) {
        write(fd2,AM_PLAYER_INFO,strlen(AM_PLAYER_INFO));
        close(fd2);
    }
 ...
 */
/* --------------------------------------------------------------------------*/
int player_get_extern_priv(int pid)
{
    long externed = 0;
...
char* player_aformat2str(aformat_t value)
{
    switch (value) {
    case AFORMAT_MPEG:
        return "AFORMAT_MPEG";
...
/* --------------------------------------------------------------------------*/
/**
 * @function    player_set_speed
 *
 * @brief       set play speed from amplayer parament
 *
 * @param[in]   pid; player tag which get from player_start return value
 *
...
int player_set_speed(int pid, float  speed)
{
    int ret = -1;
    play_para_t * player_para;
    if (speed < 0.5 || speed > 2) {
        log_print("[%s:%d] speed is error :%f!", __FUNCTION__, __LINE__,speed);
        return 0;
    }
    player_para = player_open_pid_data(pid);
    if (player_para == NULL) {
    ...
    play_para_t * vcodec = NULL;
    play_para_t * acodec = NULL;
    acodec = get_audio_codec(player_para);
    if (acodec != NULL) {
        log_print("[%s:%d] codec_set_audio_speed\n!", __FUNCTION__, __LINE__);
        codec_set_audio_speed(acodec, speed);
    } else {
        log_print("[%s:%d] acodec is null\n!", __FUNCTION__, __LINE__);
        player_close_pid_data(pid);
        return -1;
    }
    vcodec = get_video_codec(player_para);
    if (vcodec != NULL) {
        log_print("[%s:%d] codec_set_video_speed\n!", __FUNCTION__, __LINE__);
        codec_set_video_speed(vcodec, speed);
    } else {
        log_print("[%s:%d] vcodec is null\n!", __FUNCTION__, __LINE__);
        player_close_pid_data(pid);
        return -1;
    }
    player_close_pid_data(pid);
