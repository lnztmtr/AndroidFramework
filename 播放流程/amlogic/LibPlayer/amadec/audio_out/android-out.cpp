...
static float DbToAmpl(float decibels)
{
    if (decibels <= -758) {
        return 0.0f;
    }
...
void apply_volume(float volume, void *buf, int bytes) {
...
#ifdef RESAMPLE_ENALBE
static int get_playback_rate (double * rate, aml_audio_dec_t* audec)
{
    char tmp[128]={0};
    int ret = 0 ;
    if (property_get("media.amplayer.playrate", tmp, NULL) > 0) {
        *rate = (double)(atof(tmp));
        //adec_print("media.amplayer.playrate:%s\n", tmp);
    } else if (audec->audio_speed != 0 ) {
         *rate = audec->audio_speed;
    } else
        return 0;
    ...
    {
        ret = 1;
        //adec_print("playback audio resample -> *rate %f\n", *rate);
    } else {
        adec_print("playback audio resample speed err,%f\n", *rate);
        ret = 0;
    }
    return ret;
}
...
void audioCallback(int event, void* user, void *info)
{
   ...
   //    adec_print("audioCallback start: request %d, in: %d, out: %d, diff: %d, filtered: %d",buffer->size, last_checkin/90, last_checkout/90, diff, diff_avr);
    ...
    if (audec->adsp_ops.dsp_on) {
        ...
        double rate = 0;
        //adec_print("11size:%d\n\n",buffer->size);
        if (get_playback_rate(&rate, audec)) {
             af_resample_api_new((char*)(buffer->i16), (unsigned int*)&buffer->size, audec->channels, audec,1,rate);
        } else if (wfd_enable) {
        ...
        /*agc params dectect */
        if (property_get("media.amlagc.disable", tmp, "true") > 0 &&
            (!strcmp(tmp, "1") || !strcmp(tmp, "true"))) {
            //aml_agc_enable(0);
            audec->has_agc = 0;
            //adec_print("media.amlagc.disabled");
        } else {
           audec->has_agc = 1;
           if (property_get("media.amlagc.peak_level", tmp, NULL) > 0) {
                peak_level = atof(tmp);
            } else {
                peak_level = audec->peak_level;
            }
           if (property_get("media.amlagc.dynamic_theshold", tmp, NULL) > 0) {
                dynamic_theshold = atof(tmp);
            } else {
                dynamic_theshold = audec->dynamic_theshold;
            }
            if (property_get("media.amlagc.noise_threshold", tmp, NULL) > 0) {
                noise_threshold = atof(tmp);
            } else {
                noise_threshold = audec->noise_threshold ;
            }
            if (property_get("media.amlagc.response_time", tmp, NULL) > 0) {
                response_time = atoi(tmp);
            } else {
                response_time = audec->response_time;
            }
            if (property_get("media.amlagc.gain_value", tmp, NULL) > 0) {
                gain_value = atoi(tmp);
                if (gain_value != audec->gain_value) {
                    audec->gain_value = gain_value;
                    audec->vol_agc = DbToAmpl(audec->gain_value);
                }
            } else {
                gain_value = audec->gain_value;
            }
            if (peak_level != audec->peak_level || dynamic_theshold != audec->dynamic_theshold ||
                noise_threshold !=  audec->noise_threshold || response_time != audec->response_time) {
                audec->peak_level = peak_level;
                audec->dynamic_theshold = dynamic_theshold;
                audec->noise_threshold = noise_threshold;
                audec->response_time = response_time;
                audec->gain_value = gain_value;
                int ret = aml_agc_init(audec->peak_level,audec->dynamic_theshold,audec->noise_threshold,audec->response_time);
                if (ret == 0) {
                     aml_agc_enable(1);
                    adec_print("%s, init agc success!\n", __FUNCTION__);
                } else {
                    adec_print("%s, init agc fail!\n", __FUNCTION__);
                    audec->has_agc = 0;
                }
           }
       }
       /* do agc process */
       //adec_print("audec->has_agc;%d audec->channels:%d",audec->has_agc,audec->channels);
       if (audec->has_agc && audec->channels == 2) {
           aml_agc_process((void *)(buffer->i16), buffer->size >> 1);
           apply_volume(audec->vol_agc ,buffer->i16,buffer->size);
       }
    } else {
...
extern "C" int android_init(struct aml_audio_dec* audec)
{
   ...
   {		
    ...
    //default agc config params
    //warning --- if dolby or dts drc cerfitcation ,this moudle should be closed
    audec->peak_level = -32;
    audec->dynamic_theshold = -32;
    audec->noise_threshold = -60;
    audec->response_time = 512;
    audec->gain_value = 5;
    audec->vol_agc = DbToAmpl(audec->gain_value);
    if (property_get("media.amlagc.disable", tmp, "true") > 0 &&
        (!strcmp(tmp, "1") || !strcmp(tmp, "true"))) {
            audec->has_agc = 0;
            adec_print("Amlogic AGC disabled");
    } else {
        int ret = aml_agc_init(audec->peak_level,audec->dynamic_theshold,audec->noise_threshold,audec->response_time);
        if (ret == 0) {
            audec->has_agc = 1;
            aml_agc_enable(1);
            adec_print("%s:%d, init agc success!\n", __FUNCTION__,__LINE__);
        } else {
            adec_print("%s, init agc fail!\n", __FUNCTION__);
            audec->has_agc = 0;
            adec_print("Amlogic AGC disabled");
...
extern "C" int android_set_volume(struct aml_audio_dec* audec, float vol)
{
    ...
    char tmp[128]={0};
    int ctcplayer_enable = 0;
    if (property_get("media.ctcplayer.enable", tmp, "0") > 0 &&
        (!strcmp(tmp, "1") || !strcmp(tmp, "true")))
        ctcplayer_enable = 1;
    //for mutiplayer amadec switch to omx and omx the switch to omx,passthrough mute issue
    if (ctcplayer_enable && vol == 1.0 && out_ops->audio_out_raw_enable) {
