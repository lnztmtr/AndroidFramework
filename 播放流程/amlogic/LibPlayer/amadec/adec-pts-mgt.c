...
#define DROP_PCM_MAX_TIME 10000 // unit :ms
#define DROP_PCM_PTS_DIFF_THRESHHOLD   (90000*12)
...
int adec_switch_pts(aml_audio_dec_t *audec, unsigned long firstapts)
{	
    unsigned long first_vpts = 0;
	unsigned long first_apts = 0;
	unsigned long cur_pcr = 0;
	unsigned long cur_vpts = 0;
	unsigned long diff = 0;
	int wait_count = 0;
	first_apts = firstapts;
	do{
		 sysfs_get_int(TSYNC_FIRSTVPTS, &first_vpts);
		 amthreadpool_thread_usleep(10000);
		 wait_count++;
	}while(first_vpts == 0 && wait_count < 200 );//max  wait 2s++	
	
	wait_count = 0;
	
	diff = (first_apts > first_vpts) ? (first_apts - first_vpts) : 0;
	
	adec_print("[%s::%d] first_apts 0x%x, first_vpts 0x%x diff:0x%x\n", __FUNCTION__, __LINE__, first_apts, first_vpts, diff);
	
	if((diff/90) > 100){
		adec_print("--- set vpts to pcr ---\n");
	    if(amsysfs_set_sysfs_str(TSYNC_EVENT, "AUDIO_WAIT") == -1){
		    return -1;
	    }
	    do{
		   amthreadpool_thread_usleep(10000);
	    ...
	    }while(cur_pcr < first_apts && wait_count < 300);//max wait 3s+		
	}
	return 0;
}
 ...
 */
long adec_calc_pts(aml_audio_dec_t *audec)
{
    unsigned long delay_pts = 0;
	long pts = 0;
    audio_out_operations_t *out_ops = NULL;
    dsp_operations_t *dsp_ops = NULL;
...
int adec_pts_start(aml_audio_dec_t *audec)
{
	...
	if (audec->format == ACODEC_FMT_RAAC || audec->format == ACODEC_FMT_WMAPRO)
	    adec_switch_pts(audec, first_apts);
    ...
    if (enable_drop_pcm) {
        ...
        if (amsysfs_get_sysfs_str(path, buf1, 32) >= 0) {
            if (!strncmp(buf1, "1: enabled", 10)) {
	...
	/* best not cache exceed 300ms */
	//add by amlogic 20181219 to solve biaoqing kadun
    int cache_level = 0;
    if (property_get("libplayer.amadec.cache", value, NULL) > 0) {
        cache_level = atoi(value);
    }
    unsigned long firstvpts, curvpts;
    int retry = am_getconfig_int_def("libplayer.amadec.cache_retry", 300);
    sysfs_get_int(TSYNC_FIRSTVPTS, &firstvpts);
	cache_level = am_getconfig_int_def("libplayer.amadec.cache_size", 300);
    while (!audec->need_stop && audec->has_video && retry-- > 0) {
        sysfs_get_int("/sys/class/tsync/last_checkout_vpts", &curvpts);
        if ((curvpts - firstvpts)/90 > cache_level)
            break;
            usleep(1000);
    }
    adec_print("first:%x last:%x cache:%dms - %dms retry:%d\n", firstvpts, curvpts, (curvpts-firstvpts)/90, cache_level, retry);
   //end by amlogic 20181219
...
int adec_pts_droppcm(aml_audio_dec_t *audec)
{
    ...
    while (!audec->first_apts) {
        ...
        count++;
        amthreadpool_thread_usleep(10000); // 50ms
    ...
    // check mode
    // Local playback     - media.amplayer.local_drop_slow  0-drop 1-slow
    // online live vod    - media.amplayer.live_drop_slow   0-drop 1-slow
    // online request vod - media.amplayer.net_drop_slow    0-drop 1-slow

    sync_switch_ms = 200;
    
    static int live_slow = -1;
    static int net_slow = -1;
    static int local_slow = -1;

    if(local_slow == -1) {
        local_slow = 0;
        if (property_get("media.amplayer.local_drop_slow", value, NULL) > 0) {
            if(atoi(value) > 0) {
                local_slow = 1;
            } else {
                local_slow = 0;
        ...
        adec_print("local slow setto %d\n", local_slow);
    }

    if(net_slow == -1) {
        net_slow = 0;
        if (property_get("media.amplayer.net_drop_slow", value, NULL) > 0) {
            if(atoi(value) > 0) {
                net_slow = 1;
            } else {
                net_slow = 0;
        ...
        adec_print("net slow setto %d\n", net_slow);
    }

    if(live_slow == -1) {
        live_slow = 1;
        if (property_get("media.amplayer.live_drop_slow", value, NULL) > 0) {
            if(atoi(value) > 0) {
                live_slow = 1;
            } else {
                live_slow = 0;
        ...
        adec_print("live slow setto %d\n", live_slow);
    }

    if(audec->localplay_flag == 1) {
        sync_switch_ms = (local_slow == 1)?10000:200;
    } else {
        int is_live = 0;
        if (property_get("media.amplayer.livemode", value, NULL) > 0) {
            if(atoi(value) > 0) {
                is_live = 1;
            }
        }
        ...
        if(is_live) {
            sync_switch_ms = (live_slow == 1)?10000:200;
        } else {
            sync_switch_ms = (net_slow == 1)?10000:200;
        }
    }
    ...
    //when start to play,may audio discontinue happens, in this case, don't drop pcm operation
        ...
        return 0;
    } else if (diff > 5*90000 && audec->first_apts == 0) {
        adec_print("pts diff %ld bigger than 5000 (ms), and audec->first_apts = 0,don't drop pcm \n", diff);
        return 0;
    }
#endif
...
int droppcm_use_size(aml_audio_dec_t *audec, int drop_size)
{
    ...
    int enable_quick_start = 0;
    if (property_get("sys.amplayer.quick_start", value, NULL) > 0) {
        enable_quick_start = atoi(value);
    }
    adec_print("enable_quick_start=%d\n", enable_quick_start);
    ...
    while (drop_size > 0 && !audec->need_stop/*&& drop_duration < DROP_PCM_DURATION_THRESHHOLD*/) {
        ...
        if (nDropConsumeTime / 1000 > drop_max_time) {
            ...
            ret = -1;
            break;
        }else if((audec->localplay_flag != 1) && (enable_quick_start == 1) &&
                amsysfs_get_sysfs_int("/sys/module/di/parameters/first_frame_output")==1) {
			adec_print("di first frame output,stop drop,break\n");
			drop_size -= ret;
			ret = -1;
			break;
		}
...
int droppcm_get_refpts(aml_audio_dec_t *audec, unsigned long *refpts)
{
    ...
    if (sscanf(buf, "%d: %10s", &tsync_mode, tsync_mode_str) < 1) {
        adec_print("unable to get tsync_mode from: %s", buf);
        return -1;
    }
