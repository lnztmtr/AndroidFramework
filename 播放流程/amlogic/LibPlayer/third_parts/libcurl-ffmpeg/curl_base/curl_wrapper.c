...
static int curl_wrapper_easy_setopt_http_basic(CURLWHandle *h, Curl_Data *buf)
{
    ...
    */
    ...
    if ((ret = curl_wrapper_setopt_error(h, curl_easy_setopt(h->curl, CURLOPT_DNS_USE_GLOBAL_CACHE,1L))) != 0) {
        goto CRET;
    }
    if ((ret = curl_wrapper_setopt_error(h, curl_easy_setopt(h->curl, CURLOPT_DNS_CACHE_TIMEOUT, 120L))) != 0) {
        goto CRET;
    }
...
int curl_wrapper_perform(CURLWContext *con)
{
    ...
    int select_zero_max = SELECT_RETRY_WHEN_CONNECTING;
    int rrsip_support = am_getconfig_int_def("libplayer.rrsip.support", 0);
    if(strstr(con->curl_handle->uri,"rrsip") && rrsip_support){
          CLOGI("is rrsip uri\n");
          select_zero_max = 10;
    }
    /*add end*/
    ...
    while (running_handle_cnt) {
        ...
        if (!con->connected && select_zero_cnt == select_zero_max/*modify by zhaolianghua for jiangxi RongZai @20170117*/) {
            con->open_fail = 1;
            select_breakout_flag = 1;
            break;
        }
