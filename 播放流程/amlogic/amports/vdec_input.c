...
int vdec_input_set_buffer(struct vdec_input_s *input, u32 start, u32 size)
{
	...
	if (vdec_secure(input->vdec))
		...
		input->swap_page_bak = alloc_page(GFP_KERNEL);
		if (input->swap_page_bak) {
			input->swap_page_phys_bak =
				page_to_phys(input->swap_page_bak);
		}
		if (input->swap_page_phys_bak == 0) {
			pr_info("VDEC_SWAP: swap_page_phys_bak allocate fail %p %ld\n",
				input->swap_page_bak,
				input->swap_page_phys_bak);
			return -ENOMEM;
		}
		pr_info("VDEC_SWAP: swap_page_phys_bak allocated success\n");
#endif
	}
...
#ifdef DEBUG_SWAP_BUG
	if (input->swap_page_bak) {
		__free_page(input->swap_page_bak);
		input->swap_page_bak = NULL;
		input->swap_page_phys_bak = 0;
	}
#endif
