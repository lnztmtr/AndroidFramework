...
static u32 open_time;
static u32 open_endtime;
static u32 close_time;
static u32 close_endtime;
static u32 press_close_time;
static u32 codec_close_time;
static u32 close_init_time;
static u32 codec_init_time;
static s32 init_vformat = -1;
static bool time_debug;
static u32  multi_vdec_run;
...
#define DEFAULT_SUBTITLE_BUFFER_SIZE     (1024*512)

#define multidec(vdec)    ((vdec == NULL) ? 0 :  (!vdec_single(vdec)))
...
static int (*amstream_avinfo_vdec_status)
(struct vdec_s *vdec, struct vdec_info *vstatus);
...
u32 get_am_open_time(void)
{
	return open_time;
}
EXPORT_SYMBOL(get_am_open_time);

u32 get_am_open_endtime(void)
{
	return open_endtime;
}
EXPORT_SYMBOL(get_am_open_endtime);
u32 get_am_close_time(void)
{
	return close_time;
}
EXPORT_SYMBOL(get_am_close_time);
u32 get_press_close_time(void)
{
	return press_close_time;
}
EXPORT_SYMBOL(get_press_close_time);
u32 get_codec_close_time(void)
{
	return codec_close_time;
}
EXPORT_SYMBOL(get_codec_close_time);
u32 get_close_init_time(void)
{
	return close_init_time;
}
EXPORT_SYMBOL(get_close_init_time);
u32 get_codec_init_time(void)
{
	return codec_init_time;
}
EXPORT_SYMBOL(get_codec_init_time);
s32 get_stream_vformat(void)
{
	return init_vformat;
}
EXPORT_SYMBOL(get_stream_vformat);
	  ...
	  struct stream_buf_s *pbuf, int release_num)
{
	...
	bool is_multidec = multidec(vdec);
			  ...
			  struct stream_buf_s *pbuf)
{
	...
	is_poweroned_when_open = port->type & PORT_TYPE_HEVC;

	if (has_hevc_vdec() && is_poweroned_when_open == 0) {
...
static void audio_port_release(struct port_priv_s *priv,
		   struct stream_buf_s *pbuf, int release_num)
{
	struct stream_port_s *port = priv->port;
	struct vdec_s *vdec = priv->vdec;
...
static int audio_port_reset(struct port_priv_s *priv,
				struct stream_buf_s *pbuf)
{
	int r;
	struct stream_port_s *port = priv->port;
	...
	r = adec_init(port);
	if (r < 0) {
		audio_port_release(priv, pbuf, 2);
		return r;
	}
...
static int sub_port_reset(struct port_priv_s *priv,
				struct stream_buf_s *pbuf)
{
	int r;
	struct stream_port_s *port = priv->port;
...
static int audio_port_init(struct port_priv_s *priv,
			struct stream_buf_s *pbuf)
{
	int r;
	struct stream_port_s *port = priv->port;
	...
	if (port->type & PORT_TYPE_ES) {
		...
		if (r < 0) {
			audio_port_release(priv, pbuf, 3);
			return r;
		}
	}
	pbuf->flag |= BUF_FLAG_IN_USE;
...
static void sub_port_release(struct port_priv_s *priv,
					struct stream_buf_s *pbuf)
{
	struct stream_port_s *port = priv->port;
	struct vdec_s *vdec = priv->vdec;
...
static int sub_port_init(struct port_priv_s *priv, struct stream_buf_s *pbuf)
{
	int r;
	struct stream_port_s *port = priv->port;
	struct vdec_s *vdec = priv->vdec;
...
static int amstream_port_init(struct port_priv_s *priv)
{
	...
	if (time_debug)
		pr_info("CTC_KPI::Stage 2_1_1 codec init cost,open_endtime\n");
	mutex_unlock(&amstream_mutex);
	return 0;
	/*errors follow here */
error5:
	tsdemux_release();
error4:
	sub_port_release(priv, psbuf);
error3:
	video_port_release(priv, pvbuf, 0);
error2:
	audio_port_release(priv, pabuf, 0);
error1:
	mutex_unlock(&amstream_mutex);
	return r;
}
...
static int amstream_open(struct inode *inode, struct file *file)
{
	...
	if (port->type & PORT_TYPE_VIDEO
	&& multi_vdec_run
	&& (0 == (port->type & PORT_TYPE_DECODER_SCHED))) {
		pr_info(" used %d ,run =%d, type =%d\n",
		port->flag & BUF_FLAG_IN_USE, multi_vdec_run, port->type);
		return -EBUSY;
	}
	time_debug = get_time_debug();
	if (time_debug) {
		pr_info("CTC_KPI::Stage 2_1_1 codec init cost,open_time\n");
	}
...
static int amstream_release(struct inode *inode, struct file *file)
{
	...
	time_debug = get_time_debug();
	if (time_debug) {
		pr_info("CTC_KPI::Stage 1_3_2 video codec close cost,close_time\n");
	}
	...
	kfree(priv);
	if (time_debug) {
		pr_info("CTC_KPI::Stage 1_3_2 video codec close cost,close_endtime\n");
	}
...
static long amstream_ioctl_get(struct port_priv_s *priv, ulong arg)
{
	...
	case AMSTREAM_GET_APTS_LOOKUP:
		if (this->type & PORT_TYPE_AUDIO) {
			...
			offset = parm.data_32;
			pts_lookup_offset(PTS_TYPE_AUDIO,
				offset, &pts, &frame_size, 300);
			parm.data_32 = pts;
		}
		break;
	...
	case AMSTREAM_SET_VFORMAT:
		if ((this->type & PORT_TYPE_VIDEO) &&
			...
			if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXL) {
				if (parm.data_vformat == VFORMAT_H264_4K2K) {
...
static long amstream_ioctl_get_ex(struct port_priv_s *priv, ulong arg)
{
	...
	case AMSTREAM_GET_EX_AB_STATUS:
		if (this->type & PORT_TYPE_AUDIO) {
			...
			r = -EINVAL;
		break;
		case AMSTREAM_GET_EX_SB_STATUS:
		if (this->type & PORT_TYPE_SUB) {
			struct am_ioctl_parm_ex *p = &parm;
			struct stream_buf_s *buf = &bufs[BUF_TYPE_SUBTITLE];
			...
			if (sub_wp >= sub_rp)
				...
				data_size = buf->buf_size - sub_rp + sub_wp;
			p->status.size = buf->buf_size;
			p->status.data_len = data_size;
			p->status.free_len = buf->buf_size - data_size;
			p->status.read_pointer = sub_rp;
		} else
			r = -EINVAL;
		break;
...
int amstream_get_buf_status(struct av_buffer_status *buf_status)
{
	struct stream_buf_s *buf = NULL;
	/*video buffer*/
	int vformat = get_stream_vformat();
	buf = (vformat == VFORMAT_HEVC ||
		vformat == VFORMAT_VP9 || vformat == VFORMAT_AVS2) ?
		&bufs[BUF_TYPE_HEVC] :
		&bufs[BUF_TYPE_VIDEO];
	if (buf_status == NULL)
		return -1;
	buf_status->vbuf_size = stbuf_canusesize(buf);
	buf_status->vbuf_len = stbuf_level(buf);
	/*audio_buffer*/
	buf = &bufs[BUF_TYPE_AUDIO];
	buf_status->abuf_size = stbuf_canusesize(buf);
	buf_status->abuf_len = stbuf_level(buf);
	return 0;
}
	...
	case AMSTREAM_IOC_GET_AVINFO:
		{
			...
			if (this->type & PORT_TYPE_VIDEO) {
				...
				av_info.frame_format =
					get_frame_format_for_avinfo();
	...
	unsigned int cmd, ulong arg)
{
	...
	//don't print AMSTREAM_IOC_GET_AVINFO error temporary
	/*if (r != 0 && cmd != 0x8c7453c7)
		pr_err("amstream_do_ioctl error :%lx, %x\n", r, cmd);*/

	return r;
}

int get_avinfo_vdec_status(struct vdec_s *vdec, struct vdec_info *vstatus)
{
	if (amstream_avinfo_vdec_status != NULL) {
		amstream_avinfo_vdec_status(vdec, vstatus);
		return 0;
	} else {
		return -1;
	}
...
*/
	...
	amstream_avinfo_vdec_status = NULL;
...
void set_avinfo_vdec_func(int (*vdec_func)(struct vdec_s *, struct vdec_info *))
{
	amstream_avinfo_vdec_status = vdec_func;
	return;
}
...
int get_av_info_flag(void)
{
	return av_info_get_flag;
}
...
module_param(multi_vdec_run, uint, 0664);
MODULE_PARM_DESC(multi_vdec_run, "\n multi vdec run or not\n");
