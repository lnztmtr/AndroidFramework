...
#define DEC_CONTROL_FLAG_FORCE_RATE_2397_FPS_FIX_FRAME_RATE  0x00100
#define DEC_CONTROL_FLAG_FORCE_RATE_2997_FPS_FIX_FRAME_RATE  0x00200
...
*/
/*[SE] [BUG][BUG-167638][chuanqi.wang]:don't mark error according to last pic*/
static unsigned int error_proc_policy = 0x3fb6; /*0x1f14*/
...
extern unsigned char amvdec_enable_flag;
...
static void    hevc_mcr_config_mc_ref(struct vdec_h264_hw_s *hw)
{
	...
	/*REFLIST[0]*/
	for (i = 0; i <
		(unsigned int)(pSlice->listXsize[0]); i++) {
...
static int check_force_interlace(struct vdec_s *vdec,
...
int prepare_display_buf(struct vdec_s *vdec, struct FrameStore *frame)
{
	...
	/* some stream (interlaced) with a error of missed field,
		so check this error here */
	if (frame->frame == NULL &&
	...
	bForceInterlace = check_force_interlace(vdec, frame);
	if (bForceInterlace)
		vf_count = 2;
...
#define H264_CO_MB_RW_CTL         VLD_C3D /* 0xc3d */
	...
	if (last_pic)
		dpb_print(DECODE_ID(hw), PRINT_FLAG_ERRORFLAG_DBG,
				"last_pic->data_flag %x   slice_type %x\n",
				last_pic->data_flag, pSlice->slice_type);
...
/*
	if (h264_debug_flag & PRINT_FLAG_PROFILE)
	...
	u32 param1, u32 param2, u32 param3, u32 param4)
{
	...
	if (time_debug) {
		pr_info("CTC_KPI::Stage 3_2 %s\n",
			"firstcheckin_cmd1_time cost,set_cmd1_time");
		pr_info("CTC_KPI::Stage 3_3 %s\n",
			"cmd1_firstcheckout_time,set_cmd1_time\n");
	}
#ifdef CONFIG_AM_VDEC_DV
	...
	if (mb_width > 0x110 ||
		...
		mb_width <= 0 ||
		mb_height <= 0) {
		hw->param_config_fails++;
		if (hw->param_config_fails < 20)
			dpb_print(DECODE_ID(hw), 0,
				"!!!wrong seq_info2 0x%x mb_width/mb_height (0x%x/0x%x) %d\r\n",
				seq_info2,
				mb_width,
				mb_height,
				hw->param_config_fails);
...
static irqreturn_t vh264_isr_thread_fn(struct vdec_s *vdec, int irq)
{
	...
	if (dec_dpb_status == H264_CONFIG_REQUEST) {
		...
		if (p_H264_Dpb->mVideo.dec_picture) {
			...
			if (slice_header_process_status == 1) {
				...
				if (p_H264_Dpb->mSPS.profile_idc == BASELINE
					|| (((unsigned long)
					(hw->vh264_amstream_dec_info.param)) &
					0x8) && !hw->i_only)
				...
				hw->skip_frame_count);
					...
					if ((error_proc_policy & 0x100) &&
						...
						READ_VREG(MBY_MBX),
						decode_mb_count);
			if (!p_H264_Dpb->dpb_param.l.data[FIRST_MB_IN_SLICE]) {
							p->data_flag |= ERROR_FLAG;
							goto pic_done_proc;
						}
						else
			...
			if (error_proc_policy & 0x400) {
				...
				if (ret != 0) {
					dpb_print(DECODE_ID(hw), 0,
						...
						hw->skip_frame_count);
					hw->data_flag |= ERROR_FLAG;
					/* +[SE] [BUG][BUG-166452][chuanqi.wang]:don't reset
					   bufmgr when reference frame error in i only mode*/
					if (!hw->i_only &&
		...
		hw->reg_rv_ai_mb_count = READ_VREG(RV_AI_MB_COUNT);
		hw->vld_dec_control = READ_VREG(VLD_DECODE_CONTROL);
		pr_info("reg 1 %x:%x:%x:%x\n",hw->reg_iqidct_control,
		hw->reg_vcop_ctrl_reg, hw->reg_rv_ai_mb_count,
		hw->vld_dec_control);*/
...
pic_done_proc:
		...
		if (p_H264_Dpb->mVideo.dec_picture) {
			...
			if (hw->chunk) {
				...
				if (pts_lookup_offset_us64(PTS_TYPE_VIDEO,
					offset, &pic->pts, &pic->frame_size, 0, &pic->pts64)) {
					pic->pts = 0;
					pic->pts64 = 0;
				}
			}
		...
		dpb_print(DECODE_ID(hw), PRINT_FLAG_VDEC_STATUS,
			...
			hw->decode_pic_count);
			time_debug = get_time_debug();
			if ((1 == hw->decode_pic_count) &&
				(time_debug)) {
				pr_info("CTC_KPI::Stage 3_3 %s\n",
					"first frame decoded cost,first_decoded_time");
				pr_info("CTC_KPI::Stage 4_1 %s\n",
					"decode->di 0 cost,send_out_time\n");
				}
		/* WRITE_VREG(DPB_STATUS_REG, H264_ACTION_SEARCH_HEAD); */
		hw->dec_result = DEC_RESULT_DONE;
#ifdef CONFIG_AM_VDEC_DV
...
static void check_timer_func(unsigned long arg)
{
	...
	if ((h264_debug_cmd & 0x200) != 0 &&
		DECODE_ID(hw) == (h264_debug_cmd & 0xff)) {
		pr_info("vdec %d is forced to reset bufmgr\n",
			h264_debug_cmd & 0xff);
		hw->reset_bufmgr_flag = 1;
		h264_debug_cmd = 0;
		return;
	...
	if (radr != 0) {
		if (rval != 0) {
			...
			dpb_print(DECODE_ID(hw), 0, "READ_VREG(%x)=%x\n", radr, READ_VREG(radr));
		rval = 0;
		radr = 0;
	}
	...
	*/
	if (!firmwareloaded) {
		...
		if (cpu_type >= MESON_CPU_MAJOR_ID_GXM) {
			/*
			pr_info("start load gxm orignal firmware ...\n");
			*/
			/*
			...
			*/
			...
			r6 = get_decoder_firmware_data(VFORMAT_H264,
					"gxm_vmh264_mc",
					(u8 *) hw->mc_cpu_addr + MC_OFFSET_MAIN,
					0x800 * 4);

			r7 = get_decoder_firmware_data(VFORMAT_H264,
					"gxm_vmh264_data_mc",
					(u8 *) hw->mc_cpu_addr + MC_OFFSET_MAIN
					+ 0x800 * 4, 0x400 * 4);

			r8 = get_decoder_firmware_data(VFORMAT_H264,
					"gxm_vmh264_list_mc",
					(u8 *) hw->mc_cpu_addr + MC_OFFSET_MAIN
					+ 0x800 * 4	+ 0x400 * 4, 0x400 * 4);

		} else {
			/*
			pr_info("start load gxtvbb orignal firmware ...\n");
			*/
			/*
			r0 = amvdec_loadmc_ex(VFORMAT_H264, "vmh264_mc", NULL);
			*/
			...
			r8 = get_decoder_firmware_data(VFORMAT_H264,
					"vmh264_list_mc",
					(u8 *) hw->mc_cpu_addr
					+ MC_OFFSET_MAIN + 0x800 * 4 + 0x400 * 4
					, 0x400 * 4);
		}
		if (mmu_enable)
	...
	void (*callback)(struct vdec_s *, void *), void *arg)
{
	...
	unsigned int cpu_type = get_cpu_type();
	if (cpu_type >= MESON_CPU_MAJOR_ID_GXM) {
		if (amvdec_vdec_loadmc_ex(vdec, "gxm_vmh264_mc") < 0) {
			amvdec_enable_flag = false;
			amvdec_disable();
			pr_info("%s: Error gxm amvdec_vdec_loadmc fail\n",
				__func__);
			return;
		}
	} else {
		if (amvdec_vdec_loadmc_ex(vdec, "vmh264_mc") < 0) {
			amvdec_enable_flag = false;
			amvdec_disable();
			pr_info("%s: Error gxtvbb amvdec_vdec_loadmc fail\n",
				__func__);
			return;
		}
	}
...
static void h264_reset_bufmgr(struct vdec_s *vdec)
{
	...
	vf_notify_receiver(vdec->vf_provider_name,
					   VFRAME_EVENT_PROVIDER_RESET, NULL);
...
static int ammvdec_h264_probe(struct platform_device *pdev)
{
	...
	if (force_enable_mmu && pdata->sys_info &&
		...
		(get_cpu_type() != MESON_CPU_MAJOR_ID_GXLX2) &&
...
static int ammvdec_h264_remove(struct platform_device *pdev)
{
	...
	dpb_print(DECODE_ID(hw), 0, "clear mbox in ARM %x\n", hw->stat);
