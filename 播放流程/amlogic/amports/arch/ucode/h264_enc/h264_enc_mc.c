...
#define DEF_FIRMEARE_FOR_GXLX(n) \
	REGISTER_FIRMARE_PER_CPU_VER(MESON_CPU_MAJOR_ID_GXLX, \
				FOR_VFORMAT, n, CODEC_VERSION)

#define DEF_FIRMEARE_FOR_GXLX2(n) \
	REGISTER_FIRMARE_PER_CPU_VER(MESON_CPU_MAJOR_ID_GXLX2, \
				FOR_VFORMAT, n, CODEC_VERSION)
...
#define REG_FIRMWARE_ALL()\
	do {\
		...
		DEF_FIRMEARE_FOR_GXLX(h264_enc_mc_txl);\
		DEF_FIRMEARE_FOR_GXLX2(h264_enc_mc_txl);\
	} while (0)
