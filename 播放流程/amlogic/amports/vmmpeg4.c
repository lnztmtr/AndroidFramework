...
static irqreturn_t vmpeg4_isr_thread_fn(struct vdec_s *vdec)
{
	u32 reg;
	struct vframe_s *vf = NULL;
	u32 picture_type;
	int index;
	u32 pts, frame_size, offset = 0;
...
static void vmpeg4_dump_state(struct vdec_s *vdec)
{
	...
	mmpeg4_debug_print(DECODE_ID(hw), 0,
		"is_framebase(%d), eos %d, state 0x%x, dec_result 0x%x dec_frm %d,counter=%d\n",
