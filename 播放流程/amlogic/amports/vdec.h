...
/*DEBUG_SWAP_BUG define in input.h*/
...
#define CORE_MASK_HEVC_BACK (1 << VDEC_HEVC_BACK)
...
struct vdec_s {
	...
	int swap_error_flag;
	int swap_dma_time;
	int swap_restore_flag; /*bit0, restore bak*/
	u8 swap_debug_test_flag;
#endif
#ifdef VDEC_DEBUG_SUPPORT
	u64 profile_start_clk;
	u64 total_clk;
	u32 check_count;
	u32 not_run_ready_count;
	u32 input_underrun_count;
	u32 run_count;
	u64 run_clk;
	u64 start_run_clk;
#endif
...
#ifdef DEBUG_SWAP_BUG
extern void vdec_poweron_safe(struct vdec_s *vdec);

extern void vdec_poweroff_safe(struct vdec_s *vdec);
