...
static struct vframe_qos_s vframe_qos;
static u32 frameinfo_enable;
...
static void fill_frame_info(u32 slice_type, int frame_size, u32 pts)
{
	unsigned char a[3];
	unsigned char i, j, t;
	unsigned long  data;
	struct vframe_qos_s *v_qos = &vframe_qos;

	v_qos->type = ((slice_type & PICINFO_TYPE_MASK) == PICINFO_TYPE_I) ? 1 :
		((slice_type & PICINFO_TYPE_MASK) == PICINFO_TYPE_P) ? 2 : 3;
	v_qos->size = frame_size;
	v_qos->pts = pts;
...
{
	u32 reg, info, seqinfo, offset, pts, frame_size, pts_valid = 0;
	struct vframe_s *vf;
	u64 pts_us64 = 0;
	...
	if ((reg >> 16) == 0xfe) {
		...
		if ((first_i_frame_ready == 0) &&
			...
			if ((time_debug) && (slice_type_num == 0)) {
				...
				pr_info("Enter set cmd1:%u\n", set_cmd1_time);
				first_checkin_vpts_time =
					get_first_checkin_vpts_time();
		...
		if ((pts_lookup_offset_us64
			 (PTS_TYPE_VIDEO, offset, &pts, &frame_size, 0, &pts_us64) == 0)
			&& (((info & PICINFO_TYPE_MASK) == PICINFO_TYPE_I)
		...
		if (frame_prog & PICINFO_PROG) {
			...
			fill_frame_info(info, frame_size, pts);
			...
			if ((error_skip(info, vf)) ||
				...
				if ((time_debug) && (slice_type_num == 0)) {
					...
					pr_info("CTC_KPI::Stage 3_3 %s,%s\n",
						"first_frame_decodec cost",
						"first_decodec_time end");
					pr_info("CTC_KPI::Stage 4_1 %s\n",
						"decode->di 0 cost,send_out_time1");
					slice_type_num = 1;
				}
			}
			...
			if ((error_skip(info, vf)) ||
				...
				if ((time_debug) && (slice_type_num == 0)) {
					...
					pr_info("CTC_KPI::Stage 3_3 %s,%s\n",
						...
						"first_decodec_time end");
					pr_info("CTC_KPI::Stage 4_1 %s\n",
						"decode->di 0 cost,send_out_time2");
					slice_type_num = 1;
		    	}
			}
			if (kfifo_get(&newframe_q, &vf) == 0) {
				...
				vf_notify_receiver(PROVIDER_NAME,
					...
					if ((time_debug) &&
						...
						pr_info("CTC_KPI::Stage 3_3 %s,%s\n",
							...
							"first_decodec_time end");
						pr_info("CTC_KPI::Stage 4_1 %s\n",
							"decode->di 0 cost,send_out_time3");
						slice_type_num = 1;
					}
			}
...
u32 get_vmpeg12_set_cmd1_time(void)
{
	return set_cmd1_time;
}
EXPORT_SYMBOL(get_vmpeg12_set_cmd1_time);
u32 get_vmpeg12_first_decoded_time(void)
{
	return first_decoded_time;
}
EXPORT_SYMBOL(get_vmpeg12_first_decoded_time);
u32 get_vmpeg12_firstcheckin_cmd1_time(void)
{
	return firstcheckin_cmd1_time;
}
EXPORT_SYMBOL(get_vmpeg12_firstcheckin_cmd1_time);
u32 get_vmpeg12_firstcheckout_decoded_time(void)
{
	return firstcheckout_decoded_time;
}
EXPORT_SYMBOL(get_vmpeg12_firstcheckout_decoded_time);
static int vmpeg12_prot_init(void)
{
	int ret;
...
static void vmpeg12_local_init(void)
{
	...
	memset(&vframe_qos, 0x0, sizeof(struct vframe_qos_s));
