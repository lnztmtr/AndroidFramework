...
#ifdef MAP_8K
#define TOTAL_4K_NUM (0x8000*4)
/*compute_losless_comp_body_size(4096, 2304, 1) = 18874368(0x1200000)*/
#define MAX_FRAME_4K_NUM (0x1200*4)
#else
...
#endif
#ifdef MAP_8K
#define FRAME_MMU_MAP_SIZE  (MAX_FRAME_4K_NUM * 4 * 4)
#else
...
#ifdef AVS2_10B_MMU
#ifdef MAP_8K
#define MMU_COMPRESS_HEADER_SIZE  (0x48000*4)
#else
...
static struct BuffInfo_s amvavs2_workbuff_spec[WORK_BUF_SPEC_NUM] = {
	...
	},
	{
		...
		.cm_header = {
			/*0x44000 = ((1088*2*1024*4)/32/4)*(32/8)*/
			.buf_size = MMU_COMPRESS_HEADER_SIZE * 17,
		},
#endif
#endif
		.mpred_above = {
	...
	int32_t lcu_size_log2)
{
	...
	/*alloc AVS2 compress header first*/
		if (decoder_bmmu_box_alloc_buf_phy
				(dec->bmmu_box,
				HEADER_BUFFER_IDX(-1), MMU_COMPRESS_HEADER_SIZE,
				DRIVER_HEADER_NAME,
				&buf_addr1) < 0){
			avs2_print(dec, 0,
	...
	while (1) {
		...
		if (pic->error_mark) {
			avs2_print(dec, AVS2_DBG_BUFMGR_DETAIL,
				"!!!error pic, skip\n",
				0);
vdec_count_info(dec->gvs, 1, pic->stream_offset);
			continue;
		}
...
#if 0
/*
static void print_parser_reg(struct AVS2Decoder_s *dec)
{
	WRITE_VREG(HEVC_PARSER_DEBUG_IDX, 0x0);
	avs2_print(dec, 0,
		"HEVC_PARSER_DEBUG_DAT %x %x %x %x, %x %x %x %x,
		%x %x %x %x, %x %x %x %x,
		HEVC_PARSER_RESULT_0/1,2,3 %x %x %x %x\n",
		READ_VREG(HEVC_PARSER_DEBUG_DAT),
		READ_VREG(HEVC_PARSER_DEBUG_DAT),
		READ_VREG(HEVC_PARSER_DEBUG_DAT),
		READ_VREG(HEVC_PARSER_DEBUG_DAT),
		READ_VREG(HEVC_PARSER_DEBUG_DAT),
		...
		READ_VREG(HEVC_PARSER_DEBUG_DAT),
		READ_VREG(HEVC_PARSER_DEBUG_DAT),
		READ_VREG(HEVC_PARSER_RESULT_0),
		READ_VREG(HEVC_PARSER_RESULT_1),
		READ_VREG(HEVC_PARSER_RESULT_2),
		READ_VREG(HEVC_PARSER_RESULT_3));
}
*/
#define HEVC_MPRED_MCR_CNT_DATA                    0x325d

static void print_reg(struct AVS2Decoder_s *dec)
{
	avs2_print(dec, 0,
		"HEVC_DBLK_STS1 %x, HEVC_MPRED_MCR_CNT_DATA %x HEVC_SAO_MMU_STATUS %x\n",
		READ_VREG(HEVC_DBLK_STS1),
		READ_VREG(HEVC_MPRED_MCR_CNT_DATA),
...
static s32 vavs2_init(struct vdec_s *vdec)
{
	...
	amhevc_enable();
	if (debug & AVS2_DBG_LOAD_UCODE_FROM_FILE) {
		pr_info("load ucode from file of vavs2_mc_debug\n");
		if (amhevc_loadmc_ex(VFORMAT_AVS2,
				"vavs2_mc_debug", NULL) < 0) {
			amhevc_disable();
			return -EBUSY;
		}
#if 0
	} else if (get_double_write_mode(dec) & 0x10) {
		pr_info("load ucode from file of vavs2_mc_dw\n");
		if (amhevc_loadmc_ex(VFORMAT_AVS2,
				"vavs2_mc_dw", NULL) < 0) {
			amhevc_disable();
			return -EBUSY;
...
#endif
	} else if (amhevc_loadmc_ex(VFORMAT_AVS2, "vavs2_mc", NULL) < 0) {
		amhevc_disable();
		return -EBUSY;
	}
...
static int amvdec_avs2_probe(struct platform_device *pdev)
{
	...
	pr_info("xia 1 dec->vavs2_amstream_dec_info.width = %d,dec->vavs2_amstream_dec_info.height = %d",
		dec->vavs2_amstream_dec_info.width,
		dec->vavs2_amstream_dec_info.height);
	...
	void (*callback)(struct vdec_s *, void *), void *arg)
{
	...
	if (amhevc_loadmc_ex(VFORMAT_AVS2, "vavs2_mc", NULL) < 0) {
		amhevc_disable();
		avs2_print(dec, 0,
...
static int ammvdec_avs2_probe(struct platform_device *pdev)
{
	...
	pr_info("xia 2 dec->vavs2_amstream_dec_info.width = %d,dec->vavs2_amstream_dec_info.height = %d",
		dec->vavs2_amstream_dec_info.width,
		dec->vavs2_amstream_dec_info.height);
...
static int __init amvdec_avs2_driver_init_module(void)
{
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXLX2) {
		if (vdec_is_support_4k())
