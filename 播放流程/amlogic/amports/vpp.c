...
unsigned int sr_core;

static unsigned int sr_off_limit = 1280;
module_param(sr_off_limit, uint, 0664);
MODULE_PARM_DESC(sr_off_limit, "sr_off_limit");
...
unsigned int force_use_dw;
MODULE_PARM_DESC(force_use_dw, "force_use_dw");
module_param(force_use_dw, uint, 0664);
 ...
 */
#define MIN_RATIO_1000	1050
#define SIZE_4K        0
#define SIZE_1080      1
#define SIZE_720       2
#define SIZE_576       3
static const int ratio[4][4] = {
	{1250, 1050, 1250, 1250},
	{2570, 2500, 3600, 6000},
	{2500, 2500, 2500, 2500},
	{2500, 2500, 2500, 2500}
...
unsigned int h_skip_ratio;
MODULE_PARM_DESC(h_skip_ratio, "h_skip_ratio");
module_param(h_skip_ratio, uint, 0664);
		...
		const struct vinfo_s *vinfo, struct vframe_s *vf)
{
	...
	u32 width_out = 0;
	u32 disp_size = SIZE_4K;
	u32 source_size = SIZE_4K;
		...
		if (height_in * bpp > height_out) {
			...
			*/
			...
			if (get_cpu_type() >=
				...
				if (super_debug)
					pr_info("ratio %d in %d out %d vinfo %d disp %d source %d\n",
						cur_ratio, height_in,
						height_out,
						vinfo->height,
						disp_size, source_size);
			...
			if (vf->type & VIDTYPE_VIU_422) {
				...
				if (height_out == 0
					...
					if (super_debug)
						pr_info("vpp_process_speed_check width_in %d height_in %d vtotal %d height_out %d bypass_ratio %d clk_in_pps %d\n",
						width_in, height_in,
						vtotal,
						height_out,
						bypass_ratio, clk_in_pps);
...
RESTART:
	...
	*/
	...
	if (h_skip_ratio) {
		...
		if ((next_frame_par->VPP_hsc_endp -
			next_frame_par->VPP_hsc_startp + 1) > 0)
			h_ratio = (next_frame_par->VPP_hd_end_lines_ -
				next_frame_par->VPP_hd_start_lines_ + 1) /
		...
		if (next_frame_par->hscale_skip_count < hskip) {
			next_frame_par->hscale_skip_count = hskip;
			restart_flag = true;
		}

		if (!(vpp_flags & VPP_FLAG_INTERLACE_IN)) {
			if ((next_frame_par->VPP_vsc_endp -
				next_frame_par->VPP_vsc_startp + 1) > 0)
				v_ratio = next_frame_par->VPP_pic_in_height_ /
					(next_frame_par->VPP_vsc_endp -
					next_frame_par->VPP_vsc_startp + 1);
		}
		if (v_ratio >= 2 &&
			next_frame_par->vscale_skip_count < MAX_VSKIP_COUNT) {
			next_frame_par->vscale_skip_count++;
			cur_skip_count++;
			restart_flag = true;
		}
	...
	if ((horz_scaler_filter == 0xff) && (vert_scaler_filter == 0xff)) {
		if ((vinfo->width > 3840) && (vinfo->height >= 2160) &&
			...
			filter->vpp_horz_coeff
				= filter_table[COEF_3POINT_TRIANGLE];
			filter->vpp_horz_filter = COEF_3POINT_TRIANGLE;
		}
	}
		...
		int vpp_postblend_out_height)
{
	...
	if (is_meson_gxtvbb_cpu())
		...
		tmp_data = READ_VCBUS_REG(SRSHARP1_SHARP_SR2_CTRL);
	if (is_meson_gxtvbb_cpu() ||
	...
	if (get_cpu_type() != MESON_CPU_MAJOR_ID_GXTVBB)
		tmp_data2 = READ_VCBUS_REG(SRSHARP1_SHARP_HVSIZE);
	...
	if (is_meson_txhd_cpu())
		tmp_data = ((reg_srscl0_hsize & 0x1fff) << 16) |
			(reg_srscl0_vsize & 0x1fff);
	else if ((is_meson_gxlx_cpu() && (scaler_path_sel == CORE1_AFTER_PPS))
		|| (is_meson_gxlx2_cpu() &&
		(scaler_path_sel == CORE1_BEFORE_PPS)))
		tmp_data = ((pps_w_output & 0x1fff) << 16) |
			(pps_h_output & 0x1fff);
	else
		tmp_data = ((reg_srscl1_hsize & 0x1fff) << 16) |
			(reg_srscl1_vsize & 0x1fff);
	...
	if (is_meson_txhd_cpu())
		...
		data_path_chose = 5;
	tmp_data2 = (READ_VCBUS_REG(VPP_VE_ENABLE_CTRL) >> data_path_chose)&0x1;
	if ((((scaler_path_sel == CORE0_PPS_CORE1) ||
		(scaler_path_sel == CORE1_BEFORE_PPS) ||
	...
	* todo:if you have better idea,you can improve it*/
	...
	if (scaler_path_sel >= SCALER_PATH_MAX) {
		if (is_meson_gxlx_cpu() || is_meson_gxlx2_cpu()) {
			...
			else if ((vinfo->width > 3840 && vinfo->height >= 2160)
				&& (src_height < 720))
				next_frame_par->supscl_path = CORE1_BEFORE_PPS;
			else
	...
	if ((next_frame_par->supscl_path == CORE0_PPS_CORE1) ||
		(next_frame_par->supscl_path == CORE1_AFTER_PPS)) {
		next_frame_par->pps_w_out = width_out >>
			next_frame_par->supsc1_hori_ratio;
		next_frame_par->pps_h_out = height_out >>
			next_frame_par->supsc1_vert_ratio;
	} else if (next_frame_par->supscl_path == CORE0_AFTER_PPS) {
		next_frame_par->pps_w_out = width_out >>
			next_frame_par->supsc0_hori_ratio;
