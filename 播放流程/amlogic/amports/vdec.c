...
static int step_mode;
	...
	/* reset_mode:
	1, reset before run
	2, power_off/power_on before run`
	3, power_off when none_active,
		and power_on before run
	*/
...
unsigned char amvdec_enable_flag;
...
#ifdef DEBUG_SWAP_BUG
				if (vdec->swap_restore_flag & 0x1) {
					WRITE_VREG(VLD_MEM_SWAP_ADDR,
						input->swap_page_phys_bak);
					pr_info("VDEC_SWAP: restore back swap\n");
				} else
					WRITE_VREG(VLD_MEM_SWAP_ADDR,
						input->swap_page_phys);
				WRITE_VREG(VLD_MEM_SWAP_CTL, 1);

				vdec->swap_dma_time = 0;
				vdec->swap_error_flag = 0;
				while (READ_VREG(VLD_MEM_SWAP_CTL) & (1<<7)) {
					vdec->swap_dma_time++;
					if (vdec->swap_dma_time >
						(100 * 1000)) {
						vdec->swap_error_flag = 3;
						pr_info("VDEC_SWAP: restore timeout\n");
						step_mode = 0xffffffff;
						return -1;
					}
				}
#else
...
void vdec_save_input_context(struct vdec_s *vdec)
{
	...
	if (input_stream_based(input) && (input->swap_needed)) {
		if (input->target == VDEC_INPUT_TARGET_VLD) {
			...
			vdec->swap_dma_time = 0;
			vdec->swap_error_flag = 0;
			while ((vdec->swap_debug_test_flag) ||
				(READ_VREG(VLD_MEM_SWAP_CTL) & (1<<7))) {
				vdec->swap_dma_time++;
				if (vdec->swap_dma_time > (100 * 1000)) {
					vdec->swap_error_flag = 1;
					pr_info("VDEC_SWAP: save timeout\n");
					step_mode = 0xffffffff;
					return;
				}
			}
			...
			if ((vdec->swap_restore_flag & 0x2) == 0) {
				WRITE_VREG(VLD_MEM_SWAP_ADDR,
					input->swap_page_phys_bak);
				WRITE_VREG(VLD_MEM_SWAP_CTL, 3);
				vdec->swap_dma_time = 0;
				vdec->swap_error_flag = 0;
				while (READ_VREG(
					VLD_MEM_SWAP_CTL) & (1<<7)) {
					vdec->swap_dma_time++;
					if (vdec->swap_dma_time >
						(100 * 1000)) {
						vdec->swap_error_flag = 2;
						pr_info("VDEC_SWAP: save back timeout\n");
						step_mode = 0xffffffff;
						return;
					}
				}
...
void vdec_prepare_run(struct vdec_s *vdec, unsigned long mask)
{
	...
	if (input->target == VDEC_INPUT_TARGET_VLD) {
		if (reset_mode == 1)
			...
			vdec_poweroff_safe(vdec);
			vdec_poweron_safe(vdec);
...
#ifdef DEBUG_SWAP_BUG
void vdec_poweroff_safe(struct vdec_s *vdec)
{
	unsigned long flags;
	spin_lock_irqsave(&vdec_spin_lock, flags);
	codec_dmcbus_write(DMC_REQ_CTRL,
...
void vdec_poweron_safe(struct vdec_s *vdec)
{
	unsigned long flags;
	vdec_poweron(VDEC_1);
	spin_lock_irqsave(&vdec_spin_lock, flags);
		...
		const char *buf, size_t size)
{
	...
	list_for_each_entry(vdec,
		...
		if (((vdec->status == VDEC_STATUS_CONNECTED
			...
			(vdec->id == id)) {
#ifdef DEBUG_SWAP_BUG
				if (strcmp(cbuf, "swap_error_flag") == 0) {
					pr_info(
						"VDEC_SWAP: set swap_error_flag to 0x%x\n",
						val);
					vdec->swap_error_flag = val;
				} else if (strcmp(cbuf,
						"swap_restore_flag") == 0) {
					pr_info("VDEC_SWAP: set swap_restore_flag to 0x%x\n",
						val);
					vdec->swap_restore_flag = val;
				} else if (strcmp(cbuf,
						"swap_debug_test_flag") == 0) {
					pr_info("VDEC_SWAP: set swap_debug_test_flag to 0x%x\n",
						val);
					vdec->swap_debug_test_flag = val;
				}
#endif
				break;
		}
		...
		struct class_attribute *attr, char *buf)
{
	...
	list_for_each_entry(vdec,
		...
		if ((vdec->status == VDEC_STATUS_CONNECTED
			|| vdec->status == VDEC_STATUS_ACTIVE)) {
				pbuf += sprintf(pbuf, "name: %s, ",
					vdec->vf_provider_name);
				pbuf += sprintf(pbuf, "swap_error_flag: %d, ",
					vdec->swap_error_flag);
				pbuf += sprintf(pbuf, "swap_restore_flag: %d, ",
					vdec->swap_restore_flag);
				pbuf += sprintf(pbuf,
					"swap_debug_test_flag: %d\n",
					vdec->swap_debug_test_flag);
		}
	}
#endif

	pbuf += sprintf(pbuf,
		"name\tschedule_count\trun_count\tinput_underrun\tdecbuf_not_ready\trun_time\n");
	list_for_each_entry(vdec,
		&core->connected_vdec_list, list) {
		if ((vdec->status == VDEC_STATUS_CONNECTED
			|| vdec->status == VDEC_STATUS_ACTIVE)) {
				...
				pbuf += sprintf(pbuf, "\t%d",
					vdec->run_count);
				pbuf += sprintf(pbuf, "\t%d",
					vdec->input_underrun_count);
				pbuf += sprintf(pbuf, "\t%d",
					vdec->not_run_ready_count);
				pbuf += sprintf(pbuf,
					"\t%d%%\n",
					vdec->total_clk == 0 ? 0 :
					(u32)((vdec->run_clk * 100)
					/ vdec->total_clk));
		}
	}
...
#ifdef VDEC_SCHEDULE_PROFILE
static ssize_t store_vdec_schedule_profile(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	struct vdec_s *vdec;
	...
	list_for_each_entry(vdec,
		&core->connected_vdec_list, list) {
		vdec->total_clk = 0;
		vdec->profile_start_clk = 0;
		vdec->check_count = 0;
		vdec->not_run_ready_count = 0;
		vdec->input_underrun_count = 0;
		vdec->run_count = 0;
		vdec->run_clk = 0;
		vdec->start_run_clk = 0;
	}
	vdec_core_unlock(vdec_core, flags);
...
static ssize_t show_vdec_schedule_profile(struct class *class,
		struct class_attribute *attr, char *buf)
{
	char *pbuf = buf;
	struct vdec_s *vdec;
	...
	if (list_empty(&core->connected_vdec_list)) {
		...
		pbuf += sprintf(pbuf,
			"name    schedule_count run_count input_underrun decoder_buf_not_ready run_time\n");
		list_for_each_entry(vdec,
			&core->connected_vdec_list, list) {
			if ((vdec->status == VDEC_STATUS_CONNECTED
				|| vdec->status == VDEC_STATUS_ACTIVE)) {
...
struct vframe_qos_s *vdec_get_qos_info(void)
{
	...
	if (frame_qos_rd > qos_wr) {
		wirte_count = QOS_FRAME_NUM - frame_qos_rd;
		if (wirte_count > 0 && wirte_count <= QOS_FRAME_NUM) {
			memcpy(frame_info_buf_out, &frame_info_buf_in[0],
				wirte_count*sizeof(struct vframe_qos_s));
			if ((wirte_count + qos_wr) <= QOS_FRAME_NUM)
				memcpy(&frame_info_buf_out[wirte_count], frame_info_buf_in,
		...
		wirte_count =  qos_wr - frame_qos_rd;
		if (wirte_count > 0 && wirte_count < QOS_FRAME_NUM)
...
module_param(frame_info_buf_out, charp, 0444);
MODULE_PARM_DESC(frame_info_buf_out,
				"\n frame_info_buf_out\n");

module_param(decode_underflow, int, 0664);
MODULE_PARM_DESC(decode_underflow,
				"\n decode_underflow\n");
