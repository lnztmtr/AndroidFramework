			 ...
			 struct StorablePicture *dec_picture)
{
	...
	dec_picture->slice_type = currSlice->slice_type;
...
/* p->buf_spec_num = fs->index; */
	...
	case FRAME:
		...
		/*[SE] [BUG][BUG-170797][chuanqi.wang]:add pts in frame mode.*/
		fs->pts = p->pts;
		fs->pts64 = p->pts64;
	...
	case TOP_FIELD:
		...
		fs->is_used |= 1;
		fs->layer_id = p->layer_id;
/*[SE] [BUG][BUG-168844][chuanqi.wang]:use first pts when filed.*/
		if (fs->frame_size == 0) {
			fs->slice_type = p->slice_type;
			fs->pts = p->pts;
			fs->pts64 = p->pts64;
