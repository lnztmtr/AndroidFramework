...
/* enable ENABLE_SEI_ITU_T35, should  use ucode
   which cc zimu supported  at same time*/
...
#define DEC_CONTROL_FLAG_FORCE_2500_1080P_INTERLACE  0x0040
#define DEC_CONTROL_FLAG_DISABLE_FAST_POC            0x0080
#define DEC_CONTROL_FLAG_FORCE_3000_1080P_INTERLACE  0x0100
...
static int fix_duration;
static int pts_index;

static u32 radr;
static u32 rval;


static u32 print_search_count;
static u32 search_active;
static u32 search_count;
static u32 search_start_code_over = 500;
static u32 inactive_timeout;
static u32 prev_buflevel;
static u32 timer_status_debug;
...
static u32 first_decoded_time;
static u32 set_cmd1_time;
static u32 first_checkin_vpts_time;
static u32 firstcheckin_cmd1_time;
static u32 firstcheckout_decoded_time;
static bool time_debug;
#else
static u32 frame_buffer_size;
...
static u32 dec_control =
	...
	DEC_CONTROL_FLAG_FORCE_3000_1080P_INTERLACE;
...
/*h264 es with a big gop may lead to stuck,
give this limited value for calculate dts_duration in time */
static uint h264_pts_count_I_late = 60;
/*pre calculate pts_duration*/
static uint h264_pts_count_pre = 1;

static uint slice_type_num;
static uint print_slice_type_max = 2;
static uint decoded_drop_count;
...
static DEFINE_SPINLOCK(vh264_status_lock);
...
#define SUPPORT_NODE
	...
	*/
	...
	pr_info(" [Picture %d Quality] MVX_L0 AVG : %d (%lld/%d)\n",
		pic_number, value/blk22_mv_count,
		value, blk22_mv_count);
#endif
#ifdef SUPPORT_NODE
	node->avg_mv = value/blk22_mv_count;
#endif
...
static void vh264_set_params(struct work_struct *work)
{
	...
	int i, mb_mv_byte, ret, start_addr;
	unsigned long addr;
...
/*	pr_info("video_signal_type_present_flag 0x%x\n",
	...
	if (is_4k) {
		keep_frame_size = ((mb_total << 8) + (mb_total << 7));
		buf_size += keep_frame_size;
	}
	...
	if (ret < 0) {
		...
		pr_info("malloc buffer for decoder failed\n");
		return;
	}

	start_addr = addr;
	if (is_4k)
		addr += keep_frame_size;/*keep last frame */
	...
	if (!(READ_VREG(AV_SCRATCH_F) & 0x1)) {
		for (i = 0; i < actual_dpb_size; i++) {
			...
			if (is_4k && ((addr + 7) >> 3) == disp_addr)
				addr = start_addr; /*4k keep last frame */
...
static inline bool vh264_isr_parser(struct vframe_s *vf,
		...
		unsigned int pts, int slice_type)
{
	unsigned int pts_duration = 0;

	if (h264_first_pts_ready == 0) {
		if (pts_valid == 0 || slice_type != SLICE_TYPE_I) {
		...
		pr_info("[%s:%d]duration-estimate. first pts:%u\n",
			__func__, __LINE__, pts);
	} else {
		if (h264_pts_count == h264_pts_count_pre
		&& duration_from_pts_done == 0) {
			...
			bool pts_c_25 = close_to(pts_duration,
				RATE_25_FPS,
				RATE_CORRECTION_THRESHOLD);
			/*pts_duration is close to pts_c_24/25
			*diff_dur is bigger than 1/4 * frame_dur
			*then we can use pts_duration as the temp
			*frame_dur*/
			if (pts_c_24 || pts_c_25) {
				unsigned int diff_dur =
				abs(pts_duration - frame_dur);
				if ((diff_dur * 4) > frame_dur)
		...
		if (pts_valid && (pts > h264pts1) && (h264_pts_count > 24)
				...
				h264_pts_count >= h264_pts_count_I_late)) {
			unsigned int
				old_duration = frame_dur;
			h264pts2 = pts;
			pr_info("[%s:%d]duration-estimate. pts:%u\n",
				__func__, __LINE__, pts);
...
#endif
{
	...
	if (((error_recovery_mode != 1)
		...
		for (i = 0; (i < num_frame) && (!vh264_eos); i++) {
			...
			if (!error &&
				(idr_flag || (slice_type == SLICE_TYPE_I)))
				no_idr_error_count = 0;
			...
			if (slice_type_num <=
						print_slice_type_max) {
					slice_type_num++;
				}
			time_debug = get_time_debug();
			if ((time_debug) &&
				slice_type_num == 1) {
				pr_info("CTC_KPI::Stage 3_3 %s\n",
					"first_frame_decodec cost,first_decodec_time end");
			}
#ifdef DROP_B_FRAME_FOR_1080P_50_60FPS
			...
			if (unlikely
				...
				if (pts_index < 6) {
					pts_checkout[pts_index] = pts;
					if (pts_index > 0)
						dur_diff[pts_index-1] =
						pts_checkout[pts_index] -
						pts_checkout[pts_index-1];
					if (pts_index == 5
						&& dur_diff[0] != dur_diff[1]
						&& dur_diff[1] != dur_diff[2]
						&& dur_diff[2] != dur_diff[3]
						&& dur_diff[3] != dur_diff[4]) {
						fix_duration = 1;
						pr_info("force fix duration play\n");
						}
					pts_index++;
					if (pts_index == 6 && fix_duration != 1) {
						/*pr_info("set index 0\n");*/
						pts_index = 0;
						}
				}
			...
			if (decoder_debug_flag & 0x1) {
				pr_info(
				"slice_type %x framesize %x idr %x error %x count %d\n",
						slice_type, framesize,
						idr_flag, error,
						no_idr_error_count);
				pr_info(" prog %x pic_struct %x offset %x\n",
						prog_frame, pic_struct,
						b_offset);
			}
			...
			if (pts_valid) {
				...
				} else if (trickmode_i == 0) {
					/* +[SE] [BUG][BUG-171395][kwanleong.chau]
					modified:setthe real pts in trick mode which
					player cant get real time */
					pts = pts_inc_by_duration(&pts,
						&last_pts_remainder);
				}
				pts_valid = 1;
			...
			/*use force di only when receiver isnot ionvideo*/
			if (!vf_receiver
				|| strcmp(vf_receiver->name, "ionvideo")) {
			if ((dec_control &
				DEC_CONTROL_FLAG_FORCE_2997_1080P_INTERLACE)
				&& (frame_width == 1920)
				&& (frame_height >= 1080)
			...
			gvs->error_count = current_error_count;
			...
			if ((!force_interlaced_frame)
				...
				if ((error_recovery_mode_use & 2) && error) {
					...
					KPI: APK: modify the print position
					to solve the error
					of the calculation KPI.*/
					/*if (slice_type_num <=
							print_slice_type_max) {
						slice_type_num++;
						pr_info("prog send vf slice_type %x\n",
						slice_type);
					}*/
					if ((time_debug) &&
						slice_type_num == 1) {
						pr_info("CTC_KPI::Stage 4_1 decode->di 0 cost,send_out_time\n");
					}
				}
			} else {
				if (pic_struct_present
				...
				/*if (slice_type_num <=
						print_slice_type_max) {
					slice_type_num++;
					pr_info("interlace vf slice_type %x\n",
					slice_type);
				}*/
				if ((time_debug) && slice_type_num == 1) {
						pr_info("CTC_KPI::Stage 4_1 decode->di 0 cost,send_out_time\n");
				}
			}
		}
		...
		fatal_error_flag = DECODER_FATAL_ERROR_UNKNOW;
		/* this is fatal error, need restart */
		pr_info("fatal error happend, REG_VAL:0x%x\n", cpu_cmd);
		amvdec_stop();
		...
		if (pts_lookup_offset_us64
			...
			&first_frame_size, 0, &first_pts64) == 0)
		...
		WRITE_VREG(AV_SCRATCH_0, 0);
			...
			if (pts_lookup_offset_us64 (PTS_TYPE_VIDEO,
				b_offset, &pts, &frame_size, 0, &pts_us64) != 0)
...
static void vh264_put_timer_func(unsigned long arg)
{
	...
	if (atomic_read(&vh264_active) && vh264_running) {
		uVLD_Status = READ_VREG(VLD_STATUS);
		search_active = uVLD_Status & 0x3F;
	}

	if (search_active == 0x2b) {
		search_count++;
		if (search_count >= search_start_code_over) {
			search_count = 0;
			pr_info("------VLD_STATUS:0x%x is error, should force reset decoder!\n",
				uVLD_Status);
			amvdec_stop();
			vh264_running = 0;
			schedule_work(&error_wd_work);
		...
		if (print_search_count > 0) {
			if (search_count != 0) {
				pr_info("reset search_count, search_active:0x%x, search_count:%d\n",
					search_active, search_count);
				print_search_count--;
			}
		}
		search_count = 0;
	...
	vh264_isr();
#endif
	if (timer_status_debug) {
		pr_info("state %d display_q %d delay_display_q %d recycle_q %d  inactive_timeout %d prev_buflevel  0x%x  level 0x%x\n",
			state, kfifo_len(&display_q), kfifo_len(&delay_display_q),
			kfifo_len(&recycle_q), inactive_timeout, prev_buflevel, READ_VREG(VLD_MEM_VIFIFO_LEVEL));
	}
	...
	if (vh264_stream_switching_state != SWITCHING_STATE_OFF)
		...
		if (wait_buffer_status) {
			...
			if (state == RECEIVER_INACTIVE &&
				...
				(!(kfifo_len(&display_q) +
				kfifo_len(&delay_display_q)) && ( prev_buflevel > 0x400000) &&
				(prev_buflevel == READ_VREG(VLD_MEM_VIFIFO_LEVEL)))) {
				inactive_timeout++;
				if (inactive_timeout > 200) {
					pr_info("$$$$ inactive > 200\n");
					inactive_timeout = 0;
					amvdec_stop();
	...
	prev_buflevel = READ_VREG(VLD_MEM_VIFIFO_LEVEL);
	if ((error_recovery_mode == 1) && (READ_VREG(AV_SCRATCH_D) > 0x1000)) {
				pr_info("Too much error. need a fatal reset \n");
				amvdec_stop();
				WRITE_VREG(AV_SCRATCH_D, 0);
				schedule_work(&error_wd_work);
	}
	...
	if (decoder_force_reset) {
		pr_info("force_local_reset now\n");
...
{
	vstatus->frame_width = gvs->frame_width;
	vstatus->frame_height = gvs->frame_height;
	if (gvs->frame_dur != 0)
		vstatus->frame_rate = 96000 / gvs->frame_dur;
	else
		vstatus->frame_rate = -1;
	vstatus->error_count = gvs->error_count;
	vstatus->status = stat;
	if (fatal_error_reset)
		vstatus->status |= fatal_error_flag;
	vstatus->bit_rate = gvs->bit_rate;
	vstatus->frame_dur = gvs->frame_dur;
...
#define DEBUGGET_FW(t, name, buf, size, ret)\
	...
	if (!firmwareloaded) {
	 ...
	 } else if (cpu_type >= MESON_CPU_MAJOR_ID_G12A) {
		r0 = amvdec_loadmc_ex(VFORMAT_H264, "g12a_vh264_mc", NULL);
		r1 = get_decoder_firmware_data(VFORMAT_H264,
			"g12a_vh264_header_mc",
			(u8 *) mc_cpu_addr + MC_OFFSET_HEADER, MC_SWAP_SIZE);
...
u32 get_vh264_set_cmd1_time(void)
{
	return set_cmd1_time;
}
EXPORT_SYMBOL(get_vh264_set_cmd1_time);
u32 get_vh264_first_decoded_time(void)
{
	return first_decoded_time;
}
EXPORT_SYMBOL(get_vh264_first_decoded_time);
u32 get_vh264_firstcheckin_cmd1_time(void)
{
	return firstcheckin_cmd1_time;
}
EXPORT_SYMBOL(get_vh264_firstcheckin_cmd1_time);
u32 get_vh264_firstcheckout_decoded_time(void)
{
	return firstcheckout_decoded_time;
}
EXPORT_SYMBOL(get_vh264_firstcheckout_decoded_time);
static void error_do_work(struct work_struct *work)
{
	...
	if (atomic_read(&vh264_active)) {
		...
		pr_info("LEVEL:0x%x, WP:0x%x, RP:0x%x\n",
...
static int amvdec_h264_probe(struct platform_device *pdev)
{
	...
	if ((frame_width > 1920) && (frame_height > 1080)) {
		is4K = true;
		WRITE_DMCREG(DMC_AM0_CHAN_CTRL,
			READ_DMCREG(DMC_AM0_CHAN_CTRL) | (1 << 18));
		WRITE_DMCREG(DMC_AXI1_CHAN_CTRL,
			READ_DMCREG(DMC_AXI1_CHAN_CTRL) | (1 << 16));
		WRITE_DMCREG(DMC_AXI2_CHAN_CTRL,
			READ_DMCREG(DMC_AXI2_CHAN_CTRL) | (1 << 16));
	}
...
static struct mconfig h264_configs[] = {
	...
	MC_PU32("h264_pts_count_I_late", &h264_pts_count_I_late),
	MC_PU32("h264_pts_count_pre", &h264_pts_count_pre),
};
...
/****************************************/
module_param(h264_pts_count_I_late, uint, 0664);
MODULE_PARM_DESC(h264_pts_count_I_late, "\n amvdec_h264 h264_pts_count_I_late\n");
module_param(h264_pts_count_pre, uint, 0664);
MODULE_PARM_DESC(h264_pts_count_pre, "\n amvdec_h264 h264_pts_count_pre\n");
...
module_param(print_slice_type_max, uint, 0664);
MODULE_PARM_DESC(print_slice_type_max,
				"\n print slice type max num\n");
...
module_param(print_search_count, uint, 0664);
MODULE_PARM_DESC(print_search_count,
	"\n print print_search_count\n");

module_param(search_count, uint, 0664);
MODULE_PARM_DESC(search_count,
	"\n print search_count\n");

module_param(search_start_code_over, uint, 0664);
MODULE_PARM_DESC(search_start_code_over,
	"\n print search_start_code_over\n");

module_param(timer_status_debug, uint, 0664);
MODULE_PARM_DESC(timer_status_debug,
	"\n timer_status_debug\n");
