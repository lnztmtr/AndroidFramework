...
static void set_frame_info(struct vframe_s *vf)
{
	...
	vf->flag = 0;
	vf->vdec_type = VFORMAT_JPEG;
}

static irqreturn_t vmjpeg_isr(int irq, void *dev_id)
{
	u32 reg, offset, pts, frame_size, pts_valid = 0;
...
static s32 vmjpeg_init(void)
{
	...
	if (get_cpu_type() > MESON_CPU_MAJOR_ID_G12A) {
		if (amvdec_loadmc_ex(VFORMAT_MJPEG,
				"vmjpeg_g12b_mc", NULL) < 0) {
			amvdec_disable();
			pr_info("load vjmpeg g12b fw failed\n");
			return -EBUSY;
		}
	} else {
		if (amvdec_loadmc_ex(VFORMAT_MJPEG, "vmjpeg_mc", NULL) < 0) {
			amvdec_disable();
			pr_info("load vjmpeg fw failed\n");
			return -EBUSY;
		}
	}
