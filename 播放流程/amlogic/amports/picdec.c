/*
 * drivers/amlogic/amports/picdec.c
 *
...
static int output_format_mode = 0;
...
int picdec_pre_process(void)
{
	...
	/* resource_size_t offset = picdec_device.assit_buf_start -
...
static int __init picdec_init_module(void)
{
	...
	if (err) {
		...
		pr_info(KERN_ERR "Failed to register picdec driver (error=%d\n",
			   err);
