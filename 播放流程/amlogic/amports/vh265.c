...
static u32 time_debug;
static u32 decoder_debug_flag_h265;
...
#define H265_DEBUG_PROFILE                  0x08
...
static u32 dynamic_buf_num_margin_default = 10;
...
bit 6: 0, do not check head error
	1, check head error
bit 7: 0, check over decoding
	1, do not check over decoding
...
struct hevc_state_s {
	...
	int retry_again_counter;
	int pic_leak_counter0;
	int pic_leak_counter1;
...
static void dump_profile(struct vdec_s *vdec, const char *caller)
{
	struct hevc_state_s *hevc =
		(struct hevc_state_s *)vdec->private;	int chunk_size;
	int chunk_cnt = 0;
	if (vdec_frame_based(vdec))
		chunk_cnt =
			vdec_input_chunk_size(&vdec->input, &chunk_size);
	else {
		u32 rp, wp;
		chunk_cnt = 1;
		...
		if (wp < rp)
			chunk_size = vdec->input.size + wp - rp;
		else
			chunk_size = wp - rp;
	}

	hevc_print(hevc, 0,
		"%s, disp_q num %d, input chunk count %d, sizd %d\n",
		caller,
...
#else
	return dynamic_buf_num_margin_default;
	return (hevc->m_ins_flag
		&& dynamic_buf_num_margin_default) ?
			hevc->dynamic_buf_num_margin :
			dynamic_buf_num_margin_default;
#endif
}
	...
	if (hevc->i_only & 0x4) {
		...
		for (i = 0; i < MAX_REF_PIC_NUM; i++) {
			...
			if (pic->slice_type == 2 &&
				...
				pr_info("VH265: output first frame\n");
			}
		}
...
static void hevc_config_work_space_hw(struct hevc_state_s *hevc)
{
	...
	if (hevc->mmu_enable) {
		if (get_cpu_type() >= MESON_CPU_MAJOR_ID_G12A) {
			WRITE_VREG(HEVC_ASSIST_MMU_MAP_ADDR,
				hevc->frame_mmu_map_phy_addr);
			hevc_print(hevc, H265_DEBUG_BUFMGR_MORE,
				"write HEVC_ASSIST_MMU_MAP_ADDR\n");
		} else
			WRITE_VREG(H265_MMU_MAP_BUFFER,
				hevc->frame_mmu_map_phy_addr);
	}
		...
		do {
			...
			if (pic_display) {
				if ((pic_display->error_mark &&
					...
					if (!hevc->m_ins_flag &&
							pic_display->error_mark)
						gvs->error_frame_count++;
					if (hevc->i_only & 0x1
						&& pic_display->
						slice_type != 2) {
						pic_display->output_ready = 0;
...
#endif
		picture->avg_mv = value/blk22_mv_count;
		...
		int decode_pic_begin)
{
	...
	if (hevc->wait_buf == 0) {
		...
		if (time_debug && (slice_type_num == 0) &&
			(hevc->slice_type == 2)) {
			set_cmd1_time = jiffies_to_msecs(jiffies);
			pr_info("hevc get key frame:%u\n", set_cmd1_time);
			first_checkin_vpts_time = get_first_checkin_vpts_time();
			firstcheckin_cmd1_time = set_cmd1_time -
				first_checkin_vpts_time;
			pr_info("Stage 5 first_checkin->cmd1 cost:%u\n",
				firstcheckin_cmd1_time);
		}
		...
		if (rpm_param->p.slice_segment_address == 0) {
			...
			if (hevc->m_ins_flag) {
				if (hevc->decoding_pic) {
					hevc->pic_leak_counter0++;
					if (hevc->pic_leak_counter0 < 20) {
						hevc_print(hevc, 0,	"Pic leak p0, last poc %d, cur_poc %d\n",
							hevc->decoding_pic->POC,
							hevc->curr_POC);
					} else if (hevc->pic_leak_counter0 == 20
						|| hevc->pic_leak_counter0 == 25
						|| hevc->pic_leak_counter0 == 30) {
						/*vh265_dump_state(hw_to_vdec(hevc));*/
						;
					}
					hevc->decoding_pic->error_mark = 0;
					hevc->decoding_pic->output_ready = 0;
	...
	if (hevc->wait_buf == 1) {
			...
			if (hevc->m_ins_flag) {
				if (hevc->decoding_pic) {
					hevc->pic_leak_counter1++;
					if (hevc->pic_leak_counter1 < 20) {
						hevc_print(hevc, 0, "Pic leak p1, last poc %d, cur_poc %d\n",
									hevc->decoding_pic->POC,
									hevc->curr_POC);
					} else if (hevc->pic_leak_counter1 == 20
					|| hevc->pic_leak_counter1 == 25
					|| hevc->pic_leak_counter1 == 30) {
						/*vh265_dump_state(hw_to_vdec(hevc));*/
						;
					}
					hevc->decoding_pic->error_mark = 0;
...
static struct vframe_s *vh265_vf_peek(void *op_arg)
{
	...
	if (debug & H265_DEBUG_PROFILE)
		dump_profile(vdec, __func__);
...
static int prepare_display_buf(struct hevc_state_s *hevc, struct PIC_s *pic)
{
	...
	u32 first_checkout_vpts_time = 0;
	if (force_disp_pic_index & 0x100) {
		/*recycle directly*/
		pic->output_ready = 0;
		return -1;
...
/*SUPPORT_10BIT*/
		...
		*/
		...
		if (!hevc->m_ins_flag) {
			/*count info*/
			vdec_count_info(gvs, pic->error_mark, stream_offset);
		}
		vf_notify_receiver(hevc->provider_name,
				VFRAME_EVENT_PROVIDER_VFRAME_READY, NULL);
		if ((time_debug) && (slice_type_num == 0)) {
			first_decoded_time = jiffies_to_msecs(jiffies);
			pr_info("first frame decoded:%u\n", first_decoded_time);
			first_checkout_vpts_time =
				get_first_checkout_vpts_time();
			firstcheckout_decoded_time = first_decoded_time -
				first_checkout_vpts_time;
			pr_info("Stage 7 first frame decoded cost:%u\n",
			...
			pr_info("CTC_KPI::Stage 5_1 %s\n",
				"di->first_pic cost,first_frame_output_time");
			slice_type_num = 1;
		}
	}
...
static int hevc_recover(struct hevc_state_s *hevc)
{
	...
	/*[SE] [BUG][BUG-170485][chuanqi.wang] avoid dead lock when remove hold the lock"*/
	if (!hevc->init_flag) {
		hevc_print(hevc, 0, "h265 has stopped, recover return!\n");
		return ret;
	}
	if (!mutex_trylock(&vh265_mutex)) {
		hevc_print(hevc, 0, "vh265_mutex is busy , recover return!\n");
		return ret;
	}
#if 0
...
pic_done:
			...
			hevc->pic_leak_counter0 = 0;
			hevc->pic_leak_counter1 = 0;
	...
	if (use_cma &&
		...
		if (hevc->pic_w == 0 || hevc->pic_h == 0
			...
			if ((nal_skip_policy == 0) &&
				...
				hevc->mmu_box = NULL;*/
				hevc_print(hevc, 0,
					"show mosaic pic, not 10bit, set dw 0x10\n");
			} else {
				if (hevc->mmu_enable
					&& ((get_double_write_mode(hevc) & 0x10) == 0)) {
					hevc->mmu_box = decoder_mmu_box_alloc_box(DRIVER_NAME,
						...
						MAX_REF_PIC_NUM,
						64 * SZ_1M,
						vdec_secure(hw_to_vdec(hevc)) ? CODEC_MM_FLAGS_TVP : 0
						);
					if (!hevc->mmu_box) {
						pr_err("h265 alloc mmu box failed!!\n");
						return -1;
...
#ifdef MULTI_INSTANCE_SUPPORT
		if (hevc->m_ins_flag) {
			hevc->retry_again_counter++;
			if (hevc->retry_again_counter > 16) {
				hevc_print(hevc, 0,	"retry too much, flush output\n");
				flush_output(hevc, NULL);
				hevc->retry_again_counter = 0;
			}
			hevc->wait_buf = 0;
			hevc->dec_result = DEC_RESULT_AGAIN;
			amhevc_stop();
...
{
#ifdef MULTI_INSTANCE_SUPPORT
	struct hevc_state_s *hevc = vdec->private;
#else
	struct hevc_state_s *hevc = &gHevc;
#endif
...
u32 get_vh265_set_cmd1_time(void)
{
	return set_cmd1_time;
}
EXPORT_SYMBOL(get_vh265_set_cmd1_time);
u32 get_vh265_first_decoded_time(void)
{
	return first_decoded_time;
}
EXPORT_SYMBOL(get_vh265_first_decoded_time);
u32 get_vh265_firstcheckin_cmd1_time(void)
{
	return firstcheckin_cmd1_time;
}
EXPORT_SYMBOL(get_vh265_firstcheckin_cmd1_time);
u32 get_vh265_firstcheckout_decoded_time(void)
{
	return firstcheckout_decoded_time;
}
EXPORT_SYMBOL(get_vh265_firstcheckout_decoded_time);
...
static int vh265_local_init(struct hevc_state_s *hevc)
{
	...
	hevc->saved_resolution = 0;
        ...
        memset(&hevc->vframe_qos, 0, sizeof(hevc->vframe_qos));
	...
	hevc->frame_dur =
		(hevc->vh265_amstream_dec_info.rate ==
		 0) ? 3600 : (96000 / hevc->vh265_amstream_dec_info.rate);
#ifndef CONFIG_MULTI_DEC
	gvs->frame_dur = hevc->frame_dur;
#endif
...
static s32 vh265_init(struct hevc_state_s *hevc)
{
	...
	} else if (hevc->mmu_enable && (get_cpu_type() >=
...
static void vh265_work(struct work_struct *work)
{
	...
	if (((hevc->dec_result == DEC_RESULT_GET_DATA) ||
		...
		*/
		...
		hevc_print(hevc, 0, "%s: disable mbox int %x\n",
				__func__, (hevc->stat & STAT_ISR_REG));
		if (hevc->stat & STAT_ISR_REG) {
...
static unsigned long run_ready(struct vdec_s *vdec, unsigned long mask)
{
	...
	if ((!vdec_frame_based(vdec)) &&
			((hevc->fatal_error & DECODER_FATAL_ERROR_SIZE_OVERFLOW)
			|| (hevc->fatal_error & DECODER_FATAL_ERROR_UNKNOW)
...
static int ammvdec_h265_probe(struct platform_device *pdev)
{
	...
	if (((get_dbg_flag(hevc) & IGNORE_PARAM_FROM_CONFIG) == 0) &&
		...
		if (get_config_int(pdata->config, "hevc_buf_margin",
			...
			hevc_print(hevc, 0,
				"config0 hevc->dynamic_buf_num_margin: %d\n",
				hevc->dynamic_buf_num_margin);
		} else {
			hevc->dynamic_buf_num_margin =
				dynamic_buf_num_margin_default;
			hevc_print(hevc, 0,
				"config1 hevc->dynamic_buf_num_margin: %d\n",
				hevc->dynamic_buf_num_margin);
		}
		...
		hevc->vh265_amstream_dec_info.height = 0;
		hevc->vh265_amstream_dec_info.rate = 30;
		hevc->dynamic_buf_num_margin = dynamic_buf_num_margin_default;
		hevc->double_write_mode = double_write_mode;
		hevc_print(hevc, 0,
				"config2 hevc->dynamic_buf_num_margin: %d\n",
				hevc->dynamic_buf_num_margin);
	}
...
static struct mconfig h265_configs[] = {
	...
	MC_PU32("decoder_debug_flag_h265", &decoder_debug_flag_h265),
...
module_param(decoder_debug_flag_h265, uint, 0664);
MODULE_PARM_DESC(decoder_debug_flag_h265, "\n amvdec_h265 decoder_debug_flag\n");
...
module_param(dynamic_buf_num_margin_default, uint, 0664);
MODULE_PARM_DESC(dynamic_buf_num_margin_default, "\n dynamic_buf_num_margin_default\n");
