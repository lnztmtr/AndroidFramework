...
#define DEC_CONTROL_FLAG_FORCE_2343_1080P_INTERLACE 0x0001
static u32 dec_control = DEC_CONTROL_FLAG_FORCE_2343_1080P_INTERLACE;
...
#endif
{
	...
	u32 buffer_index;
	u32 picture_struct;
	unsigned int pts, pts_valid = 0, offset, frame_size;
	u64 pts_us64;
	...
	if (reg) {
		...
		if (pts_by_offset) {
			...
			if (pts_lookup_offset_us64(PTS_TYPE_VIDEO, offset, &pts,
				...
				pts_missed++;
#endif
				pr_info("loop failed offset = 0x%x\n", offset);
			}
		}
		...
		if ((dec_control & DEC_CONTROL_FLAG_FORCE_2343_1080P_INTERLACE)
		...
		} else if (reg & INTERLACE_FLAG || force_interlaced_frame) {
			/* interlace */
			throw_pb_flag = 0;
