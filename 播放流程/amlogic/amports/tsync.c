...
static int checkin_vpts;
static int apts_error_num;
static int vpts_error_num;
static int apts_dis_num;
static int pts_dis_num;
char audio_discontinue_info[512];
char video_discontinue_info[512];
...
static int video_hold_threshold = 3*90000;
static int start_video_delay;
...
static int print_pts_num_max;
static int print_vpts_time = 3;
...
int tsync_get_discontinue_info(struct pts_discontinue_info *dis_info)
{
	if (pts_dis_num == 0) {
		return 0;
	} else {
	int num = pts_dis_num;
	memcpy(dis_info, pts_dis_info,
		pts_dis_num *
		sizeof(struct pts_discontinue_info));
	memset(pts_dis_info, 0,
		pts_dis_num *
		sizeof(struct pts_discontinue_info));
	pts_dis_num = 0;
	return num;
...
int codec_video_start (void) {
	return tsync_video_started;
}
EXPORT_SYMBOL(codec_video_start);
...
void tsync_avevent_locked(enum avevent_e event, u32 param)
{
		...
		if (slowsync_enable == 1) {	/* slow sync enable */
			...
			if (tsync_stat == TSYNC_STAT_PCRSCR_SETUP_NONE) {
				if (tsync_syncthresh
					...
					|| start_video_delay) {
					int delay = 0;
					if (start_video_delay)
						delay =  start_video_delay;
					else
						delay = video_hold_threshold;
					timestamp_pcrscr_set(param -
						delay);
					pr_info("scr delay:%d, pcr = 0x%x, vpts = 0x%x\n",
	...
	case VIDEO_STOP:
		set_video_dropnumber(0);
		set_video_checkin_number(0);
		tsync_stat = TSYNC_STAT_PCRSCR_SETUP_NONE;
	...
	case VIDEO_TSTAMP_DISCONTINUITY: {
		unsigned oldapts = timestamp_apts_get();
		unsigned oldvpts = timestamp_vpts_get();
		unsigned oldpcr = timestamp_pcrscr_get();
		...
		if ((abs(param - oldvpts) > tsync_av_threshold_min)
		...
		if (tsync_mode != TSYNC_MODE_VMASTER && param - t > 60*90000
				&& (!get_vsync_pts_inc_mode())) {
			tsync_mode_switch('V', abs(param - t),
					param-oldvpts);
		}
		...
		vpts_error_num++;
		pts_dis_info[pts_dis_num].pts_dis_format
			= PTS_DISCONTINUE_VIDEO;
		pts_dis_info[pts_dis_num].vpts_dis_num
			= vpts_error_num;
		pts_dis_info[pts_dis_num].old_mode = oldmod;
		pts_dis_info[pts_dis_num].old_apts = oldapts;
		pts_dis_info[pts_dis_num].old_vpts = oldvpts;
		pts_dis_info[pts_dis_num].old_pcr = oldpcr;
		pts_dis_info[pts_dis_num].new_mode
			= (enum tsync_mode_e)tsync_mode;
		pts_dis_info[pts_dis_num].new_apts = timestamp_apts_get();
		pts_dis_info[pts_dis_num].new_vpts = timestamp_vpts_get();
		pts_dis_info[pts_dis_num].new_pcr = timestamp_pcrscr_get();
	}
	break;
	...
	case AUDIO_TSTAMP_DISCONTINUITY: {
		...
		amlog_level(LOG_LEVEL_ATTENTION,
				...
				t, param);
		if ((abs(param - oldapts) > tsync_av_threshold_min)
			&& (!get_vsync_pts_inc_mode())) {
			apts_discontinue = 1;
		...
		if (dobly_avsync_test) {
			if (get_vsync_pts_inc_mode()
				...
				(((int)(timestamp_apts_get() - t) >
				  (int)60 * TIME_UNIT90K / 1000)
				 || (int)(t - timestamp_apts_get()) >
				 (int)100 * TIME_UNIT90K / 1000)) {
				pr_info
...
int tsync_print_pts_max_num(void)
{
	return print_pts_num_max;
}
EXPORT_SYMBOL(tsync_print_pts_max_num);

int tsync_print_vpts_time(void)
{
	return print_vpts_time;
}
EXPORT_SYMBOL(tsync_print_vpts_time);
...
int tsync_get_video_hold_threshold(void)
{
	return video_hold_threshold;
}
EXPORT_SYMBOL(tsync_get_video_hold_threshold);

int tsync_set_video_hold_threshold(int mode)
{
	return video_hold_threshold = mode;
}
EXPORT_SYMBOL(tsync_set_video_hold_threshold);

int tsync_set_checkin_vpts(int val)
{
	checkin_vpts = val;
	return 0;
}
EXPORT_SYMBOL(tsync_set_checkin_vpts);
...
static ssize_t show_print_pts_num_max(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", print_pts_num_max);
}

static ssize_t store_print_pts_num_max(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	unsigned pts_num_max;
	ssize_t r;

	r = sscanf(buf, "%d", &pts_num_max);
	if ((r != 1))
		return -EINVAL;

	print_pts_num_max = pts_num_max;

	return size;
}

static ssize_t show_print_vpts_time(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", print_vpts_time);
}

static ssize_t store_print_vpts_time(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	unsigned vpts_time;
	...
	r = sscanf(buf, "%d", &vpts_time);
	if ((r != 1))
		return -EINVAL;

	print_vpts_time = vpts_time;
...
static ssize_t show_last_checkout_apts(struct class *class,
		struct class_attribute *attr, char *buf)
{
	unsigned int last_apts;
	last_apts = get_last_checkout_pts(PTS_TYPE_AUDIO);
	return sprintf(buf, "0x%x\n", last_apts);
}
...
static ssize_t show_last_checkout_vpts(struct class *class,
		struct class_attribute *attr, char *buf)
{
	unsigned int last_vpts;
	last_vpts = get_last_checkout_pts(PTS_TYPE_VIDEO);
	return sprintf(buf, "0x%x\n", last_vpts);
}

static ssize_t show_checkin_vpts(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n", checkin_vpts);
}
...
static ssize_t show_start_video_delay(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", start_video_delay);
}

static ssize_t store_start_video_delay(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	unsigned mode;
	...
	start_video_delay = mode;
	return size;
}

static ssize_t show_video_hold_threshold(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n", tsync_get_video_hold_threshold());
}

static ssize_t store_video_hold_threshold(struct class *class,
		struct class_attribute *attr,
		const char *buf, size_t size)
{
	unsigned mode;
	...
	tsync_set_video_hold_threshold(mode);
	return size;
}

static ssize_t show_apts_error_num(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n", apts_error_num);
}

static ssize_t show_vpts_error_num(struct class *class,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "0x%x\n", vpts_error_num);
}

static ssize_t show_audio_discontinue_info(struct class *class,
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", audio_discontinue_info);
}

static ssize_t show_video_discontinue_info(struct class *class,
	struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%s\n", video_discontinue_info);
}
...
static struct class_attribute tsync_class_attrs[] = {
	...
	__ATTR(pcr_recover, S_IRUGO | S_IWUSR | S_IWGRP, show_pcr_recover,
	store_pcr_recover),
	__ATTR(discontinue, S_IRUGO | S_IWUSR, show_discontinue,
	store_discontinue),
	__ATTR(debug_pts_checkin, S_IRUGO | S_IWUSR, show_debug_pts_checkin,
	store_debug_pts_checkin),
	__ATTR(debug_pts_checkout, S_IRUGO | S_IWUSR, show_debug_pts_checkout,
	store_debug_pts_checkout),
	__ATTR(debug_video_pts, S_IRUGO | S_IWUSR, show_debug_vpts,
	store_debug_vpts),
	__ATTR(debug_audio_pts, S_IRUGO | S_IWUSR, show_debug_apts,
	store_debug_apts),
	__ATTR(av_threshold_min, S_IRUGO | S_IWUSR | S_IWGRP,
	show_av_threshold_min, store_av_threshold_min),
	__ATTR(av_threshold_max, S_IRUGO | S_IWUSR | S_IWGRP,
	show_av_threshold_max, store_av_threshold_max),
	__ATTR(last_checkin_apts, S_IRUGO | S_IWUSR, show_last_checkin_apts,
	NULL),
	__ATTR(firstvpts, S_IRUGO | S_IWUSR, show_firstvpts, NULL),
	__ATTR(vpause_flag, S_IRUGO | S_IWUSR, show_vpause_flag,
	store_vpause_flag),
	__ATTR(slowsync_enable, S_IRUGO | S_IWUSR, show_slowsync_enable,
	store_slowsync_enable),
	__ATTR(startsync_mode, S_IRUGO | S_IWUSR, show_startsync_mode,
	store_startsync_mode),
	__ATTR(firstapts, S_IRUGO | S_IWUSR | S_IWGRP, show_firstapts,
	store_firstapts),
	__ATTR(checkin_firstvpts, S_IRUGO | S_IWUSR, show_checkin_firstvpts,
	NULL),
	__ATTR(apts_error_num, S_IRUGO | S_IWUSR, show_apts_error_num,
	NULL),
	__ATTR(vpts_error_num, S_IRUGO | S_IWUSR, show_vpts_error_num,
	NULL),
	__ATTR(print_pts_num_max, S_IRUGO | S_IWUSR, show_print_pts_num_max,
	store_print_pts_num_max),
	__ATTR(print_vpts_time, S_IRUGO | S_IWUSR, show_print_vpts_time,
	store_print_vpts_time),
	__ATTR(video_hold_threshold, S_IRUGO | S_IWUSR,
	show_video_hold_threshold, store_video_hold_threshold),
	__ATTR(last_checkout_apts, S_IRUGO | S_IWUSR, show_last_checkout_apts,
	NULL),
	__ATTR(last_checkin_vpts, S_IRUGO | S_IWUSR, show_last_checkin_vpts,
	NULL),
	__ATTR(last_checkout_vpts, S_IRUGO | S_IWUSR, show_last_checkout_vpts,
	NULL),
	__ATTR(checkin_vpts, S_IRUGO | S_IWUSR, show_checkin_vpts,
	NULL),
	__ATTR(audio_discontinue_info, S_IRUGO | S_IWUSR,
	show_audio_discontinue_info, NULL),
	__ATTR(video_discontinue_info, S_IRUGO | S_IWUSR,
	show_video_discontinue_info, NULL),
	__ATTR(start_video_delay, S_IRUGO | S_IWUSR,
	show_start_video_delay, store_start_video_delay),
	__ATTR_NULL
};
...
int tsync_store_fun(const char *trigger, int id, const char *buf, int size)
{
	...
	case 22:return -1;
	case 23:return -1;
	case 24:return store_print_pts_num_max(NULL, NULL, buf, size);
	case 25:return store_video_hold_threshold(NULL, NULL, buf, size);
	case 26:return -1;
	case 27:return -1;
	case 28:return -1;
	case 29:return -1;
...
int tsync_show_fun(const char *trigger, int id, char *sbuf, int size)
{
	...
	case 21:
		...
		break;
	case 22:
		ret = show_apts_error_num(NULL, NULL, buf);
		break;
	case 23:
		ret = show_vpts_error_num(NULL, NULL, buf);
		break;
	case 24:
		ret = show_print_pts_num_max(NULL, NULL, buf);
		break;
	case 25:
		ret = show_video_hold_threshold(NULL, NULL, buf);
		break;
	case 26:
		ret = show_last_checkout_apts(NULL, NULL, buf);
		break;
	case 27:
		ret = show_last_checkin_vpts(NULL, NULL, buf);
		break;
	case 28:
		ret = show_last_checkout_vpts(NULL, NULL, buf);
		break;
	case 29:
		ret = show_checkin_vpts(NULL, NULL, buf);
		break;
	default:
		ret = -1;
	}
...
static struct mconfig tsync_configs[] = {
	...
	MC_FUN_ID("apts_error_num", tsync_show_fun, NULL, 22),
	MC_FUN_ID("vpts_error_num", tsync_show_fun, NULL, 23),
	MC_FUN_ID("print_pts_num_max", tsync_show_fun, tsync_store_fun, 24),
	MC_FUN_ID("video_hold_threshold", tsync_show_fun, tsync_store_fun, 25),
	MC_FUN_ID("last_checkout_apts", tsync_show_fun, NULL, 26),
	MC_FUN_ID("last_checkin_vpts", tsync_show_fun, NULL, 27),
	MC_FUN_ID("last_checkout_vpts", tsync_show_fun, NULL, 28),
	MC_FUN_ID("checkin_vpts", tsync_show_fun, NULL, 29),
	MC_FUN_ID("print_vpts_time", tsync_show_fun, tsync_store_fun, 30),
};
