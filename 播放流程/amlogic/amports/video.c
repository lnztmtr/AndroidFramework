...
static int amplayer_enable;
static int amplayer_flag;
...
int64_t video_drop_number = 0;
int64_t total_frame_number = 0;
unsigned int current_vpts = 0;
int sys_time_count = 0;
...
static u32 di_firstpic_time;
static u32 cmd1_firstpic_time;
static u32 init_firstpic_time;
static u32 lststop_firstpic_time;
static u32 press_firstpic_time;
static u32 remove_waitkey_time;
static u32 first_frame_type;
...
/* disable video */
static u32 disable_video = VIDEO_DISABLE_FORNEXT;
...
enum FRAME_FORMAT get_frame_format_for_avinfo(void)
{
	enum FRAME_FORMAT frame_format = FRAME_FORMAT_UNKNOW;
	if (cur_dispbuf) {
		if ((cur_dispbuf->type & VIDTYPE_TYPEMASK) ==
		    VIDTYPE_INTERLACE_TOP)
			frame_format = FRAME_FORMAT_INTERLACE;
		else if ((cur_dispbuf->type & VIDTYPE_TYPEMASK) ==
			 VIDTYPE_INTERLACE_BOTTOM)
			frame_format = FRAME_FORMAT_INTERLACE;
		else
			frame_format = FRAME_FORMAT_PROGRESS;

		if (cur_dispbuf->type & VIDTYPE_COMPRESS)
			frame_format = FRAME_FORMAT_UNKNOW;
	}
	return frame_format;
}
EXPORT_SYMBOL(get_frame_format_for_avinfo);
...
static u32 pcr_slow_factor = 0;

/* frame rate calculate */
static u32 last_frame_count;
static u32 frame_count;
static u32 frame_count_field = 0;
static u32 toggle_count = 0;
...
static u32 manual_duration;
static u32 manual_last_pcr;
static u32 manual_dur_rem;
u32 manual_duration_change = 0;
u32 ctc_fix_dur = 0;
...
static int is_video_data_avaliable(void)
{
	int ret = 0;
	if ((zoom_end_x_lines > zoom_start_x_lines) &&
...
{
	u32 first_picture = 0;
	u32 first_picture_time = 0;
	u32 first_frame_output_time = 0;
	u32 open_time = 0;
	u32 close_time = 0;
	u32 press_time = 0;
	u32 set_cmd1_time = 0;
	u32 firstcheckin_cmd1_time = 0;
	s32 init_vformat = -1;
	...
	if ((cur_dispbuf) && (cur_dispbuf != &vf_local) && (cur_dispbuf != vf)
		...
		if (cur_dispbuf->source_type == VFRAME_SOURCE_TYPE_OSD) {
			...
			if (vf->flag & VFRAME_FLAG_SINGLE_FIELD)
				frame_count_field++;
			else
				frame_count_field +=  2;
#ifdef CONFIG_VSYNC_RDMA
		...
		first_picture = 1;
		printk("----------- first_picture comming\n");
		if (cur_dispbuf != vf) {
			first_frame_toggled = 1;
			time_debug = get_time_debug();
			if (time_debug) {
				first_picture_time = jiffies_to_msecs(jiffies);
				pr_info("%u first picture coming\n",
					first_picture_time);
				first_frame_output_time =
					get_di_first_frame_output_time();
				di_firstpic_time = first_picture_time -
					first_frame_output_time;
				first_frame_type = get_di_first_frame_type();
				if (first_frame_type) {
					/*+[SE][REQ][BUG 168918][wenjie.chen]
					KPI: APK: Add KPI print information
					for H265 and MPEG2 formats.*/
					set_video_kpi_flag(1);
					pr_info("CTC_KPI::Stage 5_1 di->first_pic cost,first_picture_time\n");
				} else if (get_first_quick_output()) {
					/*+[SE][REQ][BUG 168918][wenjie.chen]
					KPI: APK: Add KPI print information
					for H265 and MPEG2 formats.*/
					set_video_kpi_flag(1);
					pr_info("CTC_KPI::Stage 5_2 di->first_pic cost,first_picture_time\n");
				} else {
					/*+[SE][REQ][BUG 168918][wenjie.chen]
					KPI: APK: Add KPI print information
					for H265 and MPEG2 formats.*/
					set_video_kpi_flag(1);
					pr_info("CTC_KPI::Stage 5_3 di->first_pic cost,first_picture_time\n");
				}
			}
		}
		rpt_tab_idx = 0;
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXBB) {
		...
		if (type & VIDTYPE_COMPRESS) {
			...
			VSYNC_WR_MPEG_REG(DI_IF1_GEN_REG3,
				((bit_mode&0x3)<<8) | (3<<4) | 3);
			if (is_meson_gxlx_cpu() || is_meson_gxlx2_cpu())
				   ...
				   struct vframe_s *next_vf, u32 dur)
{
	...
	if ((cur_vf == NULL) || (cur_dispbuf == &vf_local))
		return true;
    if(tsync_get_vpause_flag()) {
		return false;
    }
	pts = next_vf->pts;
	frame_dur = DUR2PTS(cur_vf->duration);
	pts_dur = pts - timestamp_vpts_get();
	if (pts == 0)
		dur_disp = frame_dur;
	else
		dur_disp = pts_dur;
	...
	if (dur_disp < 0) {
		...
		dur_disp = 0;
		if (first_vpts == pts &&
				pts + frame_dur == cur_vpts) {
			dur_disp = frame_dur;
			pr_info("pts:0x%x,first_vpts=0x%x, cur_vpts:0x%x\n",
					pts, first_vpts, timestamp_vpts_get());
		}
	} else {
		/*if duration and pts diff large than 20fps, treat as error*/
		if (abs(dur_disp - frame_dur) > 4500) {
			/*frame_dur correct */
			if ((frame_dur >= 1400) && (frame_dur <= 4500)) {
				dur_disp = frame_dur;
				pr_info("dur_disp discontinue,pts:0x%x vpts:0x%x\n",
						pts, timestamp_vpts_get());
			} else {
				if (abs(pts_dur) > 4500) {
					dur_disp = 3000;
					pr_info("pts_dur discontinue,pts:0x%x vpts:0x%x\n",
						pts, timestamp_vpts_get());
				}
			}
		}
			       ...
			       struct vframe_s *next_vf)
{
	...
	if (manual_duration != 0) {
		...
		dur_pts = DUR2PTS(manual_duration);

		if (manual_last_pcr == 0 || (manual_last_pcr > systime)
				|| manual_duration_change) {
			pr_info("start manual duration pcr:0x%x, last pcr:0x%x\n",
					systime, manual_last_pcr);
			pr_info("manual_duration_change:%d\n",
					manual_duration_change);
			manual_last_pcr = systime;
			manual_dur_rem = 0;
			if (manual_duration_change == 1) {
				manual_last_pcr = 0;
				manual_dur_rem = 0;
				manual_duration_change = 0;
				timestamp_pcrscr_enable(1);
			}
			return true;
		}
		if ((systime-manual_last_pcr+manual_dur_rem) >= dur_pts) {
			manual_dur_rem = systime-manual_last_pcr+
				manual_dur_rem - dur_pts;
			manual_last_pcr = systime;
			return true;
		} else{
			return false;
	...
	systime = timestamp_pcrscr_get();
	if (debug_flag & DEBUG_FLAG_PRINT_VPTS) {
		pr_info("pcr:%x,vf->idx:%x,vpts:%x,apts:%x, nvpts:%x\n", systime,
				cur_vf->index, pts,
				timestamp_apts_get(),
				timestamp_vpts_get() +
	...
	/* check video PTS discontinuity */
		 ...
		 ((next_vf->flag & VFRAME_FLAG_NO_DISCONTINUE) == 0 || tsync_vpts_discontinuity_margin() <= 90000)) {
                // NO_DISCONTINUE FLAG no need to work in video only stream
		pts =
		    timestamp_vpts_get() +
		    (cur_vf ? DUR2PTS(cur_vf->duration) : 0);
		/*bug143608 the case of vpts larger than pcr 60 seconds;[chengshun.wang]*/
		/* pr_info("system=0x%x vpts=0x%x\n", systime,
		timestamp_vpts_get()); */
		if ((int)(systime - pts) >= 0
			|| ((pts - systime) > 60 * 90000)
			/*repeate play, in vmaster state*/
			|| (int)(systime - next_vf->pts) > 30 * 90000) {
			if (next_vf->pts != 0)
...
static void dmc_adjust_for_mali_vpu(
	unsigned int width, unsigned int height, bool force_adjust)
{
...
#endif
{
	...
	if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXLX && dmc_adjust) {
		...
		if (vf) {
			...
			width = (cur_dispbuf->type & VIDTYPE_COMPRESS) ?
				cur_dispbuf->compWidth : cur_dispbuf->width;
			height = (cur_dispbuf->type & VIDTYPE_COMPRESS) ?
				cur_dispbuf->compHeight : cur_dispbuf->height;
			force_adjust =
...
#if defined(CONFIG_AM_VECM)
	if (cur_frame_par)
		amvecm_on_vs(vf, cur_frame_par->supsc1_hori_ratio,
	...
	if (vsync_pts_inc_upint) {
		...
		if (vsync_slow_factor == 1) {
			if (pcr_slow_factor == 0 || pcr_slow_factor == 1000) {
				timestamp_pcrscr_inc_scale(vsync_pts_inc_scale,
						vsync_pts_inc_scale_base);
			} else {
				int pcr_factor = pcr_slow_factor;
				timestamp_pcrscr_inc((vsync_pts_inc*pcr_factor)/1000);
			}
		} else
	...
	/* +[SE] [BUG][BUG-170890][jiwei.sun]
		added: fix render with dur for ctc video only stream */
	if (ctc_fix_dur > 0 && vf == NULL) {
		timestamp_pcrscr_set(bak_pcr);
	}
	...
	/* +[SE] [REQ][BUG-168972][jiwei.sun]
		Only render one frame under ff/fb mode */
	if (vf && trickmode_fffb) {
		if (fffb_max_count > 0
			&& toggle_count > fffb_max_count) {
			goto SET_FILTER;
		}
	}
	...
	while (vf) {
		if (vpts_expire(cur_dispbuf, vf) || show_nosync) {
			...
			if (!show_nosync && rpt_tab[0] == rpt0
				...
				rpt_tab[2] = 0x300;
				rpt_tab[3] = 0x300;
				pr_info("reset rpt_table:[1 1 3 3]\n");
			}
...
cur_dev->vpp_off,0,VPP_VD2_ALPHA_BIT,9);//vd2 alpha must set
	...
	((cur_frame_par->VPP_vd_end_lines_ + 1) >> 1) - 1;
		...
		if (get_cpu_type() >= MESON_CPU_MAJOR_ID_GXTVBB)
			vpp_set_super_scaler_regs(cur_frame_par->supscl_path,
				...
				cur_frame_par->supsc1_hori_ratio,
				cur_frame_par->supsc1_vert_ratio,
				cur_frame_par->pps_w_out,
				cur_frame_par->pps_h_out,
				vinfo->width,
				vinfo->height);
 ...
 exit:
	...
	if (((vpp_misc_save & VPP_VD1_PREBLEND) != 0)
		|| ((vpp_misc_save & VPP_VD1_POSTBLEND) != 0)) {
		if (!is_video_data_avaliable()) {
...
#ifndef PROVINCE_NINGXIA
#ifndef PROVINCE_SC
#ifndef PROVINCE_GUIZHOU
...
static ssize_t video_sr_show(struct class *cla, struct class_attribute *attr,
			       char *buf)
{
	return sprintf(buf, "super_scaler:%d,sr_core:%d\n",
		super_scaler, sr_core);
}
		...
		char *buf)
{
	...
	len += sprintf(buf + len, "pps_w_out %u.\n",
		    cur_frame_par->pps_w_out);
	len += sprintf(buf + len, "pps_w_out %u.\n",
		    cur_frame_par->pps_h_out);
#ifdef TV_3D_FUNCTION_OPEN
...
static ssize_t current_vpts_show(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", current_vpts);
}
...
static ssize_t video_pcr_slow_factor_show(struct class *cla,
					    struct class_attribute *attr,
					    char *buf)
{
	return sprintf(buf, "%d\n", pcr_slow_factor);
...
static ssize_t video_pcr_slow_factor_store(struct class *cla,
					     struct class_attribute *attr,
					     const char *buf, size_t count)
{
	size_t r;

	r = sscanf(buf, "%d", &pcr_slow_factor);

	if (debug_flag)
		pr_info("%s(%d)\n", __func__, pcr_slow_factor);
	if (r != 1)
		return -EINVAL;
...
static ssize_t timedebug_info_show(struct class *cla,
				   struct class_attribute *attr,
				   char *buf)
{
	u32 press_close_time = 0;
	u32 codec_close_time = 0;
	u32 close_init_time = 0;
	u32 codec_init_time = 0;
	u32 init_firstcheckin_time = 0;
	u32 firstcheckin_cmd1_time = 0;
	u32 cmd1_firstcheckout_time = 0;
	u32 firstcheckout_decoded_time = 0;
	u32 decoded_frame0_time = 0;
	u32 frame0_frame1_time = 0;
	u32 frame1_frame2_time = 0;
	u32 di0_firstout_time = 0;
	u32 di1_firstout_time = 0;
	u32 di2_firstout_time = 0;
	s32 init_vformat = -1;

	press_close_time = get_press_close_time();
	codec_close_time = get_codec_close_time();
	close_init_time = get_close_init_time();
	codec_init_time = get_codec_init_time();
	init_firstcheckin_time = get_init_firstcheckin_time();
	init_vformat = get_stream_vformat();
	if (init_vformat == VFORMAT_H264 ||
		init_vformat == VFORMAT_H264_4K2K) {
		firstcheckin_cmd1_time = get_vh264_firstcheckin_cmd1_time();
		firstcheckout_decoded_time =
			get_vh264_firstcheckout_decoded_time();
	} else if (init_vformat == VFORMAT_MPEG12) {
		firstcheckin_cmd1_time = get_vmpeg12_firstcheckin_cmd1_time();
		firstcheckout_decoded_time =
			get_vmpeg12_firstcheckout_decoded_time();
	} else if (init_vformat == VFORMAT_HEVC) {
		firstcheckin_cmd1_time = get_vh265_firstcheckin_cmd1_time();
		firstcheckout_decoded_time =
			get_vmpeg12_firstcheckout_decoded_time();
	} else
		pr_info("UnSupport vformat debug:%d\n",
			init_vformat);
	cmd1_firstcheckout_time = get_cmd1_firstcheckout_time();
	decoded_frame0_time = get_decoded_frame0_time();
	if (first_frame_type) {
		di0_firstout_time =  get_di0_firstout_time();
		return sprintf(buf, "total_time:%u ms, total_time(no wait keyframe):%u ms, total_time(init->fistpic):%u ms, total_time(key->firstpic):%u ms, total_time(no press key):%u ms, press_close_time:%u ms, codec_close_time:%u ms, close_init_time:%u, codec_init_time:%u ms, init_firstcheckin_time:%u ms, firstcheckin_cmd1_time:%u ms, cmd1_firstcheckout_time:%u ms, firstcheckout_decoded_time:%u ms, decoded_frame0_time:%u ms, di0_firstout_time:%u ms, di_firstpic_time:%u ms\n",
			press_firstpic_time, remove_waitkey_time,
			init_firstpic_time, cmd1_firstpic_time,
			lststop_firstpic_time, press_close_time,
			codec_close_time, close_init_time,
			codec_init_time, init_firstcheckin_time,
			firstcheckin_cmd1_time, cmd1_firstcheckout_time,
			firstcheckout_decoded_time, decoded_frame0_time,
			di0_firstout_time, di_firstpic_time);
	} else if (get_first_quick_output()) {
		frame0_frame1_time = get_frame0_frame1_time();
		di1_firstout_time = get_di1_firstout_time();
		return sprintf(buf, "total_time:%u ms, total_time(no wait keyframe):%u ms, total_time(init->fistpic):%u ms, total_time(key->firstpic):%u ms, total_time(no press key):%u ms, press_close_time:%u ms, codec_close_time:%u ms, close_init_time:%u, codec_init_time:%u ms, init_firstcheckin_time:%u ms, firstcheckin_cmd1_time:%u ms, cmd1_firstcheckout_time:%u ms, firstcheckout_decoded_time:%u ms, decoded_frame0_time:%u ms, frame0_frame1_time:%u, di1_firstout_time:%u ms, di_firstpic_time:%u ms\n",
			press_firstpic_time, remove_waitkey_time,
			init_firstpic_time, cmd1_firstpic_time,
			lststop_firstpic_time, press_close_time,
			codec_close_time, close_init_time,
			...
			firstcheckin_cmd1_time, cmd1_firstcheckout_time,
			firstcheckout_decoded_time, decoded_frame0_time,
			frame0_frame1_time, di1_firstout_time,
			di_firstpic_time);
	} else {
		frame0_frame1_time = get_frame0_frame1_time();
		frame1_frame2_time =  get_frame1_frame2_time();
		di2_firstout_time = get_di2_firstout_time();
		return sprintf(buf, "total_time:%u ms, total_time(no wait keyframe):%u ms, total_time(init->fistpic):%u ms, total_time(key->firstpic):%u ms, total_time(no press key):%u ms, press_close_time:%u ms, codec_close_time:%u ms, close_init_time:%u, codec_init_time:%u ms, init_firstcheckin_time:%u ms, firstcheckin_cmd1_time:%u ms, cmd1_firstcheckout_time:%u ms, firstcheckout_decoded_time:%u ms, decoded_frame0_time:%u ms, frame0_frame1_time:%u, frame1_frame2_time:%u, di2_firstout_time:%u ms, di_firstpic_time:%u ms\n",
			press_firstpic_time, remove_waitkey_time,
			init_firstpic_time, cmd1_firstpic_time,
			lststop_firstpic_time, press_close_time,
			codec_close_time, close_init_time,
			...
			firstcheckin_cmd1_time, cmd1_firstcheckout_time,
			firstcheckout_decoded_time, decoded_frame0_time,
			frame0_frame1_time, frame1_frame2_time,
			di2_firstout_time, di_firstpic_time);
	}
}
...
static ssize_t video_drop_number_show(struct class *class,
			struct class_attribute *attr, char *buf)
{
...
u32 get_current_fps(void)
{
	u32 cnt = (frame_count_field - last_frame_count)/2;
	u32 time = jiffies;
	u32 tmp = time;
	u32 rate = 0;
	time -= last_frame_time;
	last_frame_time = tmp;
	last_frame_count = frame_count_field;
...
static ssize_t current_fps_show(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	u32 cnt = (frame_count_field - last_frame_count)/2;
	u32 time = jiffies;
...
int set_video_dropnumber(int64_t number)
{
	video_drop_number = number;
	return 0;
}
EXPORT_SYMBOL(set_video_dropnumber);
...
static ssize_t rpt0_store(struct class *cla,
					    struct class_attribute *attr,
					    const char *buf, size_t count)
{
	size_t r;
...
static ssize_t rpt0_show(struct class *cla,
					   struct class_attribute *attr,
					   char *buf)
{
	return sprintf(buf, "rpt0 = 0x%x\n", rpt0);
}

static ssize_t rpt1_store(struct class *cla,
					    struct class_attribute *attr,
					    const char *buf, size_t count)
{
	size_t r;
...
static ssize_t rpt1_show(struct class *cla,
					   struct class_attribute *attr,
					   char *buf)
{
	return sprintf(buf, "rpt1 = 0x%x\n", rpt1);
}

static ssize_t rpt2_store(struct class *cla,
					    struct class_attribute *attr,
					    const char *buf, size_t count)
{
	size_t r;
...
static ssize_t rpt2_show(struct class *cla,
					   struct class_attribute *attr,
					   char *buf)
{
	return sprintf(buf, "rpt2 = 0x%x\n", rpt2);
}

static ssize_t rpt3_store(struct class *cla,
					    struct class_attribute *attr,
					    const char *buf, size_t count)
{
	size_t r;
...
static ssize_t rpt3_show(struct class *cla,
					   struct class_attribute *attr,
					   char *buf)
{
	return sprintf(buf, "rpt3 = 0x%x\n", rpt3);
}

static ssize_t vf_next_pts_show(struct class *cla,
		struct class_attribute *attr, char *buf)
{
	int ret = 0;
	struct vframe_states states;
	...
	if (video_vf_get_states(&states) == 0) {
		...
		ret += sprintf(buf + ret, "vframe no next pts\n");
...
static struct class_attribute amvideo_class_attrs[] = {
	...
	__ATTR_RO(fps_info),
	__ATTR_RO(timedebug_info),
	__ATTR_RO(vf_next_pts),
	__ATTR_RO(video_layer1_state),
	__ATTR_RO(current_vpts),
	__ATTR(rpt0,
	       S_IRUGO | S_IWUSR | S_IWGRP,
	       rpt0_show,
	       rpt0_store),
	...
	__ATTR(rpt3,
	       ...
	       rpt3_show,
	       rpt3_store),
	__ATTR_RO(video_drop_number),
	__ATTR(pcr_slow_factor,
			S_IRUGO | S_IWUSR | S_IWGRP,
			video_pcr_slow_factor_show,
			video_pcr_slow_factor_store),
	__ATTR_NULL
};
...
static int __init video_early_init(void)
{
	...
	if (is_meson_txl_cpu() || is_meson_txlx_cpu() ||
		 is_meson_gxlx_cpu() || is_meson_gxlx2_cpu()) {
...
module_param(amplayer_enable, uint, 0664);
MODULE_PARM_DESC(amplayer_enable, "\n amplayer_enable\n");

module_param(amplayer_flag, uint, 0664);
MODULE_PARM_DESC(amplayer_flag, "\n amplayer_flag\n");
...
MODULE_PARM_DESC(ctsplayer_exist, "\n ctsplayer_exist\n");
module_param(ctsplayer_exist, uint, 0664);
...
MODULE_PARM_DESC(rpt0, "\n rpt0\n");
module_param(rpt0, uint, 0664);

MODULE_PARM_DESC(rpt1, "\n rpt1,\n");
module_param(rpt1, uint, 0664);

MODULE_PARM_DESC(rpt2, "\n rpt2\n");
module_param(rpt2, uint, 0664);

MODULE_PARM_DESC(rpt3, "\n rpt3\n");
module_param(rpt3, uint, 0664);
...
MODULE_PARM_DESC(manual_duration, "\n display manual_duration\n");
module_param(manual_duration, uint, 0664);

MODULE_PARM_DESC(manual_duration_change, "\n display manual_duration_change\n");
module_param(manual_duration_change, uint, 0664);
MODULE_PARM_DESC(platform_type, "\n platform_type\n");
module_param(platform_type, bool, 0664);

MODULE_PARM_DESC(ctc_fix_dur, "\n disp video with fixed dur\n");
module_param(ctc_fix_dur, uint, 0664);
...
MODULE_PARM_DESC(trickmode_i, "\n trickmode_i\n");
module_param(trickmode_i, uint, 0664);
...
MODULE_PARM_DESC(fffb_max_count, "\n max toggle count in ff/fb status\n");
module_param(fffb_max_count, uint, 0664);
