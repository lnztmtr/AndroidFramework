...
struct vdec_mpeg12_hw_s {
	...
	u32 offset[DECODE_BUFFER_NUM_MAX];
...
#endif
static irqreturn_t vmpeg12_isr_thread_fn(struct vdec_s *vdec)
{
	u32 reg, info, seqinfo, offset, frame_size, pts, pts_valid = 0;
	...
	if ((reg >> 16) == 0xfe) {
		...
		"!!!cpts=%d,pts64=%lld,size=%d,offset=%d\n",
			...
			hw->chunk->size, hw->chunk->offset);
				...
				offset, &pts, &frame_size, 0, &pts_us64) == 0) {
		...
		if (hw->chunk)
			hw->offset[index] = hw->chunk->offset;
		else
			hw->offset[index] = offset;
		...
		pb_skip =  (hw->offset[hw->refs[0]] == hw->offset[hw->refs[1]]);
		info = hw->buffer_info[index];
		pts_valid = hw->pts_valid[index];
		pts = hw->pts[index];
		pts_us64 = hw->pts64[index];

		fill_frame_info(hw, info, frame_size, pts);
		...
		"ammvdec_mpeg12: display buffer %d, frame_type %c bp_skip %x first_i_frame_ready %x\n",
		index, GET_SLICE_TYPE(info), pb_skip, hw->first_i_frame_ready);
...
static void vmpeg2_dump_state(struct vdec_s *vdec)
{
	...
	debug_print(DECODE_ID(hw), 0,
		"is_framebase(%d), eos %d, state 0x%x, dec_result 0x%x dec_frm %d put_frm %d run %d not_run_ready %d,input_empty %d, init(%d)\n",
...
static void check_timer_func(unsigned long arg)
{
	...
	if (timeout_debug) {
		pr_info("debug: %d, level: %x, timeoutval: %d, start: %x, jiffies_time: %lx\n",
			debug_enable, READ_VREG(VLD_MEM_VIFIFO_LEVEL),
			timeout_val, hw->start_process_time,
			(1000 * (jiffies - hw->start_process_time) / HZ));
	}
...
static s32 vmpeg12_init(struct vdec_mpeg12_hw_s *hw)
{
	...
	/* +[SE] [BUG][BUG-167947][chuanqi.wang]:don't touch
		hw resource until schedule*/
	if (!amvdec_enable_flag) {
		amvdec_enable();
		amvdec_enable_flag = 1;
	}
		...
		void *arg)
{
	...
	if (amvdec_loadmc_ex(VFORMAT_MPEG12, "vmmpeg12_mc", NULL) < 0) {
		...
		hw->dec_result = DEC_RESULT_ERROR;
		debug_print(DECODE_ID(hw), PRINT_FLAG_ERROR,
		"ammvdec_mpeg12 errorloading ucode\n");
		vdec_schedule_work(&hw->work);
		return;
	}
...
module_param(timeout_debug, uint, 0664);
MODULE_PARM_DESC(timeout_debug, "\n ammvdec_mpeg12 timeout_debug\n");
