...
audio_init:
    ...
    */
    ...
    if (para->stream_type == STREAM_ES && para->vstream_info.has_video) {
        ...
        if (para->playctrl_info.cache_enable == 1
            && para->playctrl_info.pause_cache == 0) {
            if (avpkt_cache_set_cmd(CACHE_CMD_SWITCH_AUDIO) != PLAYER_SUCCESS) {
                ret = time_search(para, -1);
                avpkt_cache_set_cmd(CACHE_CMD_SEARCH_OK);
                para->playctrl_info.pause_cache = 0;
                log_print("player_switch_audio,no read end,pause_cache is 0!\n");
            }
        } else {
            ret = time_search(para, -1);
        }
...
int check_avpktlevel_high(play_para_t *para) {
	...
	if ((para->astream_info.has_audio && para->state.audio_bufferlevel >= high_limit)
		|| (para->vstream_info.has_video && para->state.video_bufferlevel >= high_limit)) {
		return 1;
	} 
