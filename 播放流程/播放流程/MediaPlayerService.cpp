...
#include "CMCCMediaPlayer/CMCCMediaPlayerManager.h"
...
#define SOCKET_PATH "/data/video_player_states.sock"
static void InitSendDataReport(int * socketfd)
{
    if (!check_prop_enable("media.player.report.udp")){
        return;
    }
    ...
    *socketfd = socket (PF_LOCAL, SOCK_DGRAM, 0);
    if(*socketfd > 0){
        ALOGV("sock open sucess:%d",*socketfd );
    }
}
static void DeinitSendDataReport(int *scoketfd)
{
    if (!check_prop_enable("media.player.report.udp")){
        return;
    }
...
static bool sendData(int *socketfd,const char * buf, int32_t len)
{
	int line_count = 0;
	int addr_len;
	int send_len = -1;
	int xml_data_len = -1;
	char startbuf[32] = "<videoplayer_state>";
	char nbuf[16] = "\n";
	char endbuf[32]= "</videoplayer_state>";
	char *xml_data = NULL;
	struct sockaddr_un name;
	memset(&name, 0, sizeof(name));
	name.sun_family = AF_LOCAL;
	strcpy (name.sun_path, SOCKET_PATH);
	addr_len = offsetof(struct sockaddr_un, sun_path) + strlen(name.sun_path);
	//head
	if((socketfd == NULL)||(buf == NULL)||(*socketfd < 0)){
		ALOGV("sendData param error ");
	}
	xml_data_len = strlen(startbuf)+strlen(buf)+strlen(endbuf)+2*strlen(nbuf);
	ALOGV("xml_data_len %d",xml_data_len);
	xml_data =(char*) malloc(xml_data_len);
	if(xml_data == NULL)
	{
		ALOGV("xml_data malloc err");
		return false;
	}
	memset(xml_data,0,xml_data_len);
	sprintf(xml_data,"%s%s%s%s%s", startbuf, nbuf,buf,nbuf,endbuf);
	ALOGI("sendData xml_data_len [%d], real len:%d\n%s\n",xml_data_len,strlen(xml_data),xml_data);

	if((send_len=sendto(*socketfd,xml_data, strlen(xml_data),0,(struct sockaddr *)&name, addr_len))<0)
	{
		ALOGV("send error,%d",send_len);
		free(xml_data);
		xml_data=NULL;
		return false;
	}
	...
	{
		ALOGV("send buf sucess datalen:%d  info len %d",send_len,len);
	}
	free(xml_data);
	xml_data=NULL;
	return true;
...
//static
NUPLAYER_STREAMTYPE MediaPlayerService::mNUStreamType = NU_STREAM_NONE;

void MediaPlayerService::instantiate() {
    AmlogicPlayer::BasicInit();/*make sure amplayer's basic data have inited before build a player.*/
...
MediaPlayerService::~MediaPlayerService()
{
    ...
    DeinitSendDataReport(&mSocketFd);
}
...
status_t MediaPlayerService::sendDataReport(
        const char *buf, int32_t len) {
        ALOGI("sendDataReport,len:%d\n%s\n",len,buf);
        sendData(&mSocketFd, buf, len);
    return NO_ERROR;
}

status_t MediaPlayerService::stopPlayerIfNeed(void) {
    Mutex::Autolock lock(mLock);
    for (int i = 0, n = mClients.size(); i < n; ++i) {
            sp<Client> c = mClients[i].promote();
            if (c != 0) c->stopPlayerIfNeed();
        }
    return NO_ERROR;
}
...
#if (defined PROVINCE_NINGXIA || defined PROVINCE_SC || defined PROVINCE_GUIZHOU || defined PROVINCE_TYPE_CM201_GUANGDONG_ZHUOYING)
    mStarted = false;
#endif
...
static bool check_prop_enable(const char* str) {
    ...
    if (property_get(str, value, NULL)
...
static player_type getDefaultPlayerType() {
    char value[PROPERTY_VALUE_MAX];
    if (check_prop_enable("media.ctcplayer.enable")) {
        return CTC_PLAYER;
    }
    if (check_prop_enable("media.amsuperplayer.enable"))
        return AMSUPER_PLAYER;
...
player_type getPlayerType(int fd, int64_t offset, int64_t length)
{
    ...
    // We  just use partial  of header info to check if it is a music file
      if(ident == 0x5367674f && (buf[28] == 1)&&  (!(((buf[29] == 'v') && (buf[30] == 'i') && (buf[31] == 'd' ))  || buf[29] == 't')))
        return STAGEFRIGHT_PLAYER;
...
player_type getPlayerType(const char* url)
{
    ...
    int is_http =(!strncasecmp("http://", url, 7) || !strncasecmp("https://", url, 8));
    if (check_prop_enable("media.cmccplayer.enable")) {
        if (is_http || !strncasecmp("igmp://", url, 7) || !strncasecmp("udp://", url, 6)) {
            ALOGI("use CMCC_PLAYER");
            return CMCC_PLAYER;
        }
    }
    StreamSniffer sniffer(url);
    if (check_prop_enable("media.hls.use-nuplayer")) {
        if (is_http) {
            size_t len = strlen(url);
            if (len >= 5 && !strcasecmp(".m3u8", &url[len - 5])) {
                return NU_PLAYER;
            ...
            if (StreamSniffer::STREAM_HLS == sniffer.sniffStreamType(1024)) {
                MediaPlayerService::mNUStreamType = NU_STREAM_HLS;
                return NU_PLAYER;
            }
        }
    }
	if (check_prop_enable("media.http-mp4.use-nuplayer")) {
        if (is_http) {
            size_t len = strlen(url);
            if (len >= 4 && !strcasecmp(".mp4", &url[len - 4])) {
                return NU_PLAYER;
            }
            if (strcasestr(url,".mp4")) {
                return NU_PLAYER;
            }
        }
    }
    ...
    // use MidiFile for MIDI extensions
    char* doubt_end=strchr(url,'?');
    int lenURL = strlen(url);
    if(doubt_end!=NULL)
        lenURL-=strlen(doubt_end);
    for (int i = 0; i < NELEM(FILE_EXTS); ++i) {
        int len = strlen(FILE_EXTS[i].extension);
        int start = lenURL - len;
        if (start > 0) {
            if (!strncasecmp(url + start, FILE_EXTS[i].extension, len)) {
                ALOGI("getPlayerType-%d,when found ext '%s'",FILE_EXTS[i].playertype,FILE_EXTS[i].extension);
                return FILE_EXTS[i].playertype;
            }
        }
    }
...
player_type MediaPlayerService::Client::getPlayerType(
   ...
   if (!check_prop_enable("media.amsuperplayer.enable")) {/*if not used  hw decoder*/
        ...
        ptype = AMLOGIC_PLAYER;
   }
#endif
   return ptype;
}

int GetCallingAPKName(char *name, int size)
{
    char path[64];
    int ret = -1;
    strcpy(name, "NA");
    ...
    ALOGV("GetCallingAPKName %s,name=[%s]", path, name);
    return ret;
}
        ...
        notify_callback_f notifyFunc)
{
    ...
    ALOGV("GetCreate calling apk name...[%s]\n", CallingAPkName);
    char aging_state[64] = {0};
    if(inaging&&!strstr(CallingAPkName,"agingdragon")){
	return NULL;
    }
    if(strstr(CallingAPkName,"agingdragon")){
	inaging = true;
    }
    sp<MediaPlayerBase> p;
    switch (playerType) {
        ...
        case NU_PLAYER:
            ...
            p = new NuPlayerDriver(MediaPlayerService::mNUStreamType);
            break;
        case TEST_PLAYER:
            ...
            break;
        case CMCC_PLAYER:
            ALOGV("Create CMCCMediaPlayerManager");
            p = new CMCCMediaPlayerManager();
            break;
        case CTC_PLAYER:
            ALOGV("Create CTCMediaPlayer");
            p = new CTCMediaPlayer();
            break;
#if BUILD_WITH_AMLOGIC_PLAYER
        case AMLOGIC_PLAYER:
...
sp<MediaPlayerBase> MediaPlayerService::Client::createPlayer(player_type playerType)
{
    ...
    if (p == NULL) {
        //TODO: Change AmSuperPlayer to factory
...
void SupportMultiPlay()
{
    char CallingAPkName[64];
    CallingAPkName[0] = '\0';
    GetCallingAPKName(CallingAPkName, sizeof(CallingAPkName));
    ALOGV("GetCallingAPKName calling apk name...[%s]\n", CallingAPkName);

    /*+[SE][REQ][BUG 170227][wenjie.chen]
    APK: According to telecom's requirement to modify the package and class name.*/
    if (strcasestr(CallingAPkName, ".multivideotest")) {
...
status_t MediaPlayerService::Client::prepareAsync()
{
    ...
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
//begin； add by tianchi: ningxia fcc
#if (defined PROVINCE_NINGXIA || defined PROVINCE_SC || defined PROVINCE_GUIZHOU || defined PROVINCE_TYPE_CM201_GUANGDONG_ZHUOYING)    
    /* +[SE] [BUG][IPTV-251][yanan.wang] added:support the feature of serial amplayer*/
    ALOGD("=====leo guangdong zhuoying");
    mStarted = true;
    if (check_prop_enable("media.serial.amplayer.enable")) {
        for (int i = 0, n = mService->mClients.size(); i < n; ++i) {
            sp<Client> c = mService->mClients[i].promote();
            if ((c != 0) && (c != this) && (c->mStarted == true)) {
                ALOGD("MediaPlayerService inner exit");
                c->stop();
            }
...
#endif
//end: add by tianchi: ningxia fcc
...
{
    ALOGI("[%d] start", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    p->setLooping(mLoop);
    if (check_prop_enable("media.cmccplayer.enable")||
        check_prop_enable("media.playstatus.enable")) {
        ALOGI("set service.media.playstatus,running\n");
...
status_t MediaPlayerService::Client::stop()
{
    ...
    if (check_prop_enable("media.cmccplayer.enable")||
        check_prop_enable("media.playstatus.enable")){
        ALOGI("set service.media.playstatus,stopped\n");
...
{
    ALOGI("[%d] pause", mConnId);
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
	if (check_prop_enable("media.cmccplayer.enable")||
        check_prop_enable("media.playstatus.enable")){
        ALOGI("set service.media.playstatus,pause\n");
        property_set("service.media.playstatus", "pause");
    }
    return p->pause();
}
...
status_t MediaPlayerService::Client::isPlaying(bool* state)
{
    *state = false;
    ...
    if (p == 0) return UNKNOWN_ERROR;
    *state = p->isPlaying();
    ALOGI("[%d] isPlaying: %d", mConnId, *state);
    return NO_ERROR;
}

status_t MediaPlayerService::Client::stopPlayerIfNeed()
{
    status_t state = false;
    sp<MediaPlayerBase> p = getPlayer();
    if (p == 0) return UNKNOWN_ERROR;
    state = p->stopPlayerIfNeed();
    ALOGI("[%d] stopPlayerIfNeed: state=%d", mConnId, state);
    return NO_ERROR;
}
...
status_t MediaPlayerService::Client::reset()
{
    ...
    if (check_prop_enable("media.cmccplayer.enable")||
        check_prop_enable("media.playstatus.enable")){
        ALOGI("set service.media.playstatus,reset\n");
        property_set("service.media.playstatus", "stopped");
    }
    return p->reset();
}
...
status_t MediaPlayerService::Client::setVolume(float leftVolume, float rightVolume)
{
    ...
    sp<MediaPlayerBase> p = getPlayer();
    {
      //ALOGV("p->playerType(): %d", p->playerType());
      Mutex::Autolock l(mLock);
...
#else
        reply->writeString16(String16("UNKNOWN_PLAYER"));
#endif
        return 0;
    }
