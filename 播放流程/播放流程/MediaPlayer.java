...
public class MediaPlayer implements SubtitleController.Listener, MediaPlayerEx
{
	...
	private boolean  mSendPrepareCompletedEvent = false;
	private int tmpPlayId = 0;
    //begin:add by zhanghk at 20191107:fix huawei EPG playid err problem
	private boolean mStop = false;
	private boolean mReset = false;
	private boolean mNeedPlayIdAdd = false;
	private boolean mCreateMediaPlayer = false;
    //end:add by zhanghk at 20191107:fix huawei EPG playid err problem
...
*/
    ...
    private void setDataSource(String path, String[] keys, String[] values)
        ...
        path="http://183.235.22.18:6610/190000000009/02000022000000022020070999001297/4aeade18e83d4e9c83d5.ts?AuthInfo=xxx&version=xxx&IASHttpSessionId=RR6718220200714140249160371&ZTEUPSTREAM=1";
    ...
    */
    ...
    public  void start() throws IllegalStateException {
        //add by zhanghk at 20191107:solve lack PLAY_QUIT problem
        mQuitFlag=false;
        if(SystemProperties.getBoolean("media.player.report.udp", false)&& !mSendPrepareEvent){
	     ...
	     mSeekFlag=false;
            ...
            if(null != info){
                //begin:add by zhanghk at 20191107:solve playId err problem
                int playId = playerId;
                if(mReset){
                    playId = playerId +1; 
                    mNeedPlayIdAdd = true;
                }
                //end:add by zhanghk at 20191107:solve playId err problem
                cmd.putExtra("ID", playId);
                tmpPlayId = playerId;
            ...
            if(!mbIsFileDescriptor){
                ...
                //begin:add by zhanghk at 20191028:solve lack  PrepareCompleted event when use icntv.apk play VOD problem
                if(!mSendPrepareCompletedEvent){
                    Message m = mEventHandler.obtainMessage(MEDIA_PREPARED);
                    mEventHandler.sendMessage(m);
                }
                mSendPrepareEvent = true;
                mSendQuitEvent = false;
                mSendPrepareCompletedEvent = false;
                //end:add by zhanghk at 20191028:solve lack  PrepareCompleted event when use icntv.apk play VOD problem
    ...
    */
    ...
    private void updateId(MediaInfo info) {
...
//				+ "&" + HuaweiUtils.PARAM_VIDEO_SIZE + "=" + videoSize); 
            ...
            case MEDIA_PREPARED:
                ...
                subtitleStart();
                //begin:add by zhanghk at 20191028:solve lack  PrepareCompleted event when use icntv.apk play VOD problem
                Log.d("TAG","mSendPrepareEvent:"+mSendPrepareEvent+",tmpPlayId:"+tmpPlayId+",getMediaInfo().player_id:"+getMediaInfo().player_id);
                if(mSendPrepareEvent && tmpPlayId == playerId){
                    if (mContext != null){
                        long start = System.currentTimeMillis();
                        ...
                        if(mNeedPlayIdAdd){
                            playId = playerId +1; 
						    mNeedPlayIdAdd = false;
                        }
                        //end:add by zhanghk at 20191107:solve playId err problem
                        if(null != info)
                            cmd.putExtra("ID", playId);
                        cmd.putExtra("TIME", start);
                        if(!mbIsFileDescriptor && !mSendPrepareCompletedEvent){
                             mContext.sendBroadcast(cmd);
                             mSendPrepareCompletedEvent = true;
                        }
                    }
               }
               //end:add by zhanghk at 20191028:solve lack  PrepareCompleted event when use icntv.apk play VOD problem
            ...
            case MEDIA_INFO:
                    ...
                    case MEDIA_INFO_BUFFERING_BROADCAST_START:
                        if (SystemProperties.getBoolean("media.player.report.udp", false)) {
                            ...
                            if (!mbIsFileDescriptor) {
                                if (SystemProperties.get("ro.ysten.province", "master").contains("CM201_zhejiang")
                                        || SystemProperties.get("ro.ysten.province", "master")
                                                .contains("heilongjiang")) {
                                    if (!mSeekFlag) {
                                        mContext.sendBroadcast(cmd);
                    ...
                    case MEDIA_INFO_BUFFERING_BROADCAST_END:
                        if (SystemProperties.getBoolean("media.player.report.udp", false)) {
                            ...
                            buf.append('\n');
                            buf.append(
                                    "<bufferseconds>" + String.valueOf(time - mBufferStartTime) + "</bufferseconds>");
                            sendDataReport(buf.toString(), buf.length());
                            Log.e(TAG, buf.length() + " ,:" + buf.toString());
                            ...
                            if ((end < mSeekEndTime + 5000)
                                    && SystemProperties.getBoolean("media.player.delaybuf.seek", false)) {
