...
int update_player_states(play_para_t *para, int force)
{
    ...
    if (check_time_interrupt(&cb->callback_old_time, cb->update_interval) || force) {
        ...
        log_print("**[update_state]pid:%d status=%s(last:%s) err=0x%x curtime=%d (ms:%d) fulltime=%d lsttime=%d\n",
                  ...
                  state.full_time,
                  state.last_time);
        log_print("**[update_state]abuflevel=%.08f vbublevel=%.08f sbublevel=%.08f abufrp=%x vbufrp=%x read_end=%d\n",
                  state.audio_bufferlevel,
                  state.video_bufferlevel,
                  state.sub_bufferlevel,
        ...
        if (state.status == PLAYER_RUNNING) {
            audio_index = para->media_info.stream_info.cur_audio_index;
            state.total_audio_num = para->media_info.stream_info.total_audio_num;
            for (i = 0; i < state.total_audio_num; i++) {
                if (audio_index == para->media_info.audio_info[i]->index) {
                    state.cur_audio_index = i + 1;
                    state.aformat = para->media_info.audio_info[i]->aformat;
                    state.channel = para->media_info.audio_info[i]->channel;
                    state.sample_rate = para->media_info.audio_info[i]->sample_rate;
                    break;
                }
            }
            if ((0 == state.cur_audio_index) && (i > state.total_audio_num)) {
                state.cur_audio_index = 0;
                state.aformat = AFORMAT_UNKNOWN;
                state.channel = -1;
                state.sample_rate = -1;
            }
        }
