...
void service_start(struct service *svc, const char *dynamic_args)
{
    ...
    if (pid == 0) {
        ...
        if (strcmp(svc->name, "sqmpro") == 0 ||
            strcmp(svc->name, "sqmloader") == 0)
          umask(000);
        else
          umask(033);
    ...
    /* as requested, set our gid, supplemental gids, and uid */
        ...
        if (!dynamic_args) {
            if (strncmp(svc->name, "zygote", 6) == 0) {
                nice(-20);
            }            
...
void property_changed(const char *name, const char *value)
{
    if (strcmp(name, "system.reboot.recovery") == 0
		...
		return;
    } else if (strcmp(name, "sys.ubootenv.reload") == 0
                && (strcmp(value, "0") == 0)){
        init_bootenv_varibles();
#if BOOT_ARGS_CHECK
        check_boot_args();
#endif
        property_set("sys.ubootenv.reload","1");
    }
...
static int wait_for_coldboot_done_action(int nargs, char **args)
{
    int ret;
    INFO("wait for %s\n", coldboot_done);
    ret = wait_for_file_sleep(coldboot_done, COMMAND_RETRY_TIMEOUT, 10000);
    if (ret)
...
static void import_kernel_nv(char *name, int for_emulator)
{
    ...
    *value++ = 0;
    ...
    if (!strcmp(name,"qemu")) {
        ...
        if (!strcmp(name,"androidboot.serialno")) {
            property_set("android.os.Build.SERIAL", value);
        }
    ...
    }else if (!strcmp(name,"boardid")) {
        strlcpy(boardid, value, sizeof(boardid));
    }
}

static char* aToA(char* str, int aToA) {
    int len = strlen(str);
    int i = 0;
...
static void export_kernel_boot_props(void)
{
    ...
    struct {
        ...
        #if defined CUSTOMER_STBID
        #elif defined TELECOM_JICAI_STBID_SN
        #else
        ...
        #ifdef YM3_GD_SERIALNO
        { "ro.boot.gd_serialno", "ro.gd_serialno", "GD123456789"},
        #endif
    ...
    #if defined CUSTOMER_STBID
    char tmp1[PROP_VALUE_MAX] = {0};
    int ret1 = 0;
    unsigned len = 0;
    unsigned len1 = 0;
    unsigned j = 0;
    ret = property_get("ro.boot.serialno", tmp);
    ret1 = property_get("ro.boot.mac", tmp1);
    if ((ret > 0) && (ret1 > 0)) {
        len = strlen(tmp);
        len1 = strlen(tmp1);
        if((len + len1) < PROP_VALUE_MAX) {
            for(i=0; i<len1; i++) {
                if(tmp1[i] != ':') {
                    tmp[len+j] = tmp1[i];
                    j++;
        ...
        property_set("ro.serialno", tmp);
    } else {
        if(ret > 0) {
            property_set("ro.serialno", tmp);
        } else {
            property_set("ro.serialno", "12345678900");
        }
    }
    #elif defined TELECOM_JICAI_STBID_SN
    char tmp1[PROP_VALUE_MAX] = {0};
    char tmp2[PROP_VALUE_MAX] = "0010039901A033100001";
    int ret1 = 0;
    unsigned len = 0;
    unsigned len1 = 0;
    unsigned j = 0;
    ...
    if (ret1 > 0) {
        ...
        property_set("ro.serialno", aToA(tmp2,1));
        property_set("android.os.Build.SERIAL", tmp2);
    ...
    for (i = 0; i < ARRAY_SIZE(prop_map); i++) {
        ...
        if (ret > 0) {
            if (!strcmp(prop_map[i].dest_prop, "ro.mac"))
                property_set(prop_map[i].dest_prop, aToA(tmp, 1));
            else
                property_set(prop_map[i].dest_prop, tmp);
        } else {
            if (!strcmp(prop_map[i].dest_prop, "ro.mac"))
                property_set(prop_map[i].dest_prop, aToA(prop_map[i].def_val, 1));
            else
...
#if defined(GXL_BOARD)
    ret = property_get("ro.boot.boardid", tmp);
    if (ret)
        strlcpy(boardid, tmp, sizeof(boardid));
    property_set("ro.boardid", boardid);
#endif	
...
static int aml_firstbootinit()
{
    ...
    // set hw.ddr.size property
    struct sysinfo s_info;
    sysinfo(&s_info);

    if (s_info.totalram > 1024*1024*1024) {
        property_set("hw.ddr.size", "2048");
    } else if (s_info.totalram > 512*1024*1024) {
        property_set("hw.ddr.size", "1024");
    } else {
        property_set("hw.ddr.size", "512");
    }
...
int set_firstboot_complete_flag(int nargs, char **args) {
    ...
    property_get("ubootenv.var.firstboot", val);
...
void platform_type_initialize(void)
{
    const char* sys_file = "/sys/class/cputype/cputype";
    char buf[32]={0};
    ...
    INFO("platform_type_initialize (%s)\n", buf);

    if (!strncmp(buf, "905L2", strlen("905L2"))) {
        property_set("ro.board.platform","gxlx");
        property_set("ro.product.cpu.info","905L2");
    }
    else if (!strncmp(buf, "905L", strlen("905L"))) {
        property_set("ro.board.platform","gxl");
        property_set("ro.product.cpu.info","905L");
    }
    else if (!strncmp(buf, "905M2", strlen("905M2"))) {
        property_set("ro.board.platform","gxl");
        property_set("ro.product.cpu.info","905M2");
    }
    else if (!strncmp(buf, "905M", strlen("905M"))) {
        property_set("ro.board.platform","gxbaby");
        property_set("ro.product.cpu.info","905M");
    }
    else if (!strncmp(buf, "905", strlen("905"))) {
        property_set("ro.board.platform","gxbaby");
        property_set("ro.product.cpu.info","905");
    }
}

void system_info_initialize(void)
{
    const char* sys_file = "/sys/devices/d0074000.emmc/mmc_host/emmc/emmc:0001/block/mmcblk0/capability";
    char buf[32]={0};

    FILE* file = fopen(sys_file, "rb");
    if (file == NULL) {
        ERROR("Failed to open %s(%s)", sys_file, strerror(errno));
        property_set("ro.product.flash.info","8G");
        return;
    }
    size_t len = fread(buf, 1, 32, file);
    char temp[27]={0};
    strncpy(temp,buf,len-6);
    int size;
    sscanf(temp,"%x",&size);
    if (size > 14000)
    {
        property_set("ro.product.flash.info","16G");
    }else if ((size > 7000) && (size < 14000))
    {
        property_set("ro.product.flash.info","8G");
    }else if (size < 7000)
    {
        property_set("ro.product.flash.info","4G");
    }
}
