...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    private static final int MSG_GO_TO_SLEEP_COUNT_DOWN = 5;
    private static final int MSG_CEC_STANDBY = 6;
    ...
    private static final int GO_TO_SLEEP_COUNT_DOWN_TIMEOUT = 10000;
    ...
    private long mBroadcastTimeout;
    ...
    private AlertDialog mGoToSleepCountDownDialog = null;
    ...
    public void systemReady(TwilightService twilight, DreamManagerService dreamManager) {
        synchronized (mLock) {
            ...
            if(SystemProperties.get("ro.ysten.custom.type","none").equals("viper") || (SystemProperties.get("ro.ysten.province","master").contains("jiangxi"))
				|| (SystemProperties.get("ro.ysten.province","master").equals("CM201_hubei"))
				//add by liangk at 20200227 for liaoning
            ...
            {
             Slog.d(TAG,"custom.type:viper1");
                filter = new IntentFilter();
                filter.addAction("com.ysten.setting.action.poweroff_delay_changed");
                mContext.registerReceiver(new PowerOffDelayReceiver(),filter,null,mHandler);			
            }
            filter = new IntentFilter();
	...
	if (mContext == null){
		Slog.d(TAG, "updateWakeLockWorkSource mContext=" + mContext);
		return;
	}
        mContext.enforceCallingOrSelfPermission(android.Manifest.permission.WAKE_LOCK, null);
        if (ws != null && ws.size() != 0) {
    ...
    private void wakeUpInternal(long eventTime) {
        synchronized (mLock) {
            ...
            int screenstate = getDesiredScreenPowerStateLocked();
            if ("telecom".equals(proj_type) && "jicai".equals(tender_type)
                && null != mContext && screenstate != DisplayPowerRequest.SCREEN_STATE_BRIGHT){
    ...
    public void goToSleep(long eventTime, int reason) {
        ...
        mContext.enforceCallingOrSelfPermission(android.Manifest.permission.DEVICE_POWER, null);
        mContext.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_FINISHED));
        //begin by ysten.zhangjunjian,20191012,send end event while poweroff
        MediaPlayer mMediaPlayer = new MediaPlayer();
        mMediaPlayer.shutdownStop(mContext);
       //end by ysten.zhangjunjian,20191012,send end event while poweroff
        final long ident = Binder.clearCallingIdentity();
        String proj_type = SystemProperties.get("sys.proj.type", "ott");
        Log.d(TAG, "goToSleep, proj_type is " + proj_type);
        try {
            if ("mobile".equals(proj_type)) {
                Log.d(TAG, "goToSleep, this is mobile propject need to reboot!");
                ShutdownThread.setReboot(true);
            }
    ...
    private void goToSleepFromNative(long eventTime, int reason) {
        Slog.i(TAG, "goToSleepFromNative mDisplayReady:"+mDisplayReady);
        String proj_type = SystemProperties.get("sys.proj.type", "ott");
        String tender_type = SystemProperties.get("sys.proj.tender.type", null);
        Slog.i(TAG, "goToSleepFromNative proj_type:"+proj_type+",tender_type:"+tender_type);
        if (!("telecom".equals(proj_type) && "jicai".equals(tender_type))) {
            if (!mDisplayReady) {return;}
        }
        //if(mDisplayReady){
            mContext.sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_FINISHED));
            //goToSleepInternal(eventTime, reason);
            String cec_standby = SystemProperties.get("sys.cec_off", "0");
            Slog.i(TAG, "sys.cec_off:" + cec_standby);
            if ("mobile".equals(SystemProperties.get("sys.proj.type", "false"))
                && "1".equals(cec_standby)) {
                Slog.i(TAG, "delay gotosleep by tv cec standby");
                Message msg = mHandler.obtainMessage(MSG_CEC_STANDBY, (int)eventTime, reason);
                mHandler.sendMessageDelayed(msg, SystemProperties.getInt("sys.cecdelayms", 3050));
            } else {
                goToSleepInternal(eventTime, reason);
            }
        //}
    ...
    private boolean goToSleepNoUpdateLocked(long eventTime, int reason) {
        ...
        if (eventTime < mLastWakeTime || mWakefulness == WAKEFULNESS_ASLEEP
                || SystemService.isRunning(BOOT_ANIMATION_SERVICE)/*!mBootCompleted*/ || !mSystemReady) {
            return false;
        }
    ...
    private void updateUserActivitySummaryLocked(long now, int dirty) {
        ...
        if ((dirty & (DIRTY_USER_ACTIVITY | DIRTY_WAKEFULNESS | DIRTY_SETTINGS)) != 0) {
            ...
            if (mWakefulness != WAKEFULNESS_ASLEEP) {
                ...
                boolean dialogshow = SystemProperties.getBoolean("mbx.timeoutui.enable", true);
                Slog.d(TAG, "updateUserActivitySummaryLocked, mLastUserActivityTime: " + mLastUserActivityTime
                        + ", screenOffTimeout: " + screenOffTimeout);
                Slog.d(TAG, "updateUserActivitySummaryLocked, mBroadcastTimeout: " + mBroadcastTimeout 
                        + ", dialog show enable: " + dialogshow);
                Slog.d(TAG, "updateUserActivitySummaryLocked, now: " + now + ", nextTimeout: " + nextTimeout);
                long timeout = mLastUserActivityTime + screenOffTimeout;
                if ((timeout != mBroadcastTimeout) && (now < timeout) && dialogshow) {
                    mBroadcastTimeout = timeout;
                    Slog.d(TAG, "mGoToSleepCountDownDialog show");
                    if (mGoToSleepCountDownDialog != null && mGoToSleepCountDownDialog.isShowing())
                        mGoToSleepCountDownDialog.dismiss();
                    mHandler.removeMessages(MSG_GO_TO_SLEEP_COUNT_DOWN);
                    Message msg = mHandler.obtainMessage(MSG_GO_TO_SLEEP_COUNT_DOWN);
                    msg.setAsynchronous(true);
                    mHandler.sendMessageAtTime(msg, timeout - GO_TO_SLEEP_COUNT_DOWN_TIMEOUT);
                }
            } else {
                mUserActivitySummary = 0;
            }
	...
	//modify by zhaolianghua suspend for hebei @20181115
        if("cm201_hebei".equals(SystemProperties.get("ro.ysten.province"))){
		return mScreenOffTimeoutSetting;
	}
	//modify by zhaolianghua end
        if (!SystemProperties.getBoolean(SCREENSAVER_PROP, false)) {
            boolean mMBXTimeoutEnable = SystemProperties.getBoolean("persist.sys.mbx.timeout.enable",true);
    ...
    private void handleSandman() { // runs on handler thread
		...
		//modify by zhaolianghua suspend for hebei @20181115
		String proj_type = SystemProperties.get("sys.proj.type", "ott");
                String province_type = SystemProperties.get("ro.ysten.province", "master");
                if ("mobile".equals(proj_type) && "cm201_hebei".equals(province_type)) {
			if (mWakefulness == WAKEFULNESS_ASLEEP) {
				Slog.d(TAG, "handleSandman mWakefulness000 -->" + mWakefulness);
				ShutdownThread.setReboot(true);
			}
		}
		//modify by zhaolianghua end
	...
	private final String SYSTEM_POWER_OFF_TIME_DELAY = "system_power_off_time_delay";
    private final class BootCompletedReceiver extends BroadcastReceiver {
        ...
        public void onReceive(Context context, Intent intent) {
		    ...
		    if(SystemProperties.get("ro.ysten.custom.type","none").equals("viper") || (SystemProperties.get("ro.ysten.province","master").contains("jiangxi"))
				...
				|| (SystemProperties.get("ro.ysten.province","master").equals("cm201_guizhou")))
            {
             Slog.d(TAG,"custom.type:viper2");
			 Slog.d(TAG,"action boot_completed,begin check time poweroff");
			 String powerOffTime = Settings.System.getString(mContext.getContentResolver(),SYSTEM_POWER_OFF_TIME_DELAY);
		 if(powerOffTime == null )
			 return;
				 long powerOffDelay = getPowerOffDelay(powerOffTime);
		 Slog.i("THY", "BootCompletedReceiver in PowerManagerService , powerOffDelay = "+powerOffDelay);
		  if(powerOffDelay > 0)
			 {
			   Slog.i("THY","post powerOffRunnable Delayed in BootCompletedReceiver : "+powerOffDelay+"ms"); 
			 mHandler.postDelayed(powerOffRunnable,powerOffDelay);		 
        }
    }
	 ...
	 private final String POWER_OFF_DELAY_CHANGE_ACTION = "com.ysten.setting.action.poweroff_delay_changed";
	 private Runnable powerOffRunnable = new Runnable()
	 {
		   @Override
			 public void run() {
		    if(SystemProperties.get("ro.ysten.custom.type","none").equals("viper") || (SystemProperties.get("ro.ysten.province","master").contains("jiangxi"))
				|| (SystemProperties.get("ro.ysten.province","master").contains("CM201_hubei"))
				//add by liangk at 20200227 for liaoning
				||(SystemProperties.get("ro.ysten.province","master").contains("liaoning"))
				|| (SystemProperties.get("ro.ysten.province","master").equals("cm201_guizhou")))
            {
             Slog.d(TAG,"custom.type:viper3");
                 Slog.i("THY","run() in powerOffRunnable");  
                 boolean isUpgrading=(Settings.System.getInt(mContext.getContentResolver(), "room_upgrade_flag", -1)==1);
                   if(!isUpgrading)
                   {  
                      if(SystemProperties.get("ro.ysten.province").contains("jiangxi") || SystemProperties.get("ro.ysten.province").contains("cm201_hebei")
			||SystemProperties.get("ro.ysten.province","master").equals("cm201_guizhou")){
                          showPowerOffDialog();
                      }else{
	 ...
	 private final class PowerOffDelayReceiver extends BroadcastReceiver {
		 @Override
		 public void onReceive(Context context, Intent intent) {
			 // This is our early signal that the system thinks it has finished booting.
			 // However, the boot animation may still be running for a few more seconds
		    ...
		    if(SystemProperties.get("ro.ysten.custom.type","none").equals("viper") || (SystemProperties.get("ro.ysten.province","master").contains("jiangxi"))
				...
				|| (SystemProperties.get("ro.ysten.province","master").equals("cm201_guizhou")))
            {
             Slog.d(TAG,"custom.type:viper4");
		 String powerOffTime = Settings.System.getString(mContext.getContentResolver(),SYSTEM_POWER_OFF_TIME_DELAY);
			  if(powerOffTime == null)
						 return;  
		 long powerOffDelay = getPowerOffDelay(powerOffTime);
	
		  Slog.i("THY", "PowerOffDelayReceiver in PowerManagerService , powerOffDelay = "+powerOffDelay);
								 
		   mHandler.removeCallbacks(powerOffRunnable);
		  if(powerOffDelay > 0)
		 {
				 Slog.i("THY","post powerOffRunnable Delayed in PowerOffDelayReceiver : "+powerOffDelay+"ms");	
			   mHandler.postDelayed(powerOffRunnable,powerOffDelay);
		 }
		 }
         }
	 ...
	 private long getPowerOffDelay(String powerOffTime)
	 {
		 Slog.i("THY","getPowerOffDelay , powerOffTime = "+powerOffTime);
		 long powerOffDelay = 0;
		 if(powerOffTime.equals(""))
			 powerOffDelay = -1;
		 else{
			 long tmp_powerOffTime = string2TimeMills(powerOffTime);
			 if(tmp_powerOffTime < System.currentTimeMillis())
				 tmp_powerOffTime += 24*3600*1000;
					 
			 powerOffDelay = tmp_powerOffTime - System.currentTimeMillis();
		 }
		 return powerOffDelay;
	 }
	
	 
		 private long string2TimeMills(String msg)
	 {
		 Slog.i("THY","string2TimeMills  ,msg = "+msg);
		 String[] hourAndMinutes = msg.split(":");
		 StringBuffer temp_str = new StringBuffer();
		 Date dt = new Date();
		 SimpleDateFormat sdf = new SimpleDateFormat("yyyyMMdd");
		 temp_str.append(sdf.format(dt));
		 temp_str.append(hourAndMinutes[0]).append(hourAndMinutes[1]).append("00");
		 Calendar c = Calendar.getInstance();  
		 try {
			 c.setTime(new SimpleDateFormat("yyyyMMddHHmmss").parse(temp_str.toString()));
		 } catch (ParseException e) {
			 e.printStackTrace();
		 }	
		  Slog.i("THY","string2TimeMills ,transfered time  = "+c.getTimeInMillis()+" , current time = "+System.currentTimeMillis()+" , current date = "+temp_str.toString());
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case MSG_CHECK_IF_BOOT_ANIMATION_FINISHED:
                    ...
                    break;
                case MSG_GO_TO_SLEEP_COUNT_DOWN:
                    goToSleepCountDownInternal();
                    break;
                case MSG_CEC_STANDBY:
                    if (mContext != null) {
                    goToSleepInternal(msg.arg1, msg.arg2);
                    }
                    break;
            }
        }
    ...
    private void goToSleepCountDownInternal() {
        Runnable runnable = new Runnable() {
            @Override
            public void run() {
                synchronized (mLock) {
                    final int UPDATE_COUNT_DOWN = 0;
                        ...
                        public void handleMessage(Message msg) {
                            ...
                            switch (msg.what) {
                                case UPDATE_COUNT_DOWN:
                                    if (msg.arg1 <= 0 || msg.arg1 > GO_TO_SLEEP_COUNT_DOWN_TIMEOUT / 1000) {
                                        mGoToSleepCountDownDialog.dismiss();
                                        this.removeMessages(UPDATE_COUNT_DOWN);
                                        return;
                                    }
                                    String proj_type = SystemProperties.get("sys.proj.type", "ott");
                                    String tender_type = SystemProperties.get("sys.proj.tender.type", null);
                                    Slog.i(TAG, "countDownHandler proj_type:"+proj_type+",tender_type:"+tender_type);
                                    if ("telecom".equals(proj_type) && "jicai".equals(tender_type)) {
                                        if (!SystemProperties.getBoolean("persist.sys.mbx.timeout.enable",false)) {
                                             if (mGoToSleepCountDownDialog != null) {
                                                  mGoToSleepCountDownDialog.dismiss();
                                             }
                                             this.removeMessages(UPDATE_COUNT_DOWN);
                                             return;
				    ...
				    boolean IsHB = "cm201_hebei".equals(SystemProperties.get("ro.ysten.province"));//modify by zhaolianghua suspend for hebei @20181115
                                    if (mGoToSleepCountDownDialog == null) {
                                        final AlertDialog.Builder ab = new AlertDialog.Builder(mContext);
                                        mGoToSleepCountDownDialog = ab.create();
                                        mGoToSleepCountDownDialog.getWindow().setType(WindowManager.LayoutParams.TYPE_KEYGUARD_DIALOG);
                                        mGoToSleepCountDownDialog.setTitle(IsHB?com.android.internal.R.string.suspend_off:com.android.internal.R.string.power_off);//modify by zhaolianghua suspend for hebei @20181115
                                    }
                                    int countDown = ((int)(mBroadcastTimeout - SystemClock.uptimeMillis()) + 999) / 1000;
                                    Slog.d(TAG, "UPDATE_COUNT_DOWN: " + countDown);
                                    if (countDown <= 0 || countDown > GO_TO_SLEEP_COUNT_DOWN_TIMEOUT / 1000) {
                                        mGoToSleepCountDownDialog.dismiss();
                                        return;
                                    }
                                    long now = System.currentTimeMillis();
				   if(!IsHB){//modify by zhaolianghua suspend for hebei @20181115
                                    mGoToSleepCountDownDialog.setMessage(
						    "\n" + mContext.getResources().getString(com.android.internal.R.string.power_off) + ": "
						    + DateUtils.getRelativeTimeSpanString(now + countDown * 1000, now, DateUtils.SECOND_IN_MILLIS).toString());
				    }else{
					String[] time = DateUtils.getRelativeTimeSpanString(now + countDown * 1000, now, DateUtils.SECOND_IN_MILLIS).toString().split(" ");
					mGoToSleepCountDownDialog.setMessage(
							mContext.getResources().getString(com.android.internal.R.string.action_suspend_title)+
							mContext.getResources().getString(com.android.internal.R.string.standby_warn,Integer.parseInt(time[0])));
				    }
                                    mGoToSleepCountDownDialog.show();
                                    WindowManager.LayoutParams params = mGoToSleepCountDownDialog.getWindow().getAttributes();
                                    if (params.width != 800&&!IsHB) {
                                        params.width = 800;
                                        mGoToSleepCountDownDialog.getWindow().setAttributes(params);
                                    }
                                    Message message = this.obtainMessage(UPDATE_COUNT_DOWN, countDown, 0);
                    ...
                    int countDown = ((int)(mBroadcastTimeout - SystemClock.uptimeMillis()) + 999) / 1000;
                    Slog.d(TAG, "goToSleepInternal: " + countDown);
                    Message message = countDownHandler.obtainMessage(UPDATE_COUNT_DOWN, countDown, 0);
                    message.setAsynchronous(true);
	...
	//begin:add by zhuhengxuan at 20190706 for 定时关机弹窗
	/*private Runnable powerOffRunnable = new Runnable(){
		public void run(){
	...
	private void showPowerOffDialog(){
		Log.d(TAG, "showPowerOffDialogJX");
		mSleepDelayTextView = new TextView(mContext);
		if(SystemProperties.get("ro.ysten.province","master").equals("cm201_guizhou")){
		    mSleepDelayTextView.setText(mContext.getResources().getString(com.android.internal.R.string.standby_warn_guizhou_3am,mCount));
		}else{
		    mSleepDelayTextView.setText(mContext.getResources().getString(com.android.internal.R.string.standby_warn_jiangxi_3am,mCount));
		}
		...
		public void run(){
			...
			if(mCount>=0) {
				...
				if(SystemProperties.get("ro.ysten.province","master").equals("cm201_guizhou")){
					...
					mSleepDelayTextView.setText(mContext.getResources().getString(com.android.internal.R.string.standby_warn_jiangxi_3am,mCount));
					Log.d(TAG, (mContext.getResources().getString(com.android.internal.R.string.standby_warn_jiangxi_3am,mCount)));
				}
				mSleepDelayTextView.setTextSize(25);
			}else{
				MediaPlayer mMediaPlayer = new MediaPlayer();
				if(SystemProperties.get("ro.ysten.province").equals("cm201_hebei")){
					//mMediaPlayer.shutdownStop(mMediaPlayer,mContext);
				}
				if(SystemProperties.get("ro.ysten.province","master").equals("cm201_guizhou")){
					...
					mSleepDelayTextView.setText(mContext.getResources().getString(com.android.internal.R.string.standby_warn_jiangxi_3am));
					Log.d(TAG, (mContext.getResources().getString(com.android.internal.R.string.standby_warn_jiangxi_3am)));
				}
				mSleepDelayTextView.setTextSize(25);
				mDialog.setCancelable(false);
				Log.d(TAG, "gotosleep start");
				mPowerManager.goToSleep(SystemClock.uptimeMillis());
	...
	private void resetPowerOff(){
		mDialog.dismiss();
		mBuilder = null;
		mSleepDelayTextView = null;
		mHandler.removeCallbacks(mSleepCancleTimer);
		String powerOffTime = Settings.System.getString(mContext.getContentResolver(),SYSTEM_POWER_OFF_TIME_DELAY);
		if(powerOffTime == null)
			return;
		long powerOffDelay = getPowerOffDelay(powerOffTime);
		Log.d("ysten_sgc", "PowerOffDelayReceiver in PowerManagerService , powerOffDelay = "+powerOffDelay);
		if(powerOffDelay > 0){
			mHandler.removeCallbacks(powerOffRunnable);
			Log.d("ysten_sgc","post powerOffRunnable Delayed in PowerOffDelayReceiver : "+powerOffDelay+"ms");
 			mHandler.postDelayed(powerOffRunnable,powerOffDelay);
		}
		mCount = 20;
	}
	...
	//end:add by zhuhengxuan at 20190706 for 定时关机弹窗
}
