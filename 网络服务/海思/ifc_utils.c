...
#define INCREASED_ETH_ROUTE_METRIC 128
...
/*
 * try to bind Link-local:546
 *    copy from device/hisilicon/bigfish/external/dhcp/common/socket.c
 *    if_register_socket(..)
 */
int if_register_socket(char* ifname, char *linklocal6)
{
    ...
    addr6->sin6_port = htons(546);
    inet_pton(AF_INET6, linklocal6, &addr6->sin6_addr);
    ...
    if (setsockopt(sock, SOL_SOCKET, SO_REUSEADDR,
        ...
        printerr("ifc_utils.c: setsockopt error!");
    }
...
/*
 * get IPv6 Link-local addresses on the specified interface.
 */
const char * ifc_get_ipv6_linklocal_addresses(const char *name, char *ipv6address) {
    ...
    // Format:
    // fe80000000000000d036d1fffeb4df39 02 40 20 80     eth0
    while (fscanf(f, "%32s %*02x %02x %*02x %*02x %63s\n",
        ...
        // on the interface.
        printerr("tianjia c Link-local: %s\n", addrstr);
        if (strncmp(addrstr, "fe80:", 5) == 0) {
            snprintf(ipv6address, sizeof(addrstr), "%s", addrstr);

            if (if_register_socket(name, ipv6address) != 0) {
                fclose(f);
                return NULL;
            }else{
                fclose(f);
      ...
      struct in6_addr gw)
{
    ...
    if (action == SIOCADDRT && !strcmp(route_priority, "eth") && !strncmp(ifname, "eth", strlen("eth"))) {
        // increaseEthRoutePriorityV6 for static
        ALOGD("increaseEthRoutePriorityV6 for static: action = %d, ifname = %s ", action, ifname);
        rtmsg.rtmsg_metric = INCREASED_ETH_ROUTE_METRIC;
    }
...
{
    int devfd = 0;
    char *dev_dir = "/sys/class/net/";
    char *interface_dir = NULL;
    unsigned flag = 0;
    const char *operstate = "/operstate";
    FILE *fp = NULL;
    char readbuf[10] = {0};

    interface_dir = (char *)malloc(strlen(ifname) + 15 + strlen(operstate) + 1);
    if(interface_dir == NULL) {
        ALOGE("Cannot alloc memory for ifc_get_netlink_status!");
        return -1;
    }
    strcpy(interface_dir,dev_dir);
    strcat(interface_dir,ifname);
    devfd = open(interface_dir, O_RDONLY);

    if (devfd < 0) {
        close(devfd);
        ALOGE("Cannot open \"%s\": %s", interface_dir, strerror(errno));
        free(interface_dir);
        interface_dir = NULL;
        return -1;
    } else {
        close(devfd);
        strcat(interface_dir,operstate);
        fp = fopen(interface_dir, "r");
        if(fp == NULL) {
            free(interface_dir);
            interface_dir = NULL;
            return -1;
            ...
            if(0 == strncmp(readbuf, "down", 4)) {
                ALOGE("Interface %s LinkDown!", ifname);
                fclose(fp);
                free(interface_dir);
                interface_dir = NULL;
                return 0;
            } else if (0 == strncmp(readbuf, "up", 2)) {
                ALOGE("Interface %s LinkUp!", ifname);
                fclose(fp);
                free(interface_dir);
                interface_dir = NULL;
                return 1;
            } else {
                ALOGE("Interface %s Status Read Error!", ifname);
                fclose(fp);
                free(interface_dir);
                interface_dir = NULL;
                return -1;
...
int ifc_remove_net_routes(const char *name)
{
    char ifname[64];
    in_addr_t dest, gway, mask;
    int flags, refcnt, use, metric, mtu, win, irtt;
    ...
    for (;;) {
        ...
        if (((flags & (RTF_UP)) != (RTF_UP)) || ((flags & (RTF_GATEWAY)) == (RTF_GATEWAY)) || ((flags & (RTF_HOST)) == (RTF_HOST))
                || strcmp(ifname, name) != 0) {
            continue;
        }
        memset(&rt, 0, sizeof(rt));
        ...
        addr.s_addr = dest;
        if (ioctl(ifc_ctl_sock, SIOCDELRT, &rt) < 0) {
            ALOGD("failed to remove net route for %s to %s: %s",
                 ifname, inet_ntoa(addr), strerror(errno));
        }
    }
    fclose(fp);
