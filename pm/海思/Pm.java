...
public final class Pm {
    ...
    private void runInstall() {
        ...
        while ((opt=nextOption()) != null) {
            if (opt.equals("-l")) {
            ...
            } else if (opt.equals("--ysten")) {
                installFlags |= PackageManager.INSTALL_REPLACE_EXISTING;
                ystenopt = nextOptionData();
            }else if (opt.equals("-i")) {
                ...
                // Override if -s option is specified.
				System.err.println("\t preInstall path: -m" );	
				
				String flag = nextOptionData();
				System.err.println("Error: no package specified flag="+flag);
                installFlags |=  Integer.parseInt(flag, 16);
            } else {
                System.err.println("Error: Unknown option: " + opt);
                return;
            }
        ...
        try {
            ...
            synchronized (obs) {
                ...
                if (obs.result == PackageManager.INSTALL_SUCCEEDED) {
                    if(ystenopt != null)
                    {
                        createNewFile(ystenopt,"/data/appinstallflags");
                    }
    ...
    private void createNewFile(String name ,String dir)
    {
            ...
            if(!file.exists()){
                try{
                    ...
                    Log.i("Pm", dir + "/" + name + " file error:" + e.getMessage());
                    e.printStackTrace();
                }
            }
    }
