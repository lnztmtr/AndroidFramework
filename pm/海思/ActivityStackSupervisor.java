...
public final class ActivityStackSupervisor {
    ...
    static final int START_HOME_MSG = FIRST_SUPERVISOR_STACK_MSG + 5;
	static final int START_INPUT_MSG = FIRST_SUPERVISOR_STACK_MSG + 6;

    private boolean mIsHomeActivityStarted = false;
    private final ProbeThread mProbeThread = new ProbeThread();
	...
	// begin: add by zhanghui at 20200928: add getTopActivity method
    public static String getTopFActivity(Context context) {
        try {
            ...
            Slog.d(TAG, "current app:" + e);
        }
        return "";
    }
    //end: add by zhanghui at 20200928: add getTopActivity method
    ...
    final int startActivityMayWait(IApplicationThread caller, int callingUid,
         ...
         //begin:add by zhanghk at 20190925:to start IsSetting.apk
        /*if(SystemProperties.get("ro.ysten.province","master").contains("shanxi")
           ...
           || SystemProperties.get("ro.ysten.province","master").contains("xinjiang")){
                if(intent.getComponent()!=null && intent.getComponent().getPackageName()!=null){
                    if((intent.getComponent().getPackageName()).contains("android.settings")){
                        ComponentName componentName = new ComponentName("com.ysten.setting","com.ysten.setting.MainActivity");
                        intent.setComponent(componentName);
                    }
                }
        }*/ 
        //end:add by zhanghk at 20190925:to start IsSetting.apk
		...
		if(SystemProperties.get("ro.ysten.province","master").contains("cm201_henan")){
			if(intent != null){
				if(intent.getComponent() != null && intent.getComponent().getClassName() != null){
						String topActivityName = intent.getComponent().getClassName();
						String pkgName = getTopFActivity(mContext);
						Slog.d("zhanghui", "TCN_ADD: **********topActivityName: " + topActivityName);
						Slog.d("zhanghui", "TCN_ADD: **********pkgName: " + pkgName);
						if("com.dangbei.msg.push.ui.DBMessageActivity".equals(topActivityName)){
							Slog.d(TAG, "TCN_ADD: not allow start DBMessageActivity before");
    ...
    final boolean realStartActivityLocked(ActivityRecord r,
        ...
        try {
            ...
            if (DEBUG_SWITCH) Slog.v(TAG, "Launching: " + r
                    ...
                    + " andResume=" + andResume);
            if (andResume) {
                Trace.traceBegin(Trace.TRACE_TAG_ACTIVITY_MANAGER, "am_restart_act");
        ...
        if (app != null && app.thread != null) {
            try {
                ...
                if(intent != null && intent.getComponent() !=null && intent.getComponent().getClassName() != null && SystemProperties.get("ro.ysten.province","null").contains("jiangsu")){
                    ...
                    Log.d("ActivityStackSupervisor","ActivityManager_sgc startNextMatchingActivity jiangsu start activity className is " + pkgName + "  clsName = " + clsName);
                    if(!TextUtils.isEmpty(clsName)   && !TextUtils.isEmpty(pkgName)){
                        Log.d("ActivityStackSupervisor","ActivityManager_sgc jiangsu start activity pkgName not null  clsName not null ");
                        if(TextUtils.equals("com.chinamobile.middleware.authjs",pkgName) && clsName.contains("TestActivty")){
                            //没有输错clsName,就是少个i
                            Log.d("ActivityStackSupervisor","ActivityManager_sgc jiangsu return -1  TestActivity");
                            return;
                        }
                    }
                }
    ...
    private void sendDownAndUpKeyEvents(int keyCode) {
		  ...
		  try{
                 java.lang.Process p =  java.lang.Runtime.getRuntime().exec("input keyevent "+ keyCode);
				 Slog.i(TAG, "input keyevent "+keyCode);  
             }catch(Exception e){
                 Slog.i(TAG, "dokey error -1");  
			}
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (err == ActivityManager.START_SUCCESS) {
            ...
            final String inFo = intent.toShortString(true, true, true, false);
            Slog.i(TAG, "START u" + userId +" {" + inFo
                    + "} from pid " + (callerApp != null ? callerApp.pid : callingPid));
            if(inFo != null && inFo.contains("com.shcmcc.setting/.NetworkActivity")) {
                final String disAble = Secure.getString(mContext.getContentResolver(), "cmcc_wifi_disabled");
            
                if(disAble.equals("1")) {
                    sendDownAndUpKeyEvents(KeyEvent.KEYCODE_DPAD_LEFT);
					Message msg = mProbeHandler.obtainMessage(START_INPUT_MSG);
                    mProbeHandler.sendMessageDelayed(msg,1000);
                }
		   }
		}
        if(SystemProperties.get("ro.ysten.province","master").contains("jidi")) {
            if(intent.toShortString(true, true, true, false).contains("tv.icntv.migu") ||
                intent.toShortString(true, true, true, false).contains("com.iflytek.aichang.tv")){
                    Slog.i(TAG,"now tv.icntv.migu");
                    mContext.sendBroadcast(new Intent("com.lhxk.voice_recognition_end"));
            }
        }
    ...
    final ActivityRecord activityIdleInternalLocked(final IBinder token, boolean fromTimeout,
        ...
        if (NT > 0) {
         ...
         boolean  needFinishBooting = !SystemProperties.get("sys.boot_completed", "0").equals("1");
        
        if (localLOGV) Slog.v(TAG, "needFinishBooting: "+needFinishBooting);
        if (isFrontStack(mHomeStack) || needFinishBooting) {
    ...
    final Handler mProbeHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
			Slog.i(TAG, ">>>>>>>penghui>>>dddd>>>>msg:"+msg.what);
            switch (msg.what) {
                case START_HOME_MSG: {
                    if (!mIsHomeActivityStarted) {
						 //add by baiyh not start HomeActivity when Dragonbox is running
						 String test1=SystemProperties.get("sys.yst.dragontest", "1");   
                        if(test1.contains("1")){
                            if(SystemProperties.get("ro.ysten.province","master").contains("jidi")) {
                                SystemProperties.set("persist.sys.hainancheck", "1");
                            }
							mService.startHomeActivityLocked(mCurrentUser);
							mIsHomeActivityStarted = true;
						}
                    }
                } break;
				case START_INPUT_MSG:{
    ...
    public class ProbeThread extends Thread {
        @Override
        public void run() {
            //add by xue not start HomeActivity when Dragonbox is running
           //persist.sys.hm
            String test1=SystemProperties.get("sys.yst.dragontest", "1");
			//add by weichuyan
			//begin add by ysten hyz at 20190318:remove the active android picture when start 
			String province = SystemProperties.get("ro.ysten.province","master");
			if("cm201_xinjiang".equals(province)){
			    if (isFastplayStopped() &&test1.contains("1")) {
                    if (mProbeHandler.hasMessages(START_HOME_MSG)) {
                        mProbeHandler.removeMessages(START_HOME_MSG);
                    }
                    Message msg = mProbeHandler.obtainMessage(START_HOME_MSG);
                    mProbeHandler.sendMessage(msg);
                }
			}
			else{
                if (isFastplayStopped() && isBootVideoStopped()&&test1.contains("1")) {
                    //if (mProbeHandler.hasMessages(START_HOME_MSG)) {
                      //  mProbeHandler.removeMessages(START_HOME_MSG);
                    //}
					Slog.i(TAG, ">>>>>>>penghui>>>>send msg");
                    Message msg = mProbeHandler.obtainMessage(START_HOME_MSG);
                    //mProbeHandler.sendMessage(msg);
					int time=0;
					try{
					  time = Integer.parseInt( SystemProperties.get("persist.sys.launcher.delaytime", "0") );
					}catch(Exception e){
						 Slog.i(TAG, ">>>>>>>penghui>>>>msg:error");
					}
					Slog.i(TAG, ">>>>>>>penghui>>>>time"+time);
					if(time>0){
						try {
						  Slog.i(TAG, ">>>>>>>penghui>>>>msg:1111");
						  SystemProperties.set("sys.boot_completed", "1");
                          Thread.sleep(time*1000);
                        }catch (Exception e){
                         e.printStackTrace();
                        }
						Slog.i(TAG, ">>>>>>>penghui>>>>msg:sleep");
						 mProbeHandler.sendMessage(msg);
						 //mProbeHandler.sendMessageDelayed(msg,time*1000);
					 }else{
						Slog.i(TAG, ">>>>>>>penghui>>>>msg:000");
						mProbeHandler.sendMessage(msg);
					 }
			...
			//end add by ysten hyz at 20190318:remove the active android picture when start 
        }
    }



    private boolean isFastplayStopped() {
        boolean bFullyPlay = "true".equals(SystemProperties.get("persist.sys.fastplay.fullyplay", "false"));
        if (!bFullyPlay) {
            Slog.i(TAG, "persist.sys.fastplay.fullyplay is false");
            return true;
        }
        Slog.i(TAG, "persist.sys.fastplay.fullyplay is true");
        boolean bPlayStop = false;
        Runtime rt = Runtime.getRuntime();
        while (true) {
            try {
                java.lang.Process proc = rt.exec("cat /proc/msp/mce");
                final InputStreamReader converter = new InputStreamReader(proc.getInputStream());
                BufferedReader in = new BufferedReader(converter);
                String line = "";
                while (true) {
                    line = in.readLine();
                    Slog.i(TAG, "----->line is: " + line);
                    if (line == null) {
                        break;
                    }
                    if (line.contains("bPlayStop")) {
                        if (line.contains("1")) {
                            Slog.i(TAG, "bPlayStop == 1");
                            bPlayStop = true;
                            break;
                        }
                    }
                ...
                Slog.e(TAG,"Error when executing cat /proc/msp/mce!");
            }
            if (bPlayStop) {
                break;
            }
    ...
    private boolean isBootVideoStopped() {
        boolean bVideoStop = false;
        while (true) {
            if(SystemProperties.get("service.lunch.wait.bootvideo", "false").equals("false")){
                Slog.i(TAG,"service.lunch.wait.bootvideo is true");
                return true;
            }
            bVideoStop = "1".equals(SystemProperties.get("service.bootanim.bootvideo.exit", "1"));
            if (bVideoStop) {
                Slog.i(TAG, "service.bootanim.bootvideo.exit is 1");
                break;
            }
            Slog.i(TAG, "service.bootanim.bootvideo.exit is 0");
            try {
                Thread.sleep(100);
            } catch (InterruptedException e) {
                Slog.e(TAG, "Sleep failed");
