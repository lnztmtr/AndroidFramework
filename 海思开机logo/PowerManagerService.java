...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    private boolean timeout_show = false;
    private int time_count = 30;
    private long power_time = 0;
    private String timehead = null;
    private String timetail = null;
    private final int TIMEOUT_FRESH = 100;
    ...
    private void updateSettingsLocked() {
        ...
        if(mScreenOffTimeoutSetting==-1){
            mScreenOffTimeoutSetting = 864000000;
        }
    ...
    private void goToSleepFromNative(long eventTime, int reason) {
        String suspendType = SystemProperties.get("persist.suspend.mode");
    ...
    private void goToSleepInternal(long eventTime, int reason) {
        int launcherValue=SystemProperties.getInt("persist.sys.launcher.value",-1);
        suspendGotoHomeOtt(launcherValue);
        SystemClock.sleep(400); 
        //mDisplayManagerService.setHDMIClose();
        //mDisplayManagerService.setOutputEnable(1,0); 
        synchronized (mLock) {
    ...
    private void suspendGotoHomeOtt(int value){	
		Intent home = new Intent();
                Log.i("yzs007", "suspendGotoHomeOtt is called and value is " + value);
		if(value == 1){//ott
	        SystemProperties.set("persist.sys.start_auto", "true");//???????EPG
			//if(SystemProperties.get("persist.sys.launcher.started","false").contains("true")){
			String user_token=getValueFromStb("user_token")+"";
			     Log.d("yzs007","uspendGotoHomeOtt is called and user_token="+user_token);
	    ...
	    }else if (value == 0){//persist.sys.launcher.value ?0????Provision
    ...
    private boolean goToSleepNoUpdateLocked(long eventTime, int reason) {
        ...
        switch (reason) {
            ...
            case PowerManager.GO_TO_SLEEP_REASON_TIMEOUT:
                ...
                //break;
                //return false;
                power_time = eventTime;
                if(mConfirmDialog==null&&!timeout_show){
                time_count = 30;
                timehead = mContext.getResources().getString(com.android.internal.R.string.power_timeout_msghead);
                timetail = mContext.getResources().getString(com.android.internal.R.string.power_timeout_msgtrail);
                mConfirmDialog = new AlertDialog.Builder(mContext)
                .setMessage(timehead+String.valueOf(time_count)+timetail)
                .setPositiveButton(com.android.internal.R.string.no, new DialogInterface.OnClickListener() {
              ...
              mConfirmDialog.show();
              timeout_show = true;
              mHandler.sendEmptyMessageDelayed(TIMEOUT_FRESH,1000);
        }
        return false;
        //break;
            default:
    ...
    private void checkIfBootAnimationFinished() {
        if (DEBUG) {
            ...
            //start by wcy sometimes enter debug after bootadmation
    ...
    public void reboot(boolean confirm, String reason, boolean wait) {
        Log.e("wcy","enter reboot in pms");
        mContext.enforceCallingOrSelfPermission(android.Manifest.permission.REBOOT, null);
        Log.e("wcy","after android.Manifest.permission.REBOOT");
        final long ident = Binder.clearCallingIdentity();
        try {
            Log.e("wcy","ready shutdownreboot");
            shutdownOrRebootInternal(false, confirm, reason, wait);
            Log.e("wcy","after shutdownreboot");
        } finally {
            Binder.restoreCallingIdentity(ident);
        }
    }
    ...
    public static void lowLevelReboot(String reason) {
		Log.e("rebootwcy","enter lowlevelReboot in PMS");
        if (reason == null) {
            reason = "";
        }
        SystemProperties.set("sys.powerctl", "reboot," + reason);
		Log.e("rebootwcy","After setprop sys.powerctl = reboot");
        try {
            Thread.sleep(20000);
        } catch (InterruptedException e) {
    ...
    private final class PowerManagerHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
			...
			if(time_count>0){
			time_count--;
			mConfirmDialog.setMessage(timehead+String.valueOf(time_count)+timetail);
			mHandler.sendEmptyMessageDelayed(TIMEOUT_FRESH,1000);
		    }else{
			goToSleep(power_time, PowerManager.GO_TO_SLEEP_REASON_USER);
		    }
		    }
                    break;
            }
