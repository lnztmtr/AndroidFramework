...
final class ActivityStack {
    ...
    /**Add for Some apk need black out policy*/
    private final ArrayList<String> mAppBlackoutWhitelist = new ArrayList<String>();
    ...
    */
    ...
    native int WaitPlayEnd();
    native int FreescaleisEnable();
    ...
    final void startPausingLocked(boolean userLeaving, boolean uiSleeping) {
        ...
        if (next == null || next.task != prev.task) {
            prev.updateThumbnail(null, null);
        }
        ...
        if (null != next && null != prev && null != prev.packageName
            && null != next.packageName && prev.packageName.equals("im.yixin.tv")
            && !next.packageName.equals("im.yixin.tv")) {
          Slog.d(TAG , "prev.package name:"+prev.packageName);
          Slog.d(TAG , "next.package name:"+next.packageName);
          destroyActivityLocked(prev, true,false,"force-destroiy");
        }
    }
    ...
    final boolean readBlackoutWhiteFilter(){
        try{
            BufferedReader br = new BufferedReader(new InputStreamReader(
            new FileInputStream("/system/etc/blackout_whitelist.txt")));

            String line ="";
            while ((line = br.readLine()) != null){
                mAppBlackoutWhitelist.add(line);
            }
    ...
    final boolean PackageinBlackoutWhiteFilter(String packagename){
        Iterator<String> it = mAppBlackoutWhitelist.iterator();
        while (it.hasNext()) {
            String whitelisItem = it.next();
            if (whitelisItem.equals(packagename)) {
                Slog.d(TAG, packagename + " in whitelist");
                return true;
            }
        }
        return false;
    ...
    final boolean resumeTopActivityLocked(ActivityRecord prev, Bundle options) {
        ...
        if((next != null) && (next.packageName != null)){
            ...
            if(DEBUG_STACK)Slog.d(TAG , "the package implement intent's componet ===" + pkg);
            if ( (pkg != null) && !pkg.equals("com.magicandroidapps.iperf")
                        &&(!pkg.equals("com.ixia.ixchariot"))){
                SystemProperties.set("sys.wifi.perf" , "off");
                if(DEBUG_STACK)Slog.d(TAG , " wifi_perf off");
            }
        }
        ...
        if (next.app != null && next.app.thread != null) {
            ...
            try {
    			...
    			if (null != resumePackageName && !(resumePackageName.contains("com.antutu") || resumePackageName.contains("com.chinamobile.sh.test"))){
					SystemWriteManager sw = (SystemWriteManager)mContext.getSystemService("system_write");
					sw.writeSysfs("/sys/class/remount/persistent", "1");
			}

                // begin: add by tianchining at 20190522: fix bug when Switch channel
                if ("1".equals(SystemProperties.get("persist.sys.sc_nx.flag", "0"))
                        && resumeClassName.equals("cn.gd.snm.player.LivePlayerActivity")){
                    Slog.d(TAG, "resume Activity is LivePlayerActivity need to skip video axis set");
                    SystemProperties.set("media.amplayer.nxmobile_live", "true");
                } else {
                    Slog.d(TAG, "resume Activity is not LivePlayerActivity, no need skip, set false");
                    SystemProperties.set("media.amplayer.nxmobile_live", "false");                    
                }
                // end: add by tianchining at 20190522: fix bug when Switch channel


                    //final ContentResolver resolver = mContext.getContentResolver();
                    SystemWriteManager sw = (SystemWriteManager)mContext.getSystemService("system_write");
                    String displast = SystemProperties.get("media.amplayer.displast_frame");
                    if(PackageinBlackoutWhiteFilter(resumePackageName)
                       &&PackageinBlackoutWhiteFilter(resumeClassName)){
                       SystemProperties.set("media.amplayer.displast_frame", "1");
                       //sw.writeSysfs("/sys/class/video/disable_video","2");
                       // begin: add by tianchining at 20190522: fix bug when Switch channel
                       if("1".equals(SystemProperties.get("persist.sys.sc_nx.flag", "0")) ){
                           // do nothing
                       }else{
                    ...
                    // begin: add by tianchining at 20190522: fix bug when Switch channel 
                    if("1".equals(SystemProperties.get("persist.sys.sc_nx.flag", "0"))){
                        String vfm_map = sw.readSysfs("/sys/class/vfm/map");
                        Slog.d(TAG, "resume activity player is ruuning:" + vfm_map.indexOf("decoder(1)"));
                        if (vfm_map.indexOf("decoder(1)") == -1) {
                            sw.writeSysfs("/sys/class/video/disable_video","2");
                        }
                    } 
                    // end: add by tiancnining at 20190522: fix bug when Switch channel 
    ...
    final void startActivityLocked(ActivityRecord r, boolean newTask,
        ...
        if (r.packageName.equals("net.sunniwell.service.swupgrade.usb")){
            File f = new File("/cache/upgrade/update.zip");
            if(f.exists()){
                f.delete();
            }
        }
        if (r.packageName.contains("com.antutu") || r.packageName.contains("com.chinamobile.sh.test")){
	 	SystemWriteManager sw = (SystemWriteManager)mContext.getSystemService("system_write");
		sw.writeSysfs("/sys/class/remount/persistent", "0");
        }
        if(r.packageName.contains("com.ixia.ixchariot")
            || r.packageName.contains("com.magicandroidapps.iperf")){
            ...
            Slog.d(TAG , "start wifi type : "+wifi_type);
            if(null != wifi_type && wifi_type.contains("4356"))
				SystemProperties.set("sys.wifi.perf" , "on");
            else if(null != wifi_type && wifi_type.contains("050a")){//6174
				SystemProperties.set("ctl.start" , "start_wifi_6174");
            }
        }
        ...
        String startClassName = r.realActivity.getClassName();
        String startPackageName = r.realActivity.getPackageName();
        SystemWriteManager sw = (SystemWriteManager)mContext.getSystemService("system_write");
        String displast = SystemProperties.get("media.amplayer.displast_frame");
        
        // begin: add by tiancnining at 20190522: fix bug when Switch channel
        if("1".equals(SystemProperties.get("persist.sys.sc_nx.flag", "0"))
            && startClassName.equals("cn.gd.snm.player.LivePlayerActivity")){
            Slog.d(TAG, "start Activity is LivePlayerActivity need to skip video axis set");
            SystemProperties.set("media.amplayer.nxmobile_live", "true");
        }
        else {
            Slog.d(TAG, "start Activity is not LivePlayerActivity, no need skip, set false");
            SystemProperties.set("media.amplayer.nxmobile_live", "false");
        }
        // end: add by tiancnining at 20190522: fix bug when Switch channel

        if(PackageinBlackoutWhiteFilter(startPackageName)
            &&PackageinBlackoutWhiteFilter(startClassName)){
            SystemProperties.set("media.amplayer.displast_frame", "1");
            //sw.writeSysfs("/sys/class/video/disable_video","2");
            // begin: add by tiancnining at 20190522: fix bug when Switch channel
            if("1".equals(SystemProperties.get("persist.sys.sc_nx.flag", "0"))){
        ...
        if("1".equals(SystemProperties.get("persist.sys.sc_nx.flag", "0"))){
            String vfm_map = sw.readSysfs("/sys/class/vfm/map");
            Slog.d(TAG, "start activity player is ruuning:" + vfm_map.indexOf("decoder(1)"));
            if (vfm_map.indexOf("decoder(1)") == -1) {
                sw.writeSysfs("/sys/class/video/disable_video","2");
            }
        }
            ...
            if (activityNdx >= 0) {
                ...
                if (r.state == ActivityState.RESUMED
                    ...
                    if (!r.isHomeActivity() || mService.mHomeProcess != r.app) {
                        if (!"com.android.smart.terminal.nativeplayer/.IndexActivity".equals(
                                r.intent.getComponent().flattenToShortString())) {
                            Slog.w(TAG, "  Force finishing activity "
                                    + r.intent.getComponent().flattenToShortString());
                            finishActivityLocked(r, Activity.RESULT_CANCELED, null, "crashed", false);
