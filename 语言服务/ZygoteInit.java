...
public class ZygoteInit {
    ...
    //begin: add by ysten.tianchining at 20190408: add concurrent_preload_ningxia
    public static class PreloadOGLThread extends Thread
    {
        public void run()
        {
            preloadOpenGL();
    ...
    static void concurrent_preload_ningxia() {
        Log.d(TAG, "TCN_ADD: ------> concurrent_preload_ningxia");
        Thread preloadclass = new PreloadClassThread();
        Thread preloadres = new PreloadResThread();
        Thread preloadOGL = new PreloadOGLThread();
        ...
        try {
            ...
            Log.e(TAG, "TCN_ADD: concurrent_preload join() InterruptedException error ", ex);
        }
    }   
    //end: add by ysten.tianchining at 20190408: add concurrent_preload_ningxia
    ...
    private static void handleSystemServerProcess(
        ...
        Log.d(TAG, "BootStage handleSystemServerProcess");
        closeServerSocket();
    ...
    private static boolean startSJOS()
        ...
        String args[] = {
            ...
            "--runtime-init",
            "--nice-name=sjos_system_server",
            "com.sjos.android.server.SystemServer",
        };
     ...
     * Finish remaining work for the newly forked system server process.
     */
    private static void handleSJOSSystemServerProcess(
            ZygoteConnection.Arguments parsedArgs)
            throws ZygoteInit.MethodAndArgsCaller {
    ...
    public static void main(String argv[]) {
        try {
            ...
            Log.d(TAG, "BootStage start  preload stage");
            if(!SystemProperties.getBoolean("config.enable_quickboot", false)) {
                ...
                boolean useConcurrentPreload = SystemProperties.getBoolean("config.concurrent_preload", true);
                if(useConcurrentPreload) {
                    concurrent_preload();
                } else {
                    preload();
                }
                long endtime = SystemClock.uptimeMillis();
                EventLog.writeEvent(LOG_BOOT_PROGRESS_PRELOAD_END, endtime + " useConcurrentPreload:" + useConcurrentPreload + " cost time:" + ( endtime - begintime) );
            }
            ...
            Log.d(TAG, "BootStage fork sytemserver");
            ...
            //begin: add by ysten.tianchinig at 20190408: add concurrent_preload_ningxia
            if(SystemProperties.get("ro.ysten.province").equals("CM201_ningxia")){
                Log.d(TAG, "TCN_ADD: ningxia ==> preload src!!!");
                long begintime = SystemClock.uptimeMillis();
                //EventLog.writeEvent(LOG_BOOT_PROGRESS_PRELOAD_START, begintime);
                concurrent_preload_ningxia();
                //concurrent_preload();
                //preload();
            //end: add by ysten.tianchinig at 20190408: add concurrent_preload_ningxia
            }else{
