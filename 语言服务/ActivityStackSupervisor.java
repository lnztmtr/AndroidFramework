...
public final class ActivityStackSupervisor {
    ...
    final int startActivityMayWait(IApplicationThread caller, int callingUid,
         ...
         //begin:add by xuyunfeng at 20191122:Empty pointer judgment
        /*if(intent.getComponent()!=null)
        {
            Slog.d(TAG, "intent.getComponent() = " + intent.getComponent());
            if(intent.getComponent().getPackageName()!=null)
            {
         //end:add by xuyunfeng at 20191122:Empty pointer judgment
                Slog.d(TAG, "intent.getComponent().getPackageName() = " + intent.getComponent().getPackageName());
    ...
    if(SystemProperties.get("ro.ysten.province","master").contains("A20_jidi")){
        ...
        //begin:add by zhanghk at 20191231:forbid start different bussiness apk
        if(SystemProperties.get("ro.ysten.province").contains("hubei")){
            String bussinessType=SystemProperties.get("persist.sys.hb.bussiness");
            if("bestv".equals(bussinessType)&&className.contains("com.android.smart.terminal.iptv")){
                Slog.i(TAG,"not allow iptv start when select ott");
                return -1;                                              
            }
            if("iptv".equals(bussinessType)&&(className.contains("com.bestv.ott"))){
                Slog.i(TAG,"not allow bestv start when select iptv");
                return -1;                                              
            }
        }
        //end:add by zhanghk at 20191231:forbid start different bussiness apk
        ...
        if (SystemProperties.getBoolean("ro.jsmobile.launcher", false)) {
            Intent started = new Intent("android.intent.action.ACTIVITY_STARTED");
            started.putExtra("name", aInfo.applicationInfo.packageName);
            mContext.sendBroadcast(started);
        }
