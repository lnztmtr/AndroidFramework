    ...
    private IActivityManager.ContentProviderHolder installProvider(Context context,
        ...
        if (holder == null || holder.provider == null) {
            ...
            try {
                ...
                //if databases folder is not a folder,try delete it
                String pathPkg = "/data/data/" + c.getPackageName();
                File folderPkg = new File(pathPkg);
                if( folderPkg.exists() && !folderPkg.isDirectory()) {
                    Slog.e(TAG, "call android.database.sqlite.SQLiteCantOpenDatabaseException folder " + pathPkg + " is not a dir, delete it");
                    try {
                        folderPkg.delete();
                    } catch (Exception err) {
                    }
                ...
                String pathDb = "/data/data/" + c.getPackageName() + "/databases";
                File folderDb = new File(pathDb);
                if( folderDb.exists() && !folderDb.isDirectory()) {
                    Slog.e(TAG, "call android.database.sqlite.SQLiteCantOpenDatabaseException folder " + pathDb + " is not a dir, delete it");
                    try {
                        folderDb.delete();
                    } catch (Exception err) {
                    }
